"""
JAX Implementation of ROCKE-3D PBL (Planetary Boundary Layer) Subroutines
========================================================================
...
def find_dpsim(zet: jnp.ndarray, zet0: jnp.ndarray) -> jnp.ndarray:
    ...
    term2_unstable = jnp.log((1 + x0) * (1 + x0) * (1 + x0 * x0))
    term3_unstable = 2 * (jnp.arctan(x) - jnp.arctan(x0))
    dpsim_unstable_gt = term1_unstable - term2_unstable - term3_unstable
    
    # Unstable: zet <= zetm
    term1_unstable_le = jnp.log((1 + xm) * (1 + xm) * (1 + xm * xm))
    term2_unstable_le = jnp.log((1 + x0) * (1 + x0) * (1 + x0 * x0))
    ...
    dpsim_unstable_le = term1_unstable_le - term2_unstable_le - term3_unstable_le + term4_unstable_le - term5_unstable_le
