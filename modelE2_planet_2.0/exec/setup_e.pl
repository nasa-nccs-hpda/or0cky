...
if ( ! -f "$ModelDir/$runID.bin" ) {
    print "$ModelDir/$runID.bin","not found\n";
    exit 1;
}

`mv -f "$ModelDir/$runID.bin" $runID.bin`;
chmod 0777 & $umask_inv, "$runID.bin";
...
\#!/bin/sh
    ...
    $mpi_start
    if [ \$debug -eq 1 ] ; then
      $run_command \$DEBUG_COMMAND ./${runID}.bin -i ./\$IFILE \$opts
    else
      $run_command ./${runID}.bin -i ./\$IFILE \$opts > \$PRTFILE
    fi
    $mpi_stop
...
if (exists $ENV{LOADEDMODULES}) {
    my $loadedmodules = $ENV{LOADEDMODULES};
    $loadedmodules =~ s/:/ /g;
    open MODULES, ">modules" or die "can't open 'modules' for writing\n";
    print MODULES "$loadedmodules\n";
    close MODULES;
}
