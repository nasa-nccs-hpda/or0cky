...
!@calls RADPAR:RCOMP1, ORBPAR
...
#endif
      use ghgmod, only : GHG_exists,fulgas
      USE RADPAR, only : !rcomp1,writer,writet       ! routines
     &      fOnOff, PTLISO ,LMR=>NL, PLB, LS1_loc
     ...
     *     ,KYEARD,KJDAYD,MADDST, KYEARV,KJDAYV,MADVOL, KYEARVM
     *     ,KYEARE,KJDAYE,MADEPS, KYEARR,KJDAYR
     *     ,KCLDEP
!g95     *     ,FSXAER,FTXAER    ! scaling (on/off) for default aerosols
     *     ,ITR,ITRoma,nraero_aod=>NTRACE ! turning on options for extra aerosols
...
#endif
      USE GHGMOD, ONLY: H2O_PPM, CO2_PPM, CH4_PPM
     *                 ,KGGVDF,KPGRAD,KLATZ0,KGHGZD
#ifdef TRACERS_PHOTOLYSIS
      use photolysis, only: aer2ext,aer2sca,aer2g,nbfastj
     ...
     *     ,snoage_def
     *     ,s0_yr,s0_day,ghg_yr,ghg_day,volc_yr,volc_day,volc_mean
     *     ,aero_yr,dust_yr,O3_yr
     *     ,H2ObyCH4,dH2O,RHfix,CLDx,ref_mult,COSZ1
     ...
     *     ,scatau_as,g_as
#ifdef CACHED_SUBDD
     *     ,abstau_as,abstau_cs,abstau_dry,swftoa,swfsfc,lwfrc
#endif  /* CACHED_SUBDD */
#endif  /* TRACERS_ON */
#ifdef ALTER_RADF_BY_LAT
     *     ,FULGAS_lat,FS8OPX_lat,FT8OPX_lat
...
#ifdef USE_PLANET_RAD
      use ghgmod, only : 
     &    CO2X, O2X, NO2X, N2OX, CH4X, CFC11X, CFC12X, XGHGX, YGHGX,
     &    N2CX, SO2X, O3X
      USE planet_rad, ONLY:
     &    dir_solar_spec, file_solar_spec,
     &    dir_spectral, file_spectral_lw, file_spectral_sw,
     &    file_aer_opt_prop_lw, file_aer_opt_prop_sw,
     &    file_aer_opt_prop_diag,
     &    solar_constant, i_cloud_ice_size_scheme, l_cloud,
     &    control_lw, control_sw, l_aerosol_diag,
     &    aer_scaling_diag
      USE lw_control, ONLY:
     &    l_co2_lw, l_o2_lw, l_no2_lw, l_n2o_lw,
     &    l_ch4_lw, l_cfc11_lw, l_cfc12_lw, l_so2_lw, l_o3_lw,
     &    l_aerosol_lw
      USE sw_control, ONLY:
     &    l_co2_sw, l_o2_sw, l_no2_sw, l_n2o_sw,
     &    l_ch4_sw, l_cfc11_sw, l_cfc12_sw, l_so2_sw, l_o3_sw,
     &    l_aerosol_sw
#endif
     ...
     &     l_rad_interact_aer
      ...
      USE radpar, ONLY: dbrad_profile
#endif
#ifdef DEBUG_RADIATION_KEEP
      USE radpar, ONLY: dbrad_ilon, dbrad_jlat, dbrad_itime
#endif
#if defined DEBUG_RADIATION || defined DEBUG_RADIATION_KEEP
      use radpar, only : dbrad_aerosol
#endif
...
#ifdef USE_PLANET_RAD
C     Disable aerosols if aer_opt_prop files have not been not specified
      l_aerosol_lw = is_set_param('aer_opt_prop_lw')
      l_aerosol_sw = is_set_param('aer_opt_prop_sw')

C     Enable aerosol diagnostics only file has been provided
      l_aerosol_diag = is_set_param('aer_opt_prop_diag')
#endif
...
C**** sync radiation parameters from input
      ...
      call sync_param( "H2O_PPM", H2O_PPM )
      call sync_param( "CO2_PPM", CO2_PPM )
      call sync_param( "CH4_PPM", CH4_PPM )
      call sync_param( "l_uniform_ghg", l_uniform_ghg )
      call sync_param( "KCLDEP", KCLDEP)
      call sync_param( "EPSCON", EPSCON)
#ifdef USE_PLANET_RAD
      ...
      call sync_param('solar_spec_dir', dir_solar_spec)
      call sync_param('solar_spec', file_solar_spec)
      call sync_param('spectral_dir', dir_spectral)
      call sync_param('spectral_file_lw', file_spectral_lw)
      call sync_param('spectral_file_sw', file_spectral_sw)
      if (l_aerosol_lw)
     &    call sync_param('aer_opt_prop_lw', file_aer_opt_prop_lw)
      if (l_aerosol_sw)
     &    call sync_param('aer_opt_prop_sw', file_aer_opt_prop_sw)
      if (l_aerosol_diag)
     &    call sync_param('aer_opt_prop_diag', file_aer_opt_prop_diag)
      call sync_param('planet_s0', solar_constant)
      call sync_param('i_cloud_ice_size_scheme',
     &    i_cloud_ice_size_scheme)
      call sync_param('l_cloud', l_cloud)
#endif
      call sync_param('FTTAER', FTTAER)
      call sync_param('FSTAER', FSTAER)
#ifndef GISS_RAD_OFF
      ...
      call sync_param( "volc_mean", volc_mean, 2 )
      ...
      if(.not.file_exists('BCdalbsn')) dALBsnX = 0
      ...
      call sync_param( "cloud_aer_o3_rad_forc", cloud_aer_o3_rad_forc )
      call sync_param( "TAero_aod_diag", TAero_aod_diag )
      call sync_param( "aer_rad_forc", aer_rad_forc )
      call sync_param( "ref_mult", ref_mult )
#ifdef DEBUG_RADIATION
      call sync_param( "dbrad_profile", dbrad_profile )
#endif
#ifdef DEBUG_RADIATION_KEEP
      call sync_param( "dbrad_ilon", dbrad_ilon )
      call sync_param( "dbrad_jlat", dbrad_jlat )
      call sync_param( "dbrad_itime", dbrad_itime )
#endif
#if defined DEBUG_RADIATION || defined DEBUG_RADIATION_KEEP
      call sync_param( "dbrad_aerosol", dbrad_aerosol )
#endif
#ifdef TRACERS_ON
...
#ifdef USE_PLANET_RAD
C     Disable aerosols in planet radiation if scaling is zero
      IF (FTTAER == 0.0D+00) l_aerosol_lw = .FALSE.
      IF (FSTAER == 0.0D+00) l_aerosol_sw = .FALSE.
#endif

C     Turn off GHG distributions if requested
      IF (l_uniform_ghg) THEN
        KGGVDF=0
        KPGRAD=0
        KLATZ0=0
        KGHGZD=0
...
#ifdef PLANET_PARAMS
C     Check H2ObyCH4 flag and print warning if it is on.
      IF (am_i_root() .AND. H2ObyCH4 > 0) THEN
        WRITE(0,'(a)') '*** Warning: H2ObyCH4 > 0. ***'
        WRITE(6,'(a)') '*** Warning: H2ObyCH4 > 0. ***'
      END IF

C     Check GHG distribution flags and print warning to both .PRT file
C     and terminal if not using uniform GHG distributions.
      IF (am_i_root() .AND. (KGGVDF > 0 .OR. KPGRAD > 0 .OR.
     &    KLATZ0 > 0 .OR. KGHGZD > 0)) THEN
        WRITE(0,'(a,/,a)') '*** Warning: Prescribed GHG ' //
     &    'amounts will vary with height and/or latitude. ***',
     &    'Set l_uniform_ghg=1 in your rundeck to avoid this.'
        WRITE(6,'(a,/,a)') '*** Warning: Prescribed GHG ' //
     &    'amounts will vary with height and/or latitude. ***',
     &    'Set l_uniform_ghg=1 in your rundeck to avoid this.'
      END IF
#endif
...
C****                                         even if the year is fixed
      ...
      KYEARVM=volc_mean
      if(file_exists('RADN7')) then ! Prescribed volcanic aerosols
        MADVOL=1
...
#else
      if(file_exists('MINERALSaer'))   MADDST=1   ! Mineral components
#endif
      if(file_exists('RADN8')) THEN
        MADEPS=1   ! cloud Epsln - KCLDEP
      else if (.not. is_set_param("KCLDEP")) then
        KCLDEP=0 ! cldeps off
      end if
...
!TOMAS does not include NO3 AND VOL, which use its default radiation.
      ...
      if (nraero_aod>0) then
        ...
        if (.not.allocated(tau_as)) then
          allocate(tau_as(I_0:I_1,J_0:J_1,lm,nraero_aod))
          allocate(tau_cs(I_0:I_1,J_0:J_1,lm,nraero_aod))
          allocate(scatau_as(I_0:I_1,J_0:J_1,lm,nraero_aod))
          allocate(g_as(I_0:I_1,J_0:J_1,lm,nraero_aod))
          tau_as = 0.d0
          tau_cs = 0.d0
          scatau_as = 0.d0
          g_as   = 0.d0
          if (save_dry_aod>0) then
            allocate(tau_dry(I_0:I_1,J_0:J_1,lm,nraero_aod))
            tau_dry = 0.d0
          endif
#ifdef CACHED_SUBDD
          allocate(abstau_as(I_0:I_1,J_0:J_1,lm,nraero_aod))
          allocate(abstau_cs(I_0:I_1,J_0:J_1,lm,nraero_aod))
          abstau_as = 0.d0
          abstau_cs = 0.d0
          if (save_dry_aod>0) then
            allocate(abstau_dry(I_0:I_1,J_0:J_1,lm,nraero_aod))
            abstau_dry = 0.d0
          endif
          if (nraero_rf>0) then
            allocate(swftoa(I_0:I_1,J_0:J_1,nraero_rf))
            allocate(swfsfc(I_0:I_1,J_0:J_1,nraero_rf))
            allocate(lwfrc(I_0:I_1,J_0:J_1,nraero_rf))
            swftoa = 0.d0
            swfsfc = 0.d0
            lwfrc = 0.d0
          endif
      ...
      allocate(aer2ext(nbfastj,njaero))
      allocate(aer2sca(nbfastj,njaero))
      allocate(aer2g(nbfastj,njaero))
#endif  /* TRACERS_PHOTOLYSIS */
...
!-----------------------------------------------------------------------
      ...
      if (nraero_seasalt > 0) then
        ...
        itroma(n+1:n+nraero_seasalt)=(/5,5/)
      endif
      n=n+nraero_seasalt
...
#endif  /* TRACERS_AEROSOLS_SOA */
!     &       ,5,6/)
     &                          ,6,6/)  !Looks like denaer(5) was wrong for BCIA  SEB                              
        itroma(n+2:n+nraero_koch)=(/
#ifdef TRACERS_AEROSOLS_VBS
     &                             3
#else
     &                             3,3
...
c        write(6,*) "subClayWeights = ", subClayWeights
...
! Particle density of dust
     ...
     &                             /) * 1d-3 ! Convert from kg/m^3 to g/cm^3
        ...
        itroma(n+1:n+nraero_dust) = 4 ! all dust cases, outside ifdefs            
...
! set default FSTOPX and FTTOPX values
      ...
      if ( use_offline_aerosols ) then
        FSTOPX(:)=1.d0 ; FTTOPX(:)=1.d0
      end if
#ifdef USE_PLANET_RAD
      aer_scaling_diag(:) = 0.d0
#endif
      l_rad_interact_aer = rad_interact_aer > 0

      GHG_exists = file_exists('GHG')
      if (GHG_exists) then
...
#ifdef GISS_RAD_OFF
      nrfun(2)  = donotread ! Do not read long-wave tau-table
      nrfun(5)  = donotread ! Do not read long-wave water continuum
      nrfun(14) = donotread ! Do not read long-wave scattering corrections
#endif
      DO IU=1,14
...
C     ------------------------------------------------------------------
#ifdef USE_PLANET_RAD
C     Set which gases are to be included in radiation
      CALL set_control_gas(l_co2_lw, l_o2_lw, l_no2_lw, l_n2o_lw,
     &    l_ch4_lw, l_cfc11_lw, l_cfc12_lw, l_so2_lw, l_o3_lw, fulgas,
     &    CO2X, O2X, NO2X, N2OX, CH4X, CFC11X, CFC12X, XGHGX, YGHGX,
     &    N2CX, SO2X, O3X, control_lw)
      CALL set_control_gas(l_co2_sw, l_o2_sw, l_no2_sw, l_n2o_sw,
     &    l_ch4_sw, l_cfc11_sw, l_cfc12_sw, l_so2_sw, l_o3_sw, fulgas,
     &    CO2X, O2X, NO2X, N2OX, CH4X, CFC11X, CFC12X, XGHGX, YGHGX,
     &    N2CX, SO2X, O3X, control_sw)
#endif
      CALL RCOMP1 (NRFUN)
...
C              CCL3F1    CCL2F2   N2     CFC-Y       CFC-Z         SO2
     ...
     &           *((1d5/mair)*(gasc*tf/101325d0))
         ...
         !fulgas = 1. ! needed?
          ...
          !ulgas(:, 1) = ulgas(:, 1)*fulgas(1) 
          do n=2,13
            ulgas(:,n) = ulgas(:,n)*fulgas(n)
          end do
...
!@calls RADPAR:RCOMPT
     ...
     *     ,FULGAS_orig
      ...
      use radpar, only : dbrad_profile
#endif
      USE GHGMOD, ONLY: ip_h2o, ip_co2, ip_o3, ip_o2, ip_no2, ip_n2o
     &  , ip_ch4, ip_cfc11, ip_cfc12, ip_n2c, ip_xghg, ip_yghg, ip_so2
     &  , co2x,n2ox,ch4x,cfc11x,cfc12x,xGHGx,h2ostratx
     &  , o2x,no2x,n2cx,yghgx,so2x,o3x
      USE RAD_COM, only : 
     *      o3_yr,ghg_yr,co2ppm,Volc_yr,albsn_yr,dalbsnX
...
C**** Update orbital parameters at start of year
      ...
      halfDay = newTimeInterval(modelEclock%getSecondsPerDay() / 2)
...
!     components [e.g. a placeholder for tunable or more complex scheme]
      ...
      use resolution, only : lm,im,jm
      use Dictionary_mod, only: sync_param
      use constant, only : teeny
      use model_com, only: modelEclock, itime
      use ghgmod, only: apply_offline_dQ_to_NINT,ppmv_to_cm_at_stp
      use ghgmod, only : getgas,lxghg
      ...
      real*8, dimension(:,:,:),allocatable::dQ,dQoh,dQo1d,dQcl,dQsf3,CH4
...
!@calls tropwmo,coszs,coszt, RADPAR:rcompx ! writer,writet
      use Zenith_mod, only: calc_zenith_angle
      use RunTimeControls_mod, only: tracers_amp, tracers_tomas,
     &     oma_tramprad, use_offline_aerosols
      USE CONSTANT, only : lhe,lhs,twopi,tf,stbo,rhow,mair,grav
     *     ,bysha,pi,radian,areag,loschmidt_constant
     ...
     &     ,lat2d,lon2d
      ...
      use ghgmod, only : co2x,CH4X_RADoverCHEM
#ifdef GCC_COUPLE_RAD
     *            ,GCCco2_IN,use_tracer_GCCco2
#endif
      USE RADPAR
     &  , only :  ! routines
...
C     OUTPUT DATA
     ...
     &          ,TRDFLB_CLEAR ,TRNFLB_CLEAR ,TRUFLB_CLEAR
     &          ,SRDFLB_CLEAR ,SRNFLB_CLEAR ,SRUFLB_CLEAR
#endif
     ...
     &          ,aesqex,aesqsc,aesqcb,aesasy,ibd,CO2outCol
     &          ,aesqex_dry,aesqsc_dry,aesqcb_dry
     &          ,frac_so4_has_nh4,SRXNIR,SRDNIR
#ifdef USE_PLANET_RAD
      USE planet_rad, ONLY: npd_layer, p_level, t_layer,  w_cloud_l,
     &    frac_st_water_l, frac_st_ice_l, frac_cnv_water_l,
     &    frac_cnv_ice_l, mix_ratio_st_water_l,
     &    mix_ratio_st_ice_l, mix_ratio_cnv_water_l,
     &    mix_ratio_cnv_ice_l, dim_char_st_water_l,
     &    dim_char_st_ice_l, dim_char_cnv_water_l, dim_char_cnv_ice_l,
     &    frac_area_st_l, frac_area_cnv_l, cld_scaling,
     &    cos_zenith, solar_constant, solar_irrad,
     &    tot_cloud_cover,
     &    l_aerosol_diag
      USE lw_control, ONLY: l_clear_lw
      USE sw_control, ONLY: l_clear_sw
#endif
#if defined(USE_PLANET_RAD) && defined(GISS_RAD_OFF)
      USE RADPAR, ONLY: TRUPTOA, SRUPTOA, SRDNTOA
      USE planet_rad, ONLY: n_band_lw, n_band_sw, n_band_diag,
     &    i_cloud_ice_size_scheme
#endif
     ...
     *     ,ghg_yr,rad_forc_lev
     *     ,ntrix_aod,ntrix_rf,wttr,cloud_rad_forc,cloud_aer_o3_rad_forc
     *     ,CC_cdncx,OD_cdncx,cdncl,dALBsnX,rad_to_chem,trsurf,dirvis
...
#ifdef PCLD_INDICES_LOCAL
     *    ,cld_plow,cld_pmid
#endif
#ifdef USE_PLANET_RAD
      USE CLOUDS_COM, ONLY: w_cloud, frac_st_water, frac_st_ice,
     &    frac_cnv_water, frac_cnv_ice, mix_ratio_st_water,
     &    mix_ratio_st_ice, mix_ratio_cnv_water,
     &    mix_ratio_cnv_ice, dim_char_st_water,
     &    dim_char_st_ice, dim_char_cnv_water, dim_char_cnv_ice,
     &    frac_area_st, frac_area_cnv
#endif
     ...
     *     ,n_OCIA,N_OCII,n_so4_d2,n_so4_d3,n_SO4
     *     ,n_N_AKK_1
#ifdef TRACERS_NITRATE
...
#if (defined TRACERS_AMP)
     *     ,diag_reff_3d
     *     ,ijlt_3Dreff,ijlt_3Dreff_allspec,ijts_tauint
...
#if defined DEBUG_RADIATION || defined DEBUG_RADIATION_KEEP
      use radpar, only: l_dbrad_print, dbrad_aerosol
#endif
...
! The name will be any combination of {,TRNAME}{as,cs}{,a}aod
     ...
     &     sddarr4d
      ...
      character(len=10), dimension(3) :: sfrc=(/'swftoa',
     &                                          'swfsfc',
     &                                          'lwf   '/)
...
#ifdef GISS_RAD_OFF
!@var frac_cld_water,frac_cld_ice water and ice cloud area fractions (diagnostic)
      REAL*8 :: frac_cld_water, frac_cld_ice
#endif

!@var wcmmr,icmmr liquid,ice water mass mixing ratio (conv+strat)
!@var wcsiz,icsiz liquid,ice particle effective radius weighted by wcmmr,icmmr (conv+strat)
!@var reff_ice effective radius for ice particles (um)
      real*8 wcmmr, icmmr, wcsiz, icsiz, reff_ice
...
#ifdef USE_PLANET_RAD
      solar_irrad = solar_constant/RSDIST
#endif
#ifndef GISS_RAD_OFF
...
#if defined(DEBUG_RADIATION) || defined(DEBUG_RADIATION_KEEP)
C     Turn off all clouds in the GISS radiation scheme
      TAUSS = 0.0D+0
      TAUMC = 0.0D+0
#endif
...
#ifdef HEALY_LM_DIAGS
      FCO2=FULGAS(ip_co2)*CO2R
      FN2O=FULGAS(ip_n2o)*N2OR
      FCH4=FULGAS(ip_ch4)*CH4R
c
...
#ifdef PCLD_INDICES_LOCAL
c**** set local indices used to diagnose low,medium,high cloud
      llow = count(pmid(1:lm,i,j)>=cld_plow)
      lmid = count(pmid(1:lm,i,j)>=cld_pmid)
      lhi  = lm
#endif
...
#ifdef USE_PLANET_RAD
        CALL stop_model('RADIA: kradia > 0 not supported when using ' //
     &      'planet radiation', 255)
#endif
        do l=1,lm
...
C**** save 3D cloud fraction as seen by radiation
     ...
     &                          /cldmc(l,i,j)
#ifdef MORE_CLOUD_DIAGS_3D
              aijl(i,j,l,ijl_wcf)=aijl(i,j,l,ijl_wcf)+1.
              aijl(i,j,l,ijl_wcmmr)=aijl(i,j,l,ijl_wcmmr)
     &                             +QLmc(l,i,j)/cldmc(l,i,j)
              aijl(i,j,l,ijl_wcsiz)=aijl(i,j,l,ijl_wcsiz)
     &                             +QLmc(l,i,j)/cldmc(l,i,j)*sizewc(l)
#else
     ...
     &                          /cldmc(l,i,j)
#ifdef MORE_CLOUD_DIAGS_3D
              aijl(i,j,l,ijl_icf)=aijl(i,j,l,ijl_icf)+1.
              aijl(i,j,l,ijl_icmmr)=aijl(i,j,l,ijl_icmmr)
     &                             +QImc(l,i,j)/cldmc(l,i,j)
              aijl(i,j,l,ijl_icsiz)=aijl(i,j,l,ijl_icsiz)
     &                             +QImc(l,i,j)/cldmc(l,i,j)*sizeic(l)
#else
     ...
     &                          /cldss(l,i,j)
              ...
              aijl(i,j,l,ijl_wcf)=aijl(i,j,l,ijl_wcf)+1.
              aijl(i,j,l,ijl_wcmmr)=aijl(i,j,l,ijl_wcmmr)
     &                             +QLss(l,i,j)/cldss(l,i,j)
              aijl(i,j,l,ijl_wcsiz)=aijl(i,j,l,ijl_wcsiz)
     &                             +QLss(l,i,j)/cldss(l,i,j)*sizewc(l)
#else
              aijl(i,j,l,ijl_QLrad)=aijl(i,j,l,ijl_QLrad)
     &                             +QLss(l,i,j)/cldss(l,i,j)
#endif
     ...
     &                            /cldss(l,i,j)
                ...
                aijl(i,j,l,ijl_icf)=aijl(i,j,l,ijl_icf)+1.
                aijl(i,j,l,ijl_icmmr)=aijl(i,j,l,ijl_icmmr)
     &                               +QIss(l,i,j)/cldss(l,i,j)
                aijl(i,j,l,ijl_icsiz)=aijl(i,j,l,ijl_icsiz)
     &                               +QIss(l,i,j)/cldss(l,i,j)*sizeic(l)
#else
                aijl(i,j,l,ijl_QIrad)=aijl(i,j,l,ijl_QIrad)
     &                               +QIss(l,i,j)/cldss(l,i,j)
#endif
...
C****          1->(NRAD-1)*DTsrc (ADIURN) or skip them (HDIURN)
           ...
           END IF
         END DO
#endif /* GISS_RAD_OFF or not */

#ifdef USE_PLANET_RAD
C       Set cloud properties for all radiatively active layers. Remember to
C       reverse layer ordering to be consistent with planetary radiation.

C       For the layers in radiative equilibrium (not dynamically active), we
C       do not include clouds
        w_cloud_l(1:lm_req)             = 0.0D+00
        frac_st_water_l(1:lm_req)       = 0.0D+00
        frac_st_ice_l(1:lm_req)         = 0.0D+00
        frac_cnv_water_l(1:lm_req)      = 0.0D+00
        frac_cnv_ice_l(1:lm_req)        = 0.0D+00
        mix_ratio_st_water_l(1:lm_req)  = 0.0D+00
        mix_ratio_st_ice_l(1:lm_req)    = 0.0D+00
        mix_ratio_cnv_water_l(1:lm_req) = 0.0D+00
        mix_ratio_cnv_ice_l(1:lm_req)   = 0.0D+00
        dim_char_st_water_l(1:lm_req)   = 0.0D+00
        dim_char_st_ice_l(1:lm_req)     = 0.0D+00
        dim_char_cnv_water_l(1:lm_req)  = 0.0D+00
        dim_char_cnv_ice_l(1:lm_req)    = 0.0D+00
        frac_area_st_l(1:lm_req)        = 0.0D+00
        frac_area_cnv_l(1:lm_req)       = 0.0D+00

C       For dynamically active layers copy cloud properties from the 3D arrays
        w_cloud_l(lm_req+1:lx) =
     &      w_cloud(lm:1:-1,i,j)
        frac_st_water_l(lm_req+1:lx) =
     &      frac_st_water(lm:1:-1,i,j)
        frac_st_ice_l(lm_req+1:lx) =
     &      frac_st_ice(lm:1:-1,i,j)
        frac_cnv_water_l(lm_req+1:lx) =
     &      frac_cnv_water(lm:1:-1,i,j)
        frac_cnv_ice_l(lm_req+1:lx) =
     &      frac_cnv_ice(lm:1:-1,i,j)
        mix_ratio_st_water_l(lm_req+1:lx) =
     &      mix_ratio_st_water(lm:1:-1,i,j)
        mix_ratio_st_ice_l(lm_req+1:lx) =
     &      mix_ratio_st_ice(lm:1:-1,i,j)
        mix_ratio_cnv_water_l(lm_req+1:lx) =
     &      mix_ratio_cnv_water(lm:1:-1,i,j)
        mix_ratio_cnv_ice_l(lm_req+1:lx) =
     &      mix_ratio_cnv_ice(lm:1:-1,i,j)
        dim_char_st_water_l(lm_req+1:lx) =
     &      dim_char_st_water(lm:1:-1,i,j)
        dim_char_st_ice_l(lm_req+1:lx) =
     &      dim_char_st_ice(lm:1:-1,i,j)
        dim_char_cnv_water_l(lm_req+1:lx) =
     &      dim_char_cnv_water(lm:1:-1,i,j)
        dim_char_cnv_ice_l(lm_req+1:lx) =
     &      dim_char_cnv_ice(lm:1:-1,i,j)
        frac_area_st_l(lm_req+1:lx) =
     &      frac_area_st(lm:1:-1,i,j)
        frac_area_cnv_l(lm_req+1:lx) =
     &      frac_area_cnv(lm:1:-1,i,j)

C       Scaling of cloud mass
        cld_scaling = cldx
#endif
...
C**** EVEN PRESSURES
      ...
      aesqsc_dry(:,:,:)=0.0
      aesqcb_dry(:,:,:)=0.0
      frac_so4_has_nh4(:)=1.d0

#ifdef TRACERS_ON
      if (tracers_amp .or. use_offline_aerosols .or. oma_tramprad)
...
#if (defined TRACERS_AMP) || (defined USE_OFFLINE_AEROSOLS) || (defined OMA_TRAMPRAD)
...
#ifdef USE_PLANET_RAD
C     Set pressure on radiation levels. Remember to invert indexing and
C     convert to SI.
      p_level = PLB(LM+LM_REQ+1:1:-1)*1.0D+02

C     Set temperatures in radiation layers. Remember to invert indexing.
      t_layer = TLM(LM+LM_REQ:1:-1)

C     Set zenith angle
      cos_zenith = COSZA(I,J)
#endif
...
C**** CO2
C Update GCCco2_IN with trm to pass on CO2
C information to RADIATION
#ifdef GCC_COUPLE_RAD
      do L=1,LM
        GCCco2_IN(L)=trm(i,j,L,n_CO2n)
     &   * avog/(tr_mm(n_CO2n)*loschmidt_constant*10.d0)*CO2X
      enddo
...
#ifndef GISS_RAD_OFF /* Clear-sky fluxes calculated in main call */
...
c Including turn off of aerosols and Ox during crf calc.+++++++++++++++++++
          ...
          FTTOPX(:) = 0
          CALL RCOMPX           ! cloud_aer_o3_rad_forc=0 : clr sky
...
#ifndef GISS_RAD_OFF /* extremely expensive and not related to cloud_rad_forc */
        if (cloud_rad_forc.gt.0) then
...
#ifdef USE_PLANET_RAD
C     Set calculation of clear-sky fluxes in planet radiation
      l_clear_lw = cloud_rad_forc > 0
      l_clear_sw = cloud_rad_forc > 0
#endif
...
#if defined(USE_PLANET_RAD) && defined(GISS_RAD_OFF)
C     Get clear-sky diagnostic fluxes if requested
      IF (cloud_rad_forc > 0) THEN
C       Net flux at TOA
        TNFSCRF(I,J) = TRNFLB_CLEAR(LM+LM_REQ+1)
        LWDNCS(I,J)=STBO*(
     &      POCEAN*atmocn%GTEMPR(I,J)**4 +
     &      POICE*atmice%GTEMPR(I,J)**4 +
     &      PLICE*atmgla%GTEMPR(I,J)**4 +
     ...
     &      TRNFLB_CLEAR(1)
C       BEGIN AMIP
        AIJ(I,J,IJ_LWDCLS) = AIJ(I,J,IJ_LWDCLS) +
     &      TRNFLB_CLEAR(1)
        AIJ(I,J,IJ_LWNCLT) = AIJ(I,J,IJ_LWNCLT) +
     &      TRNFLB_CLEAR(LM+LM_REQ+1)
C       END AMIP

        SNFSCRF(I,J) = SRNFLB_CLEAR(LM+LM_REQ+1)
C       BEGIN AMIP
        AIJ(I,J,IJ_SWDCLS) = AIJ(I,J,IJ_SWDCLS) +
     &      SRDFLB_CLEAR(1)*COSZ2(I,J)
        AIJ(I,J,IJ_SWNCLS) = AIJ(I,J,IJ_SWNCLS) +
     &      SRNFLB_CLEAR(1)*COSZ2(I,J)
        AIJ(I,J,IJ_SWNCLT) = AIJ(I,J,IJ_SWNCLT) +
     &      SRNFLB_CLEAR(LM+LM_REQ+1)*COSZ2(I,J)
C       END AMIP
      END IF

C     Set available cloud diagnostics
      DO L=1,LM
        IF (w_cloud(l,i,j) > 0.0D+00) THEN
          rhodz=pdsig(l,i,j)*100/grav

C         Total cloud cover
          CALL inc_ajl(i,j,l,jl_totcld,w_cloud(l,i,j))
c         why is ijl_cf only diagnostic here dependent on cld_scaling?
          IF (cld_scaling > 0.0D+0)
     &        AIJL(I,J,L,IJL_CF) = AIJL(I,J,L,IJL_CF) + w_cloud(l,i,j)

C         Stratiform and convective cloud cover
          CALL inc_ajl(i,j,l,jl_sscld,w_cloud(l,i,j)*
     &        (frac_st_water(l,i,j) + frac_st_ice(l,i,j)))
          CALL inc_ajl(i,j,l,jl_mccld,w_cloud(l,i,j)*
     &        (frac_cnv_water(l,i,j) + frac_cnv_ice(l,i,j)))

C         Water and ice cloud cover (assume random overlap as these are
C         potentially a combination of stratiform and convective clouds)
          frac_cld_water = w_cloud(l,i,j)*
     &        (frac_st_water(l,i,j) + frac_cnv_water(l,i,j) -
     &        frac_st_water(l,i,j)*frac_cnv_water(l,i,j))
          frac_cld_ice = w_cloud(l,i,j)*
     &        (frac_st_ice(l,i,j) + frac_cnv_ice(l,i,j) -
     &        frac_st_ice(l,i,j)*frac_cnv_ice(l,i,j))
          CALL inc_ajl(i,j,l,jl_wcld,frac_cld_water)
          CALL inc_ajl(i,j,l,jl_icld,frac_cld_ice)
          CALL inc_ajl(i,j,l,jl_wcldwt,frac_cld_water*pdsig(l,i,j))
          CALL inc_ajl(i,j,l,jl_icldwt,frac_cld_ice*pdsig(l,i,j))

#ifdef MORE_CLOUD_DIAGS_3D

c         3D water and ice cloud fractions, mass mixing ratios, and mass-weighted effective radii
          aijl(i,j,l,ijl_wcf) = aijl(i,j,l,ijl_wcf) + frac_cld_water
          aijl(i,j,l,ijl_icf) = aijl(i,j,l,ijl_icf) + frac_cld_ice

          if( frac_cld_water > 0. )then
            wcmmr =
     &           (frac_area_st(l,i,j)*mix_ratio_st_water(l,i,j) +
     &            frac_area_cnv(l,i,j)*mix_ratio_cnv_water(l,i,j))
     &           /frac_cld_water
            wcsiz = 1e6*
     &           (frac_area_st(l,i,j)*mix_ratio_st_water(l,i,j)*
     &            dim_char_st_water(l,i,j) +
     &            frac_area_cnv(l,i,j)*mix_ratio_cnv_water(l,i,j)*
     &            dim_char_cnv_water(l,i,j))
     &           /frac_cld_water
            aijl(i,j,l,ijl_wcmmr) = aijl(i,j,l,ijl_wcmmr) + wcmmr
            aijl(i,j,l,ijl_wcsiz) = aijl(i,j,l,ijl_wcsiz) + wcsiz
          else
            ! zero scalar to be accumulated for water path whether cloudy or not
            wcmmr = 0.d0
          endif

          if( frac_cld_ice > 0. )then
            ! possibly replace GISS ice particle size with the parameterization
            ! used by SOCRATES, per eqns 9 and 10 of Edwards et al., Atmos. Res., 2007
            if( i_cloud_ice_size_scheme /= 1 )then
              if( tlm(l) <= 216.21 )then
                reff_ice = 0.5*(975.51*exp(0.05*(tlm(l)-279.5))+0.66)
              else
                reff_ice = 0.5*(175.44*exp(0.05*(tlm(l)-279.5))+34.45)
              endif
              dim_char_st_ice(l,i,j)  = 1e-6*reff_ice
              dim_char_cnv_ice(l,i,j) = 1e-6*reff_ice
            endif
            icmmr =
     &           (frac_area_st(l,i,j)*mix_ratio_st_ice(l,i,j) +
     &            frac_area_cnv(l,i,j)*mix_ratio_cnv_ice(l,i,j))
     &           /frac_cld_ice
            icsiz = 1e6*
     &           (frac_area_st(l,i,j)*mix_ratio_st_ice(l,i,j)*
     &            dim_char_st_ice(l,i,j) +
     &            frac_area_cnv(l,i,j)*mix_ratio_cnv_ice(l,i,j)*
     &            dim_char_cnv_ice(l,i,j))
     &           /frac_cld_ice
            aijl(i,j,l,ijl_icmmr) = aijl(i,j,l,ijl_icmmr) + icmmr
            aijl(i,j,l,ijl_icsiz) = aijl(i,j,l,ijl_icsiz) + icsiz
          else
            ! zero scalar to be accumulated for water path whether cloudy or not
            icmmr = 0.d0
          endif

#else /* not MORE_CLOUD_DIAGS_3D */

c         Mass mixing ratios and columns for water and ice clouds
          if( frac_cld_water > 0. )then
            wcmmr = (frac_area_st(l,i,j)*mix_ratio_st_water(l,i,j) +
     &         frac_area_cnv(l,i,j)*mix_ratio_cnv_water(l,i,j))
     &         /frac_cld_water
            aijl(i,j,l,ijl_qlrad) = aijl(i,j,l,ijl_qlrad) + wcmmr
          else
            ! zero scalar to be accumulated for water path whether cloudy or not
            wcmmr = 0.d0
          endif
          if( frac_cld_ice > 0. )then
            icmmr = (frac_area_st(l,i,j)*mix_ratio_st_ice(l,i,j) +
     &         frac_area_cnv(l,i,j)*mix_ratio_cnv_ice(l,i,j))
     &         /frac_cld_ice
            aijl(i,j,l,ijl_qirad) = aijl(i,j,l,ijl_qirad) + icmmr
          else
            ! zero scalar to be accumulated for water path whether cloudy or not
            icmmr = 0.d0
          endif

#endif /* MORE_CLOUD_DIAGS_3D or not */

c         Water paths for liquid and ice clouds
          aij(i,j,ij_lwprad) = aij(i,j,ij_lwprad) + wcmmr*rhodz
          aij(i,j,ij_iwprad) = aij(i,j,ij_iwprad) + icmmr*rhodz

        END IF
      END DO

C     Get total cloud cover from planet radiation scheme
      CLDCV = tot_cloud_cover
      CFRAC(I,J) = CLDCV
...
C     For now, calculate cloud fraction diagnostics not available
C     directly from planet radiation scheme as done by the GISS
C     scheme instead of setting them to zero.
      IF (cc_cdncx /= 0.0D+0 .OR. od_cdncx /= 0.0D+0) THEN
        CALL dCDNC_EST(i,j,pland, dCDNC)
      ELSE
        dCDNC = 0.0D+0
      END IF
...
C     Adjust RDSS for semi-random overlap
      ...
      TOTCLD=0.0D+0
      DO L=1,LM
C       Determine stratiform and convective cloud cover
        IF (CLDSS(L,I,J)*(1.+dcc_cdncl(l)) > RDSS(L,I,J)) THEN
          TOTCLD(L)=1.0D+0
          CSS=1.0D+0
!!!!      CALL inc_ajl(i,j,l,jl_sscld,CSS) ! available
        END IF
        IF (CLDMC(L,I,J) > RDMC(I,J)) THEN
          TOTCLD(L)=1.0D+0
          CMC=1.0d+0
!!!!      CALL inc_ajl(i,j,l,jl_mccld,CMC) ! available
          DEPTH=DEPTH+PDSIG(L,I,J)
        END IF
      END DO
...
C     Effective cloud cover diagnostics
      ...
      AIJ(I,J,IJ_PMCCLD)=AIJ(I,J,IJ_PMCCLD)+CMC
      IF (ANY(TOTCLD(1:LLOW) == 1.0D+0))
     &    AIJ(I,J,IJ_PCLDL)=AIJ(I,J,IJ_PCLDL)+1.0D+0
      IF (ANY(TOTCLD(LLOW+1:LMID) == 1.0D+0))
     &    AIJ(I,J,IJ_PCLDM)=AIJ(I,J,IJ_PCLDM)+1.0D+0
      IF (ANY(TOTCLD(LMID+1:LHI) == 1.0D+0))
     &    AIJ(I,J,IJ_PCLDH)=AIJ(I,J,IJ_PCLDH)+1.0D+0

C     Set layer index of cloud top
      LTOPCL=0
      IF (CLDCV > 0.0D+0) THEN
        DO L=LM,1,-1
          IF (w_cloud(l,i,j) > 0.0D+0) THEN
            LTOPCL = L
            EXIT
          END IF
        END DO
...
C     Band-by-band outgoing fluxes at TOA
      AIJ(I,J,IJ_TRUPTOA_BND1:IJ_TRUPTOA_BND1+n_band_lw-1) =
     &    AIJ(I,J,IJ_TRUPTOA_BND1:IJ_TRUPTOA_BND1+n_band_lw-1) +
     &    TRUPTOA
      AIJ(I,J,IJ_SRUPTOA_BND1:IJ_SRUPTOA_BND1+n_band_sw-1) =
     &    AIJ(I,J,IJ_SRUPTOA_BND1:IJ_SRUPTOA_BND1+n_band_sw-1) +
     &    SRUPTOA*COSZ2(I,J)
      AIJ(I,J,IJ_SRDNTOA_BND1:IJ_SRDNTOA_BND1+n_band_sw-1) =
     &    AIJ(I,J,IJ_SRDNTOA_BND1:IJ_SRDNTOA_BND1+n_band_sw-1) +
     &    SRDNTOA*COSZ2(I,J)
#endif /* USE_PLANET_RAD and GISS_RAD_OFF */
...
#if defined DEBUG_RADIATION || defined DEBUG_RADIATION_KEEP
        if ( l_dbrad_print .and. dbrad_aerosol == 1 ) write( 0, * )
     &       'In RAD_DRV.f in subroutine RADIA: nraero_aod', nraero_aod,
     &       '; ibd: ', ibd, '; opnsky: ', opnsky
#endif

      nsub_ntrix = 0
      do n=1,nraero_aod
#if defined DEBUG_RADIATION || defined DEBUG_RADIATION_KEEP
        if ( l_dbrad_print .and. dbrad_aerosol == 1 ) then
          write( 0, * ) 'n: ', n
          write( 0, * ) 'l  aesqex(l, ibd, n)'
          do l = 1,lm+lm_req
            write( 0, * ) l, aesqex( l, ibd, n )
          end do
        end if
...
! 3d aod
     ...
     *           =taijls(i,j,1:lm,ijlt_3Daaod(n))+
     *            (aesqex(1:lm,ibd,n)-aesqsc(1:lm,ibd,n))
            if (ijlt_3DaaodCS(n).gt.0)
     *           taijls(i,j,1:lm,ijlt_3DaaodCS(n))
     *           =taijls(i,j,1:lm,ijlt_3DaaodCS(n))+
     *            (aesqex(1:lm,ibd,n)-aesqsc(1:lm,ibd,n))*OPNSKY
     ...
     *         =taijls(i,j,1:lm,ijlt_3Dtau(n))+aesqex(1:lm,ibd,n)
            if (ijlt_3DtauCS(n).gt.0)
     *           taijls(i,j,1:lm,ijlt_3DtauCS(n))
     *         =taijls(i,j,1:lm,ijlt_3DtauCS(n))
     *         +aesqex(1:lm,ibd,n)*OPNSKY
     ...
     *         =taijls(i,j,1:lm,ijlt_3Dtau(1))+aesqex(1:lm,ibd,n)
            if (ijlt_3DtauCS(1).gt.0)
     *           taijls(i,j,1:lm,ijlt_3DtauCS(1))
     *       =taijls(i,j,1:lm,ijlt_3DtauCS(1))+aesqex(1:lm,ibd,n)*OPNSKY
...
! 2d aod, per band or just band ibd, depending on diag_rad
          IF (diag_rad /= 1) THEN
...
! 3d aod
     ...
     *            (aesqex(1:lm,ibd,n)-aesqsc(1:lm,ibd,n))*OPNSKY
     ...
     &           taijls(i,j,1:lm,ijlt_3DaaodDRY(n))
     &           =taijls(i,j,1:lm,ijlt_3DaaodDRY(n))+
     *            (aesqex_dry(1:lm,6,n)-aesqsc_dry(1:lm,ibd,n))
            if (ijlt_3Dtau(n).gt.0)
     &           taijls(i,j,1:lm,ijlt_3Dtau(n))
     &         =taijls(i,j,1:lm,ijlt_3Dtau(n))+aesqex(1:lm,ibd,n)
            if (ijlt_3DtauCS(n).gt.0)
...
#if (defined TRACERS_AMP)
            ! 3d reff, weighted by band6 aod
            IF (ijlt_3Dtau(n).gt.0 .and. diag_reff_3d > 0) THEN
              if (ijlt_3Dreff(n).gt.0)
     &             taijls(i,j,1:lm,ijlt_3Dreff(n))
     &           =taijls(i,j,1:lm,ijlt_3Dreff(n))+
     *            Reff_LEV(1:lm,n)*aesqex(1:lm,6,n)
     ...
     *            (aesqex(1:lm,ibd,n)-aesqsc(1:lm,ibd,n))*OPNSKY
     ...
     &           taijls(i,j,1:lm,ijlt_3DaaodDRY(1))
     &           =taijls(i,j,1:lm,ijlt_3DaaodDRY(1))+
     *            (aesqex_dry(1:lm,ibd,n)-aesqsc_dry(1:lm,ibd,n))
            if (ijlt_3Dtau(1).gt.0)
     &           taijls(i,j,1:lm,ijlt_3Dtau(1))
     &         =taijls(i,j,1:lm,ijlt_3Dtau(1))+aesqex(1:lm,ibd,n)
            if (ijlt_3DtauCS(1).gt.0)
     ...
     &       =taijls(i,j,1:lm,ijlt_3DtauCS(1))+aesqex(1:lm,ibd,n)*OPNSKY
            if (ijlt_3DtauDRY(1).gt.0)
     &           taijls(i,j,1:lm,ijlt_3DtauDRY(1))
     &         =taijls(i,j,1:lm,ijlt_3DtauDRY(1))+aesqex_dry(1:lm,ibd,n)
#if (defined TRACERS_AMP)
            ! 3d reff, weighted by band6 aod (combined all species)
            IF (ijlt_3Dtau(1).gt.0 .and. diag_reff_3d > 0) THEN
              if (ijlt_3Dreff_allspec.gt.0)
     &             taijls(i,j,1:lm,ijlt_3Dreff_allspec)
     &           =taijls(i,j,1:lm,ijlt_3Dreff_allspec)+
     *            Reff_LEV(1:lm,n)*aesqex(1:lm,ibd,n)
            endif !diag_reff_3d>0
#endif
          endif ! 0<diag_aod_3d<5 or 0>diag_aod_3d>-5
...
#if defined DEBUG_RADIATION || defined DEBUG_RADIATION_KEEP
      if ( l_dbrad_print ) call stop_model
     &     ("Radiation debugging completed.", 255)
#endif
      ...
      do n=1,nraero_aod
        IF (ijts_tauint.gt.0)
     &    taijs(i,j,ijts_tauint)
     &    = taijs(i,j,ijts_tauint)+
     &    SUM(aesqex(1:Lm,6,n))

        IF (ijts_taustratint.gt.0)
     &    taijs(i,j,ijts_taustratint)
     &    = taijs(i,j,ijts_taustratint)+
     &    SUM(aesqex(ltropo(i,j)+1:Lm,6,n))

        IF (ijts_reffstratint.gt.0)
     &    taijs(i,j,ijts_reffstratint)
     &    = taijs(i,j,ijts_reffstratint)+
     &    SUM(Reff_LEV(ltropo(i,j)+1:Lm,n)*aesqex(ltropo(i,j)+1:Lm,6,n))

        IF (ijts_2Dreff(ntrix_aod(n)).gt.0)
     &    taijs(i,j,ijts_2Dreff(ntrix_aod(n)))
     &    = taijs(i,j,ijts_2Dreff(ntrix_aod(n)))+
     &    SUM(Reff_LEV(1:Lm,n)*aesqex(1:Lm,6,n))
      end do ! nraero_aod
#endif 

#if (defined TRACERS_AEROSOLS_Koch && defined TRACERS_NITRATE) || \
    (defined TRACERS_AMP)
        ! 3d fraction of sulfate that contains NH4 (for separating out H2SO4)
        if (ijlt_fracso4hasnh4.gt.0)
     *       taijls(i,j,1:lm,ijlt_fracso4hasnh4)
     *     =taijls(i,j,1:lm,ijlt_fracso4hasnh4)
     *      +frac_so4_has_nh4(1:lm)
#endif /* (Koch && NITRATE)||AMP */
...
#ifdef TRACERS_ON
      if (nraero_aod>0) then
        tau_as(i,j,1:LM,1:nraero_aod)=aesqex(1:LM,ibd,1:nraero_aod)
        tau_cs(i,j,1:LM,1:nraero_aod)=aesqex(1:LM,ibd,1:nraero_aod)*
     &    OPNSKY
        scatau_as(i,j,1:LM,1:nraero_aod)=aesqsc(1:LM,ibd,1:nraero_aod)
        g_as(i,j,1:LM,1:nraero_aod)=aesasy(1:LM,ibd,1:nraero_aod)
        if (save_dry_aod>0) then
          tau_dry(i,j,1:LM,1:nraero_aod)=aesqex_dry(1:LM,6,1:nraero_aod)
        endif
#ifdef CACHED_SUBDD
        abstau_as(i,j,1:LM,1:nraero_aod)=
     &    (aesqex(1:LM,ibd,1:nraero_aod)-aesqsc(1:LM,ibd,1:nraero_aod))
...
#ifndef GISS_RAD_OFF /* skip for SOCRATES - too expensive */
         if ( cloud_rad_forc > 0 ) then
...
c longwave GHG forcing at TOA
    ...
    (defined TRACERS_AEROSOLS_SEASALT)
      if (nraero_rf>0) then
        swftoa(i,j,1:nraero_rf)=
     &    rsign_aer*(SNFST(2,1:nraero_rf,I,J)-SNFS(LFRC,I,J))*CSZ2
        swfsfc(i,j,1:nraero_rf)=
     &    rsign_aer*(SNFST(1,1:nraero_rf,I,J)-SNFS(1,I,J))*CSZ2
...
! Note that INPUTZ contains fewer variables in this version.
      ...
      USE MODEL_COM, only : HOURI,DATEI,MONTHI,YEARI
...
C****
      ...
      READ(iu_IFILE,'(A)') XLABEL ; L1=len_trim(xlabel)
      READ(iu_IFILE,'(A)') LABEL2 ; L2=len_trim(label2)
!                          combine the 2 lines if possible
      if(L2>0.and.L1<131) xlabel=trim(xlabel)//' '//label2(1:131-L1)
      !if (AM_I_ROOT())
...
!@var NDAY=(1 day)/DTsrc : even integer; adjust DTsrc to be commensurate
      ...
      dtSrcUsed = newTimeInterval(secsPerDay / NDAY)
