...
!@auth M. Kelley (cubed sphere)
     ...
     *     ,usi,vsi,iFOCEAN=>FOCEAN
      USE ICEDYN_COM, only : icij,ij_rsiu,ij_rsiv,ij_usi
...
C****
     ...
     &     UOSURF,VOSURF,DMUI,DMVI
      ...
      REAL*8 :: hemi
      INTEGER :: I,J,ip1,im1
      REAL*8 :: DMUINP,duA,dvA,rsib,rsiu,rsiv
      INTEGER :: aIM, aJM
      INTEGER :: iJ_1   , iJ_0
      INTEGER :: iJ_1S  , iJ_0S
      INTEGER :: iJ_1H  , iJ_0H
...
C**** Update halos for UICE and DMU
      ...
      CALL ICE_HALO(grid_ICDYN,  VICE, from=SOUTH     )
...
c*** diagnostics
     ...
     *         iRSI(I,J)+iRSI(IP1,J).gt.1d-4) THEN
            rsiu = .5d0*(iRSI(I,J)+iRSI(IP1,J))
            ICIJ(I,J,IJ_RSIU) =ICIJ(I,J,IJ_RSIU) + rsiu
            ICIJ(I,J,IJ_USI) =ICIJ(I,J,IJ_USI) + rsiu*
     *           0.5*(uice(i+1,j-1,1)+uice(i+1,j,1))
            ICIJ(I,J,IJ_DMUI)=ICIJ(I,J,IJ_DMUI)+DMUI(i,j)
          END IF
          IF (iFOCEAN(I,J+1).gt.0 .and. iFOCEAN(I,J).gt.0. .and.
     *         iRSI(I,J)+iRSI(I,J+1).gt.1d-4) THEN
            rsiv = .5d0*(iRSI(I,J)+iRSI(I,J+1))
            ICIJ(I,J,IJ_RSIV) =ICIJ(I,J,IJ_RSIV) + rsiv
            ICIJ(I,J,IJ_VSI) =ICIJ(I,J,IJ_VSI) + rsiv*
     *           0.5*(vice(i,j,1)+vice(i+1,j,1))
            ICIJ(I,J,IJ_DMVI)=ICIJ(I,J,IJ_DMVI)+DMVI(i,j)
          END IF
          if(iRSI(I,J).gt.0.) then
C**** SISPEED: Speed of ice (i.e. mean absolute velocity)
C**** to account for back-and-forth movement of the ice
            ICIJ(I,J,IJ_SISPEED)=ICIJ(I,J,IJ_SISPEED) + iRSI(i,j)*
     &         .25d0*sum(
     &         sqrt(uice(i:i+1,j-1:j,1)**2 + vice(i:i+1,j-1:j,1)**2))
...
!@auth Gavin Schmidt
     ...
     &     ,ij_dmvi,ij_rsi,ij_pice,ij_sispeed,ij_rsiu,ij_rsiv
     &     ,cdl_icij
...
C**** set properties for ICIJ diagnostics
      ...
      k=k+1
      IJ_RSIU=k
      lname_icij(k)="Ocean ice fraction at u-points"
      sname_icij(k)="icij_rsiu"
      units_icij(k)="%"
      scale_icij(k)=100.

      k=k+1
      IJ_RSIV=k
      lname_icij(k)="Ocean ice fraction at v-points"
      sname_icij(k)="icij_rsiv"
      units_icij(k)="%"
      scale_icij(k)=100.

      k=k+1
      IJ_USI=k
      denom_icij(k)=IJ_RSIU
      lname_icij(k)="Sea ice EW velocity"
      ...
      k=k+1
      IJ_VSI=k
      denom_icij(k)=IJ_RSIV
      lname_icij(k)="Sea ice NS velocity"
