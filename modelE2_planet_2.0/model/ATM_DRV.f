#include "rundeck_opts.h"
     ...
     &     ,MODD5S,NDAa, NDA5d,NDA5s
      ...
      use RAD_COM, only : nrad,modrd, cosz1
...
#endif
#if (defined USE_OFFLINE_AEROSOLS) || (defined OMA_TRAMPRAD)
      call SETUP_RAD
#endif
C****
      CALL RINIT (IRAND)
...
!      read SCM data and initialize model
     ...
     &   apply_offline_dQ_to_NINT <= 3) then
      ...
      if(istart<=2) call read_rad_ic

      call atm_phase1_exports

C**** Initialize lake variables (including river directions)
      if(istart<=2) call read_agrice_ic
...
C****
      if(istart<=2) call read_landsurf_ic(istart)
...
C**** Initialize land ice (must come after oceans)
      if(istart<=2) call read_landice_ic
...
c set-up for MPI implementation
      ...
      use photolysis, only: alloc_fastj2
...
#endif
#endif  /* TRACERS_ON or TRACERS_OCEAN or TRACERS_WATER */
#ifndef SCM
