...
!      ,aq,dpdt,th,ql,wmx,ttoldl,rh,taussl,cldssl,cldsavl,rh1,roice &
       ...
       ,QLss,QIss,QLmc,QImc &
       ,use_vmp,wmpr,tausslip,csizelip,wmclwp
#if (defined CLD_AER_CDNC) || (defined AIE_DIAG_FIX_MET)
  ...
  USE CLOUDS_COM, ONLY: w_cloud, frac_st_water, frac_st_ice, frac_cnv_water, &
                        frac_cnv_ice, mix_ratio_st_water, mix_ratio_st_ice, &
                        mix_ratio_cnv_water, mix_ratio_cnv_ice, &
                        dim_char_st_water, dim_char_st_ice, &
                        dim_char_cnv_water, dim_char_cnv_ice, &
                        frac_area_st, frac_area_cnv
  USE CLOUDS, ONLY: CNVMMRL, CLDSAL
#endif
...
#ifdef CACHED_SUBDD
            IWPA(I,J)=IWPA(I,J)+QCIX(L)*AIRM(L)*100.*BYGRAV
#endif
#ifdef mjo_subdd
...
#ifdef USE_PLANET_RAD
!       Use random overlap to calculate total cloud fraction
        w_cloud(:,i,j) = CLDMCL + CLDSSL - CLDMCL*CLDSSL

!       Calculate in-cloud fractions of the different types of cloud. We never
!       have both ice and water cloud in the same grid box. Loop through all
!       layers, check if it contains water or ice cloud (or no cloud) and
!       set quantities accordingly.
        DO l=1,lm

!         Stratiform cloud properties
          IF (CLDSSL(L) > 0.0D+00) THEN
!           Stratiform cloud is present

            IF (SVLHXL(L) == LHE) THEN
!             Cloud content is liquid

              frac_st_water(l,i,j) = &
                  CLDSSL(L)/(CLDMCL(L) + CLDSSL(L) + TINY(CLDSSL(L)))
              frac_st_ice(l,i,j) = 0.0D+00
              mix_ratio_st_water(l,i,j) = QCLX(L)
              mix_ratio_st_ice(l,i,j) = 0.0D+00
              dim_char_st_water(l,i,j) = CSIZSS(L,I,J)*1.0D-06 ! Convert to m
              dim_char_st_ice(l,i,j) = 0.0D+00
              frac_area_st(l,i,j) = CLDSAL(L)

            ELSE IF (SVLHXL(L) == LHS) THEN
!             Cloud content is ice

              frac_st_water(l,i,j) = 0.0D+00
              frac_st_ice(l,i,j) = &
                  CLDSSL(L)/(CLDMCL(L) + CLDSSL(L) + TINY(CLDSSL(L)))
              mix_ratio_st_water(l,i,j) = 0.0D+00
              mix_ratio_st_ice(l,i,j) = QCIX(L)
              dim_char_st_water(l,i,j) = 0.0D+00
              dim_char_st_ice(l,i,j) = CSIZSS(L,I,J)*1.0D-06 ! Convert to m
              frac_area_st(l,i,j) = CLDSAL(L)

            ELSE
!             This should not happen, cloud is either water or ice

              CALL stop_model(255, 'CONDSE: Error setting stratiform ' // &
                  'cloud properties.')
...
!           No stratiform cloud
            ...
            dim_char_st_water(l,i,j) = 0.0D+00
            dim_char_st_ice(l,i,j) = 0.0D+00
            frac_area_st(l,i,j) = 0.0D+00

          END IF

!         Convective cloud properties
          IF (CLDMCL(L) > 0.0D+00) THEN
!           Convective cloud is present

            IF (SVLATL(L) == LHE) THEN
!             Cloud content is liquid

              frac_cnv_water(l,i,j) = &
                  CLDMCL(L)/(CLDMCL(L) + CLDSSL(L) + TINY(CLDMCL(L)))
              frac_cnv_ice(l,i,j) = 0.0D+00
              mix_ratio_cnv_water(l,i,j) = CNVMMRL(L)
              mix_ratio_cnv_ice(l,i,j) = 0.0D+00
              dim_char_cnv_water(l,i,j) = CSIZMC(L,I,J)*1.0D-06 ! Convert to m
              dim_char_cnv_ice(l,i,j) = 0.0D+00
              frac_area_cnv(l,i,j) = CLDMCL(L)

            ELSE IF (SVLATL(L) == LHS) THEN
!             Cloud content is ice

              frac_cnv_water(l,i,j) = 0.0D+00
              frac_cnv_ice(l,i,j) = &
                  CLDMCL(L)/(CLDMCL(L) + CLDSSL(L) + TINY(CLDMCL(L)))
              mix_ratio_cnv_water(l,i,j) = 0.0D+00
              mix_ratio_cnv_ice(l,i,j) = CNVMMRL(L)
              dim_char_cnv_water(l,i,j) = 0.0D+00
              dim_char_cnv_ice(l,i,j) = CSIZMC(L,I,J)*1.0D-06 ! Convert to m
              frac_area_cnv(l,i,j) = CLDMCL(L)
            
            ELSE
!             This should not happen, cloud is either water or ice

              CALL stop_model(255, 'CONDSE: Error setting convective ' // &
                  'cloud properties.')

            END IF

!           Check for the special case where convective cloud condensate mass
!           is zero, but the cloud fraction is non-zero due to precipitation.
!           We ignore the radiative effects of precipitation at the moment and
!           set the convective cloud properties to zero. Any remaining cloud in
!           the layer is stratiform.
            IF (CNVMMRL(L) == 0.0D+00) THEN
!             Convective cloud condensate mass is zero
              ...
              dim_char_cnv_water(l,i,j) = 0.0D+00
              dim_char_cnv_ice(l,i,j) = 0.0D+00
              frac_area_cnv(l,i,j) = 0.0D+00

              w_cloud(l,i,j) = CLDSSL(L)
              IF (CLDSSL(L) > 0.0D+00) THEN
                frac_st_water(l,i,j) = frac_st_water(l,i,j)/( &
                    frac_st_water(l,i,j) + frac_st_ice(l,i,j))
                frac_st_ice(l,i,j) = frac_st_ice(l,i,j)/( &
                    frac_st_water(l,i,j) + frac_st_ice(l,i,j))
              END IF
...
#if (defined TRACERS_AEROSOLS_Koch) || (defined TRACERS_AMP) ||\
    (defined TRACERS_TOMAS)
            if (ijts_aq(1,n).gt.0) then ! use ij mask for jl as well
              call inc_tajls2(i,j,l,jls_incloud(1,n),sum(dt_sulf_mc(n,l,:))*(1.-fssl(l)))
              call inc_tajls2(i,j,l,jls_incloud(2,n),sum(dt_sulf_ss(n,l,:)))
              taijs(i,j,ijts_aq(1,n))=taijs(i,j,ijts_aq(1,n))+ &
                   (sum(dt_sulf_mc(n,l,:))*(1.-fssl(l))+sum(dt_sulf_ss(n,l,:)))
              taijs(i,j,ijts_aq(2,n))=taijs(i,j,ijts_aq(2,n))+&
                                      dt_sulf_mc(n,l,1)*(1.-fssl(l))
              taijs(i,j,ijts_aq(3,n))=taijs(i,j,ijts_aq(3,n))+&
                                      dt_sulf_mc(n,l,2)*(1.-fssl(l))
              taijs(i,j,ijts_aq(4,n))=taijs(i,j,ijts_aq(4,n))+&
                                      dt_sulf_ss(n,l,1)
              taijs(i,j,ijts_aq(5,n))=taijs(i,j,ijts_aq(5,n))+&
                                      dt_sulf_ss(n,l,2)
            end if
            if((trname(n).eq."SO4".or.trname(n).eq."M_ACC_SU") &
                .and.ijlt_prodSO4aq.gt.0) &
           ...
           if (trname(n).eq."ASO4__01") then
              AQSO4oxid_mc(i,j,l) = sum(dt_sulf_mc(n,l,:))*(1.-fssl(l))
              AQSO4oxid_ls(i,j,l) = sum(dt_sulf_ss(n,l,:))
           endif
#endif
          end do
#ifdef TRACERS_WATER
...
!   call inc_subdd(subdd,k,cfmip_mc_lwp)
  ...
  case ('QLss')
    do j=j_0,j_1; do i=i_0,i_1; do l=1,lm
      sddarr3d(i,j,l) = qlss_3d(l,i,j)
    enddo;        enddo;        enddo
    call inc_subdd(subdd,k,sddarr3d)
  case ('QIss')
    do j=j_0,j_1; do i=i_0,i_1; do l=1,lm
      sddarr3d(i,j,l) = qiss_3d(l,i,j)
    enddo;        enddo;        enddo
    call inc_subdd(subdd,k,sddarr3d)
  case ('QLmc')
    do j=j_0,j_1; do i=i_0,i_1; do l=1,lm
      sddarr3d(i,j,l) = qlmc_3d(l,i,j)
    enddo;        enddo;        enddo
    call inc_subdd(subdd,k,sddarr3d)
  case ('QImc')
    do j=j_0,j_1; do i=i_0,i_1; do l=1,lm
      sddarr3d(i,j,l) = qimc_3d(l,i,j)
    enddo;        enddo;        enddo
    call inc_subdd(subdd,k,sddarr3d)
  case ('dq_mc')
...
!@auth M.S.Yao/A. Del Genio (modularisation by Gavin Schmidt)
  ...
  USE ATM_COM, only : t,q,pmid,pk ! for coldstart istart=1-2 case
  ...
  use CLOUDS, only : lmcm,bydtsrc,xmass,brcld,bybr,U00wtrX,U00ice &
       ...
       ,funio_denominator,autoconv_multiplier &
       ,radiusl_multiplier,radiusi_multiplier,wmu_multiplier &
  ...
  use CLOUDS_COM, only: cld_plow,cld_pmid
...
#if (defined TRACERS_AEROSOLS_Koch) || (defined TRACERS_AMP) ||\
    (defined TRACERS_TOMAS)
  ! for diagnostics; 1 for rain, 2 for cloud
    allocate(DT_SULF_MC(NTM,LM,2))
    allocate(DT_SULF_SS(NTM,LM,2))
#endif
#ifdef TRDIAG_WETDEPO
  ...
  if (is_set_param( "radius_multiplier")) then
    call sync_param( "radius_multiplier",radiusl_multiplier)
    radiusi_multiplier = 1.d0
  else
    call sync_param( "radiusl_multiplier",radiusl_multiplier)
    call sync_param( "radiusi_multiplier",radiusi_multiplier)
  end if
  call sync_param( "wmu_multiplier",wmu_multiplier) ! for liquid cloud
...
#endif  /* CALCULATE_LIGHTNING */
  call get_param( "cld_plow", cld_plow, default=680.d0*PSF/984.d0 )
  call get_param( "cld_pmid", cld_pmid, default=440.d0*PSF/984.d0 )
...
#ifndef PCLD_INDICES_LOCAL
  !**** CLOUD LAYER INDICES USED FOR DIAGNOSTICS (MATCHES ISCCP DEFNs)
  do L=1,LM
    LLOW=L
    if (.5*(PLbot(L+1)+PLbot(L+2)).lt.cld_plow) exit
  end do
  do L=LLOW+1,LM
    LMID=L
    if (.5*(PLbot(L+1)+PLbot(L+2)).lt.cld_pmid) exit
  end do
  LHI=LM
...
!@var pl pressure
 ...
 arr(next()) = info_type_( &
  ...
  arr(next()) = info_type_( &
    sname = 'QLss', &
    lname = 'stratiform liquid water mixing ratio available to radiation', &
    units = 'kg/kg' &
       )

  arr(next()) = info_type_( &
    sname = 'QIss', &
    lname = 'stratiform ice water mixing ratio available to radiation', &
    units = 'kg/kg' &
       )

  arr(next()) = info_type_( &
    sname = 'QLmc', &
    lname = 'convective liquid water mixing ratio available to radiation', &
    units = 'kg/kg' &
       )

  arr(next()) = info_type_( &
    sname = 'QImc', &
    lname = 'convective ice water mixing ratio available to radiation', &
    units = 'kg/kg' &
       )
