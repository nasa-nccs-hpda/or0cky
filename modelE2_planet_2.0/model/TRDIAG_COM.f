...
!@var IJT_XX names for water-based taijn diagnostics
     ...
     *     ,tij_rvro,tij_icb
      integer tij_drydep,tij_vd
#ifdef TRACERS_DRYDEP_DIAG
      integer tij_vd_for 
     *      ,tij_vd_cro,tij_vd_shr,tij_vd_gra,tij_vd_bar
     *      ,tij_egs_for,tij_egs_cro,tij_egs_shr,tij_egs_gra
     *      ,tij_egstom1,tij_egcut_dry1,tij_egcut_wet1
     *      ,tij_egcut_snow1,tij_eggro_dry1,tij_eggro_wet1
     *      ,tij_eggro_snow1
#endif
...
!@var ijs_XXX index for diags not specific to a certain tracer
     ...
     &ijs_NO2_1330,ijs_NO2_1330c,ijs_O3mass,
     &ijts_clrsky=0,ijts_pocean=0,ijts_sunlit_snow=0
      ! for Acetone tracer Air/sea flux diagnostics:
      integer :: ijs_AcetOtoA=0, ijs_AcetAtoO=0
...
!@param MaxBand Maximum number of diagnostic radiation bands
      INTEGER,PARAMETER :: MaxBand=6
...
!@dbparam diag_reff_3d outputs 3d effective radius (aod-weighted),
!@+       requires matching 3D aod-> will be speciated if diag_aod_3d=1 or 3
!@+                               all species together if diag_aod_3d=-1 or -3
!@+                               else 3D Reff will not appear
      INTEGER :: diag_reff_3d=0 ! 0=off (default), 1=on
...
!@var ijts_sqex index for TAIJS total extinction diagnostic radiation bands
      INTEGER, ALLOCATABLE ::ijts_sqex(:,:,:)
!@var ijts_sqexsub index for TAIJS total extinction for diagnostic radiation
!@+   bands for tracer sub classes
      INTEGER, ALLOCATABLE ::ijts_sqexsub(:,:,:,:)
!@var ijts_sqsc index for TAIJS scattering extinction for diagnostic radiation
!@+   bands
      INTEGER, ALLOCATABLE ::ijts_sqsc(:,:,:)
!@var ijts_sqscsub index for TAIJS scattering extinction for diagnostic
!@+   radiation bands for tracer sub classes
      INTEGER, ALLOCATABLE ::ijts_sqscsub(:,:,:,:)
!@var ijts_sqcb index for TAIJS sct asymmetry factor for diagnostic radiation
      INTEGER, ALLOCATABLE ::ijts_sqcb(:,:,:)
!@var ijts_sqcbsub index for TAIJS sct asymmetry factor for diagnostic
!@+   radiation for tracer sub classes
...
#ifdef TRACERS_AMP
!@var ijts_tauint 2D vertically summed total AOD (visible/band6)
!@var ijts_taustratint 2D total AOD (visible/band6)
!@var ijts_reffstratint 2D effective radius in stratosphere (AOD-weighted)
!@var ijts_2Dreff speciated 2D effective radius (AOD-weighted)
      INTEGER ::ijts_tauint
      INTEGER ::ijts_taustratint
      INTEGER ::ijts_reffstratint
      INTEGER, ALLOCATABLE ::ijts_2Dreff(:)
#endif
...
#ifdef TRACERS_AMP_M11
     &                            + 5 ! just a guess for now
#endif /* TRACERS_AMP_M11 */
...
!@var ijlt_XXX diag names associated with 3D tracer special diags
     ...
     & ijlt_NO3,ijlt_HO2,ijlt_COp,ijlt_COd,ijlt_NOxd,
     ...
     & ,ijlt_O3cmatm, ijlt_JacetA=0, ijlt_JacetB=0
     & ,ijlt_clrsky2d=0
     & ,ijlt_airmass=0
...
!@var ijlt_3Dmass 3D tracer independent array for layer MASS (or load)
    ...
    (defined TRACERS_AMP)
!@var ijlt_fracso4hasnh4 3D tracer independent array for sulfate identification
      INTEGER              :: ijlt_fracso4hasnh4
#endif /* (Koch && NITRATE)||AMP */
#if (defined TRACERS_AMP)
!@var ijlt_3Dreff 3D tracer independent array for all-sky hydrated Reff (weighted by AOD)
!@var ijlt_3Dreff_allspec as in ijlt_3Dreff weighting all species together by AOD
      integer, allocatable :: ijlt_3Dreff(:)
      integer :: ijlt_3Dreff_allspec
#endif /* AMP */
#ifdef TRACERS_TOMAS
...
!@param maxntm_extra maximum number of extra tracers in tconsrv
      integer, parameter :: maxntm_extra=0
#ifdef TRACERS_SPECIAL_Shindell
     &     +3  ! include some extra troposphere only ones
#endif
#ifdef TRACERS_OCEAN
      ! typical maximum number of ocean tracers.   use CPP to refine further.
     &     +25
#endif
#ifdef TRACERS_TOMAS
...
!@auth Gavin Schmidt
      ...
      USE TRDIAG_COM, only: ktcon,title_tcon,scale_tcon,nsum_tcon
     *     ,nofmt,ia_tcon,name_tconsrv,lname_tconsrv,units_tconsrv
     *     ,natmtrcons,nocntrcons,maxntm_extra
      IMPLICIT NONE
!@var NAME_CON name of conservation quantity
...
!@var CONPT0_sname like CONPT0 but without spaces
      ...
      if (itr0>maxntm_extra) call
     &     stop_model('trdiag_com: increase maxntm_extra', 255)
