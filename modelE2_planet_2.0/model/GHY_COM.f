...
!@var soil_surf_moist near surf soil moisture (kg/m^3) for subdd
      real*8, ALLOCATABLE, dimension(:,:) :: mCO2cond_ij
...
!@auth NCCS (Goddard) Development Team
      ...
      INTEGER :: IER , fid
!@dbparam geotherm_landX scaling factor for geothermal heat flux->land
      real*8 :: geotherm_landX = 0. ! =1. IF file GEOTHERM_LAND exists
...
C****
     ...
     *         STAT=IER)
      ...
      allocate(fgeotherm(I_0H:I_1H,J_0H:J_1H), STAT = IER)
      fgeotherm = 1d0 ! default - to be scaled by geotherm_landX
      ...
      call sync_param("geotherm_landX",geotherm_landX)
      fgeotherm = geotherm_landX * fgeotherm
...
C**** Initialize to zero
      ...
      ALLOCATE( mCO2cond_ij(I_0H:I_1H,J_0H:J_1H) )
      mCO2cond_ij(:,:) = 0.d0

      END SUBROUTINE ALLOC_GHY_COM

      subroutine read_landsurf_ic(istart)
!@sum   read_landsurf_ic read land surface initial conditions file.
      ...
      use pario, only : par_open,par_close,read_dist_data
      use filemanager, only : file_exists
      use Dictionary_mod, only : sync_param, is_set_param, query_param
      use ghy_com
      implicit none
      integer :: fid, istart, dim, ng
      integer :: ifrac
      real*8, dimension(:,:,:), allocatable :: temp,wetness
      real*8, dimension(:,:), allocatable :: snowdp
      real*8, allocatable :: Tg_init(:),Gwet_init(:)
      real*8 :: snowdp_init
      character*1 :: ptype
      integer :: i_0,i_1,j_0,j_1,i,j

      if(file_exists('SOILIC').or.istart==1) then
        ! set IC using intensive units.  Bare and vegetated
     ...
     &       earth_ice(0:ngm,ls_nfrac,i_0:i_1,j_0:j_1))
        ! Ground temperatures in degC    
        temp = temperature_istart1 - tf
        if (is_set_param('Tg_init')) then
          call query_param ('Tg_init',dim,ptype)
          if(dim>ngm) dim=ngm
          allocate (Tg_init(ngm))
          call sync_param('Tg_init',Tg_init(1:dim),dim)
          if (dim<ngm) Tg_init(dim+1:ngm) = Tg_init(dim)
          do ng=1,ngm
            temp(ng,:,:) = Tg_init(ng)
          end do
          deallocate (Tg_init)
        ...
        wetness = .8d0
        if (is_set_param('Gwet_init')) then
          call query_param ('Gwet_init',dim,ptype)
          if(dim>ngm) dim=ngm
          allocate (Gwet_init(ngm))
          call sync_param('Gwet_init',Gwet_init(1:dim),dim)
          if (dim<ngm) Gwet_init(dim+1:ngm) = Gwet_init(dim)
          do ng=1,ngm
            wetness(ng,:,:) = Gwet_init(ng)
          end do
          deallocate (Gwet_init)
        end if
        ! Snow depth in m (water equivalent)
        snowdp = 0.
        if (is_set_param('snowdp_init')) then
          call sync_param('snowdp_init',snowdp_init)
          snowdp = snowdp_init
        endif
        tsnowtop = temp(1,:,:) 
        ...
        do ifrac=1,ls_nfrac
          ...
          enddo
        enddo
        deallocate (temp, wetness, snowdp)
...
!@ver  beta
     ...
     &     'fr_sat_ij(dist_im,dist_jm)')
      ...
      call defvar(grid,fid,mCO2cond_ij,'mCO2cond_ij(dist_im,dist_jm)')
#endif
...
#ifdef DO_CO2_CONDENSATION
        call write_dist_data(grid,fid,'mCO2cond_ij',mCO2cond_ij)
#endif
...
#ifdef DO_CO2_CONDENSATION
        call read_dist_data(grid,fid,'mCO2cond_ij',mCO2cond_ij)
#endif
