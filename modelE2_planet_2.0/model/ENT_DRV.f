...
!     &     grid%J_STRT_HALO:grid%J_STOP_HALO,N_COVERTYPES)
     ...
     &     grid%J_STRT_HALO:grid%J_STOP_HALO)
      ...
      logical :: prog_veg_flag

      prog_veg_flag = .false.
      if ( present(prog_veg) ) prog_veg_flag = prog_veg
...
cddd     &       cropsdata=cropdata_H(I0:I1,J0:J1) )
     ...
     & ,x_ent_in,meg)
      ...
      real*8, dimension(N_COVERTYPES), intent(in), optional :: x_ent_in
      real*8, dimension(N_COVERTYPES) :: x_ent
      real*8, dimension(nMeganPFT), intent(out) :: x_megan
      real*8, dimension(nMeganPFT) :: a_megan
      integer, dimension(N_COVERTYPES), intent(out), optional :: meg
