...
! Committing the following MPI hints with a hard-coded value of cb_nodes
! until the requisite information about the GPFS configuration is obtained
! which would allow cb_nodes (and cb_buffer_size/striping_unit) to be
! determined as a function of model resolution and access patterns.
! In E2.x, automatic values of cb_buffer_size/striping_unit appear to perform
! as well as optimized ones at the moment.
! Note that the hints can also be set via environment variable
! ROMIO_HINTS, e.g. ROMIO_HINTS=romio_hints.txt, with romio_hints.txt formatted as:
! cb_nodes 1
...
! striping_unit 33554432
      ...
      call mpi_info_set(info,"cb_nodes","1", mpierror)
      call mpi_info_set(info,"romio_cb_read","enable", mpierror)
      call mpi_info_set(info,"romio_cb_write","enable", mpierror)
      !call mpi_info_set(info,"cb_buffer_size","33554432", mpierror)
      !call mpi_info_set(info,"striping_unit", "33554432", mpierror)
...
#include "setup_attget_pnc.inc"
     ...
     &     ntiles,rc,vid,am_root,with_record_dim)
      ...
      logical :: am_root
      character(len=40) :: vname,dname,unlimdim_name
...
c
      ...
      if(ndims.gt.0) then
        ...
        do xdim=1,ndims
          ...
          elseif(xdim.eq.ndims .and. dname(1:6).eq.'unlim_') then
            unlimdim_name = dname(7:len_trim(dname))
            exit
          endif
     ...
     &             trim(dname),' for variable ',trim(vname)
      ...
      if(with_record_dim) then
        if(len_trim(unlimdim_name).gt.0) then
          if(am_root) write(6,*)
     &         'illegal operation: multiple unlim dims '//
     &         ' for variable ',trim(vname)
          rc = 1; return
        else
          unlimdim_name = 'record'
        endif
      elseif(len_trim(unlimdim_name).gt.0) then
        ndims = ndims - 1  ! decrement will be undone later
      endif
...
c
      if(len_trim(unlimdim_name).gt.0) then
        ...
        if(dids(ndims).le.0) then ! record dim needs defining
          status = nfmpi_def_dim(fid,trim(unlimdim_name),
     &         nfmpi_unlimited,dids(ndims))
        endif
      endif
c
