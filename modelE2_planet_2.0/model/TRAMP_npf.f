...
!-------------------------------------------------------------------------------------------------------------------
      ...
      !                               INUC = 5, TIMN, Yu et al 2018
      !-------------------------------------------------------------------------------------------------------------
      !INTEGER, PARAMETER :: INUC = 2 Now set in TRAMP_param
      ...
      ! Parameters for Yu nucleation scheme
      !  Module Variables:
      !  ============================================================================
      !  Parameters
      ...
      !  (6 ) MB   : NUMBER OF POINTS IN [NH3] DIMENSION  Arrays
      ...
      !  (6 ) B   : VALUES AT POINTS IN [NH3] DIMENSION
      !  (7) XJTIMN : ION-MEDIATED NUCLEATION RATES (cm-3s-1) AT ALL POINTS IN
...
!-------------------------------------------------------------------------------------------------------------------
      ...
      REAL(8), INTENT(IN)   :: FLAND     ! land fraction
      ...
      IF (INUC.ne.5) CALL NUCLEATION_RATE( TEMP, 1.0D+02*RH, H2SO4_TMP, NH3_TMP,
     +                      ZHEIGHT(ILAY), JTOT, JGAS, JION )
      IF (H2SO4_TMP.GT.5.D5.AND.INUC.eq.5) 
     +  CALL NUCLEATION_RATE_YU(PRS,KC,TEMP, 1.0D+02*RH, FLAND,H2SO4_TMP, NH3_TMP,
     +                      ZHEIGHT(ILAY), XQ,JTOT, JGAS, JION,JBHN)
      ...
      !-------------------------------------------------------------------------------------------------------------
      IF( INUC.EQ.1 .OR. INUC.EQ.2 .OR. INUC.EQ.3 .OR. INUC.EQ.5 ) THEN
        IF( KK02 ) THEN
      ...
      ! WRITE(34,'(A,4D9.2)')'IN NPFRATE: JTOT, JP_TO_J, DNDT, H2SO4 = ', JTOT, JP_TO_J, DNDT, H2SO4_TMP
...
!-----------------------------------------------------------------------------------------------------------------------
      ...
      IF ( INUC.ne.5.and.INCLUDE_ION_ION ) THEN
      ...
      SUBROUTINE NUCLEATION_RATE_YU (PRESS,CS,T,RH,FLAND,NA,MB,Z,XQ,JTOT,JGAS,JION,JBHN)
!-----------------------------------------------------------------------------------------------------------------------
      ...
      REAL(8), INTENT( IN ) :: CS     ! Cond Sink [s-1]   
      REAL(8), INTENT( IN ) :: FLAND  ! land fraction
      ! Output arguments.
      REAL(8)::  XQ
      ...
      REAL(8) :: JION ! ion-ion     nucleation rate [#/cm^3/s]
      REAL(8) :: JTOT ! total       nucleation rate [#/cm^3/s]
      REAL(8) :: JBHN, XN0,XNH3,VGAS,XS,XJBH,XJBIM,XJTH,XJTIM
      REAL(8) :: RBH,RBIM,RTH,RTIM
      
        IF(Z .LT. 1.00) THEN   ! To be updated later
          XQ = 2.00D+00 + FLAND*8.0D+00*(1.D+00-Z )
        ELSEIF(Z .GE. 1.00D+00 .AND. Z .LT. 2.25D+00) THEN
          XQ = 2.00D+00
        ELSEIF(Z .GE. 2.25D+00 .AND. Z .LT. 11.00D+00) THEN
          XQ = 3.10D+00 * (Z - 2.25D+00) + 2.00D+00
        ELSEIF ( Z .GE. 11.00D+00) THEN
          XQ = 29.00D+00 + (Z - 11.00D+00)
        ENDIF

!        PRESS = 101325.0*exp(-0.12*Z)    ! Z in km and Press in Pa
        XN0 = 1.d-6*PRESS/(1.3807d-23*T)   !#/cm3
        XNH3 = MB*1.d-12*XN0  !ppt to #/cm3

! From CS to XS (no Fcorr for now)
!        CS = 0.25*VGAS*AREA*FCORR  ! s-1
        VGAS = SQRT(216.03*T)*100.  ! cm/s H2SO4 gas velocity

        XS = CS*4./VGAS  ! CS in s-1 , XS in cm2/cm3
        XS =  XS*1.E8   ! XS is surface area in um2/cm3

        CALL YUJTIMN1(NA,RH,T,XQ,XS,XNH3,XJBH,XJBIM,XJTH,XJTIM,
     +                RBH,RBIM,RTH,RTIM)

        JTOT = XJTIM
        JGAS = XJTH   ! binary and ternary hom nucl
        JION = XJBIM  ! binary nucl
        JBHN = XJBH   ! binary nucl

      RETURN
      END SUBROUTINE NUCLEATION_RATE_YU
...
!
! The present lookup table should cover almost all the possible
! conditions in the troposphere relevant to atmospheric nucleation. The range and
! resolution in each parameter space can be extended in the future if needed.
!
! Written by
! Fangqun Yu
...
!      H2SO4 molecules (and associated NH3/H2O molecules, mass diameter
!      ~1.5 nm)
...
! R##: Radius of critical cluster (nm)
!
...
!------------------------------------------------------------------------------
        ...
        SUBROUTINE READJTIMN
!     
!        WRITTEN by Fangqun Yu, SUNY-Albany
!        ModelEed by Susanne Bauer
!
        ...
        filename=trim('AMP_NUC')//'/TIMN_1H2SO4.txt'
        call openunit(filename, unit_number, .false., .true.)
         read(unit_number,100)(C(IC),IC=1,MC)
        call closeunit(unit_number)
      
        filename=trim('AMP_NUC')//'/TIMN_2RH.txt'
        call openunit(filename, unit_number, .false., .true.)
        read(unit_number,100)(RH(IRH),IRH=1,MRH)
        call closeunit(unit_number)

        filename=trim('AMP_NUC')//'/TIMN_3T.txt'
        call openunit(filename, unit_number, .false., .true.)
        read(unit_number,100)(T(IT),IT=1,MT)
        call closeunit(unit_number)
        
        filename=trim('AMP_NUC')//'/TIMN_4Q.txt'
        call openunit(filename, unit_number, .false., .true.)
        read(unit_number,100)(Q(IQ),IQ=1,MQ)
        call closeunit(unit_number)
        
        filename=trim('AMP_NUC')//'/TIMN_5S.txt'
        call openunit(filename, unit_number, .false., .true.)
        read(unit_number,100)(S(IS),IS=1,MS)
        call closeunit(unit_number)
        
        filename=trim('AMP_NUC')//'/TIMN_6B.txt'
        call openunit(filename, unit_number, .false., .true.)
        read(unit_number,100)(B(IB),IB=1,MB)
        call closeunit(unit_number)
...
!
        filename=trim('AMP_NUC')//'/TIMN_J6D.txt'
        call openunit(filename, unit_number, .false., .true.)
        DO IS =1, MS
         DO IT = 1,MT
         DO IRH = 1,MRH
         DO IQ =1, MQ
         DO IB =1, MB 
          READ(unit_number,201)(XJTIMN(IC,IRH,IT,IQ,IS,IB),IC = 1,MC)
          DO IC=1, MC
...
! Critical cluster properties depend on T, RH, [H2SO4], NH3 only
        filename=trim('AMP_NUC')//'/TIMN_Rstar4D.txt'
        call openunit(filename, unit_number, .false., .true.)
        DO IT = 1,MT
         DO IRH = 1, MRH
         DO IB =1, MB 
           READ(unit_number,203)(XRSTAR(IC,IRH,IT,IB),IC=1,MC)
         ENDDO  ! B
         ENDDO  ! RH
        ENDDO   !T
        call closeunit(unit_number)
