...
C**** from total hemispere ACCPDA (kg per year)
      DTSRCzY = DTSRC /
     &     real(calendar%getSecondsPerDay()*calendar%getMaxDaysInYear())
...
C**** hemispheric masking (why isn't this in GEOM?)
      ...
      CalendarDaysPerYear = real(calendar%getMaxDaysInYear())
C**** Subtract daily ice berg mass that is added to ocean in DAILY_OC
      MICBIMP(:) = MICBIMP(:)-(/  ACCPDA,  ACCPDG /)
     &     / CalendarDaysPerYear
      EICBIMP(:) = EICBIMP(:)-(/ EACCPDA, EACCPDG /)
     &     / CalendarDaysPerYear
...
C**** adjust hemispheric mean glacial melt amounts (only on root processor)
     ...
     *         ,mdwnimp_NH
          if ( accpda > 1d-30 .and. accpdg > 1d-30) then
     ...
     *           ,(eaccpdg/accpdg+lhm)/shi
          ...
          do itm=1,ntm
             if (to_per_mil(itm).gt.0) then
                if (accpda .ne. 0) then
                   write(6,*) trim(trname(itm)),
     *                  " (AQ only before) (permil)",
     *                  1000 * (traccpda(itm)/accpda/trw0(itm)-1.)
                else
                   write(6,*) trim(trname(itm)), " accepda is 0"
                endif
             else
                write(6,*) trim(trname(itm)),
     *               " (AQ only before) (kg)", traccpda(itm), accpda
             end if
          end do
#endif
#endif   /* TNL: inserted */
     ...
     *           (eaccpdg/accpdg+lhm)/shi
        ...
        If (AM_I_ROOT())  Then
          do itm=1,ntm
             if (to_per_mil(itm).gt.0) then
              write(6,*) trim(trname(itm)),
     *               " (AQ only after) (permil)",
     *               1000 * (traccpda(itm)/accpda/trw0(itm)-1.)
            else
              write(6,*) trim(trname(itm)),
     *              " (AQ only after) (kg)", traccpda(itm), accpda
            end if
          end do
        ENDIF
#endif
...
#endif /* TNL: inserted */
          ELSE
            IF (atmocn%GMELT(I,J) > 0.d0 ) then
              call stop_model("You can't dump icebergs on the ground!"//
     &             " Fix GLMELT file or disable snow removal."//
     &             " To disable snow removal set wsn_max=0. .", 255)
            ENDIF
          ENDIF
        END DO
      END DO
