...
c****
      USE DOMAIN_DECOMP_ATM, only: grid, globalmax
...
ccc   use QUSCOM, only : im,jm,lm, xstride,am,f_i,fmom_i
     ...
     &     ,MASS_I(IM), COURMAX, BYNSTEP
      integer :: i,ip1,j,l,ierr,nerr,ICKERR,ns,nstep,err_all
...
c
      ...
      enddo ! l
C
      Call GLOBALMAX (grid, ICKERR, err_all)
      IF(err_all > 0)  CALL stop_model('Stopped in aadvtx',11)
C
      return
c****
...
ccc   use QUSCOM, only : im,jm,lm, ystride,bm,f_j,fmom_j, byim
     ...
     &        FMOM_J(NMOM,IM,grid%J_STRT_HALO:grid%J_STOP_HALO,LM)
      integer :: i,j,l,ierr,ICKERR, err_loc(3), err_all
      REAL*8, DIMENSION(LM) ::
...
C
      Call GLOBALMAX (grid, ICKERR, err_all)
      IF(err_all > 0)  CALL stop_model('Stopped in aadvty',11)
C
      return
c****
...
ccc   use QUSCOM, only : im,jm,lm, zstride,cm,f_l,fmom_l
     ...
     &        :: rmom
      ...
      integer :: i,j,l,ierr,nerr,ICKERR,ns,nstep, err_all
c**** Get useful local parameters for domain decomposition
      integer :: J_0, J_1
      call getDomainBounds( grid, J_STRT=J_0 , J_STOP=J_1 )
...
C
      Call GLOBALMAX (grid, ICKERR, err_all)
      IF(err_all > 0) call stop_model('Stopped in aadvtz',11)
      return
c****
