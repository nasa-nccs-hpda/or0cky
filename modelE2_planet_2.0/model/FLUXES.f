...
#ifdef TRACERS_DRYDEP_DIAG
     &       ,egs1,egcut_dry1
     &       ,eggro_dry1,egcut_wet1,eggro_wet1
     &       ,egcut_snow1,eggro_snow1,vd_for
     &       ,vd_cro,vd_gra
     &       ,vd_shr,vd_bar
     &       ,egs_for,egs_cro
     &       ,egs_gra,egs_shr
...
!@dbparam flake_sill lake fraction at sill level (FLAKE0)
      real*8 :: flake_sill = -1. ! used if TOPO file is not present
...
!@var dmCO2cond change in CO2 condensate per time step (kg/m^2)
      REAL*8, ALLOCATABLE, DIMENSION(:,:) :: dmCO2cond
...
#ifdef TRACERS_DRYDEP
!@var depo_flux_glob global array of flux due to dry dep. of tracers
!@+   [kg/m^2/s]
      REAL*8,ALLOCATABLE,DIMENSION(:,:,:) :: depo_flux_glob
#ifdef TRACERS_DRYDEP_DIAG_SUBDD
!@var global arrays of deposition velocities/conductances
      REAL*8,ALLOCATABLE,DIMENSION(:,:,:) :: vd_glob_for,vd_glob_cro,
     &                                        vd_glob_shr,vd_glob_gra,
     &                                        egs_glob_for,egs_glob_cro,
     &                                        egs_glob_shr,egs_glob_gra,
     &                                        egcut_wet_glob
#endif
#endif
...
!@sum FLAKE0/FEARTH0 are the values if lakes are at the sill level i.e.
!@sum the max. level before lake outflow occurs
      if(file_exists('TOPO')) then
        ...
        FOCEAN=0 ; FLAKE0=0 ; FEARTH0=1 ; FLICE=0 ! land planet
        call sync_param("flake_sill",flake_sill)  ! ... with lakes
        if(flake_sill > 0) flake0 = flake_sill
        FEARTH0 = 1 - flake0
      end if
...
!   *       ,i,j,FOCEAN(I,J),FLAKE0(I,J)
            FEARTH0(I,J) = FEARTH0(I,J) + FLAKE0(I,J)
            FLAKE0(I,J)=0
          END IF
...
#endif
#ifdef TRACERS_DRYDEP
      ALLOCATE(depo_flux_glob(I_0H:I_1H,J_0H:J_1H,Ntm)
     &     ,STAT = IER)
#ifdef TRACERS_DRYDEP_DIAG_SUBDD
      ALLOCATE(vd_glob_for(I_0H:I_1H,J_0H:J_1H,Ntm)
     &     ,STAT = IER)
      ALLOCATE(vd_glob_cro(I_0H:I_1H,J_0H:J_1H,Ntm)
     &     ,STAT = IER)
      ALLOCATE(vd_glob_shr(I_0H:I_1H,J_0H:J_1H,Ntm)
     &     ,STAT = IER)
      ALLOCATE(vd_glob_gra(I_0H:I_1H,J_0H:J_1H,Ntm)
     &     ,STAT = IER)
      ALLOCATE(egs_glob_for(I_0H:I_1H,J_0H:J_1H,Ntm)
     &     ,STAT = IER)
      ALLOCATE(egs_glob_cro(I_0H:I_1H,J_0H:J_1H,Ntm)
     &     ,STAT = IER)
      ALLOCATE(egs_glob_shr(I_0H:I_1H,J_0H:J_1H,Ntm)
     &     ,STAT = IER)
      ALLOCATE(egs_glob_gra(I_0H:I_1H,J_0H:J_1H,Ntm)
     &     ,STAT = IER)
      ALLOCATE(egcut_wet_glob(I_0H:I_1H,J_0H:J_1H,Ntm)
     &     ,STAT = IER)
#endif
#endif
#if (defined TRACERS_DUST) || (defined TRACERS_MINERALS) ||\
...
c      call defvar(grid,fid,atmsrf%dclev,'dclev(dist_im,dist_jm)')
     ...
     &   ,'lmonin_pbl(dist_im,dist_jm)')
      ...
      call defvar(grid,fid,dmCO2cond,'dmCO2cond(dist_im,dist_jm)')
#endif
      return
...
#ifdef DO_CO2_CONDENSATION
        call write_dist_data(grid,fid,'dmCO2cond',dmCO2cond)
#endif
...
#ifdef DO_CO2_CONDENSATION
        call read_dist_data(grid,fid,'dmCO2cond',dmCO2cond)
#endif
