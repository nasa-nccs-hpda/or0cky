...
!@dbparam KOCEAN: if 0 => specified, if 1 => predicted ocean
      integer :: KOCEAN = 1
!@dbparam ngrids: set 1 | 2 if atm and ocn grids are same | different
      integer :: ngrids = 2
...
! are removed from the INPUTZ namelist (harder than it sounds)
      ...
      select case (planetParams%planetName)
      ...
      case default
        ...
        allocate(orbit, source=makePlanetOrbit())
     ...
     &        source=newParameterizedEarthOrbit(orb_par_year_bp))
      ...
      function makePlanetOrbit() result(orbit)
      use PlanetaryOrbit_mod, only: PlanetaryOrbit
      use QuantizedPlanetaryOrbit_mod, only: QuantizedPlanetaryOrbit
      use BaseTime_mod
      ...
      class (PlanetaryOrbit), allocatable :: orbit
      ...
      real*8 :: Porbit, Protation, Psolar
      integer :: daysPerYear

      associate (p => planetParams)

      if (p%quantizeYearLength) then
         allocate(orbit, source=QuantizedPlanetaryOrbit(p))
      else
         allocate(orbit, source=PlanetaryOrbit(p))
      end if

      if (AM_I_ROOT()) then
         write(*,*) 'Planet :: ' // trim(planetParams%planetName)
         Psolar = real(orbit%getMeanDay())
         Protation = real(orbit%getSiderealRotationPeriod())
         Porbit = real(orbit%getSiderealOrbitalPeriod())
         write(*,*) '   Mean solar day: '
         write(*,*)'       ', Psolar,' (sec)'
         write(*,*)'       ', Psolar / INT_SECONDS_PER_DAY,
     &        ' (Earth days)'

         write(*,*)'   SiderealRotationPeriod:'
         write(*,*)'       ', Protation,' (sec)'
         write(*,*)'       ', Protation / INT_SECONDS_PER_DAY,
     &        ' (Earth days)'

         write(*,*)'   SiderealOrbitalPeriod:'
         write(*,*)'       ', Porbit,' (sec)'
         write(*,*)'       ', Porbit / INT_SECONDS_PER_DAY,
     &        ' (Earth days)'

         write(*,*)'   meanDistance (AU)           :',
     &        orbit%getMeanDistance()
         write(*,*) '   Precession (degs from VE)  :',
     &        orbit%getLongitudeAtPeriapsis()
         if (p%quantizeYearLength) then
            write(*,*)'    Solar Days per year: ',
     &           nint(pOrbit / pSolar) ! Int
         else
            write(*,*)'    Solar Days per year: ',
     &           (pOrbit / pSolar) ! FP
         end if
...
C**** Accumulating_period information
      ...
      CHARACTER*18 :: ACC_PERIOD !@var string MONyr1-yr2, yr2<10,000,000

      type :: AccumulationPeriod
#ifdef GCC_4_9_COMPAT
        ! this will reserve 7 dgits for the year
        character(len=10) :: begin
        character(len=10) :: end
#else
        character(len=:), allocatable :: begin
        character(len=:), allocatable :: end
#endif
      end type AccumulationPeriod
      type (AccumulationPeriod) :: new_acc_period
...
!@+       diagnostic categories, and is not meaningful for some.
      ...
      SUBROUTINE aPERIOD (JMON1,JYR1,JMON,JYR,months,years,moff,
     &   new_aDate,aDATE,LDATE)
!@sum  aPERIOD finds a 7 or 12-character name for an accumulation period
!@+   Parts that were needed only for off-line purposes were deleted or
!@+   commented out (moff is not needed here, e.g.)
!@auth Reto A. Ruedy
...
!@var JMON1,JYR1 month,year of beginning of period 1
      INTEGER JMON1,JYR1
!@var JMON - current month
      integer :: JMON
!@var JYR - current year
      integer :: JYR
...
!!!!  moff>0 only needed off-line, e.g. to combine Jan,Feb,Dec from several years
      integer moff
!@var yr1 end-year of the first period (not =JYR1 if period contains Dec & Jan)
      INTEGER yr1
!@var aDATE date string: MONyyr1(-yyr2) or ANNyyr1(-yyr2) etc.
      character*18 aDATE         ! limits ndigits_year to SEVEN !
!@var new_aDate data structure containing date elements MON, yyr1, yyr2
      type (AccumulationPeriod) :: new_aDate
!@var LDATE length of date string (PERy...y or PERy...y-y...y)
      INTEGER LDATE

      character(len=6) :: year_fmt
      integer :: ndigits_year = 4 ! default digits in year string

      call sync_param('ndigits_year', ndigits_year)

      LDATE = 3+ndigits_year
      if(years.gt.1) LDATE = 4+2*ndigits_year ! e.g. ANNyyy1-yyy2

#ifdef GCC_4_9_COMPAT
      ! the strings are fixed (len=10), just fill them with spaces
      new_aDate%begin = "          "    ! limits ndigits_year to SEVEN
      new_aDate%end   = "          "
#else
      if (allocated(new_aDate%begin)) then
         deallocate(new_aDate%begin)
      end if
      allocate(character(len=3+ndigits_year) :: new_aDate%begin)

      if (allocated(new_aDate%end)) then
         deallocate(new_aDate%end)
      end if
      allocate(character(len=3+ndigits_year) :: new_aDate%end)
#endif

      new_aDate%begin(1:3) = AMONTH(JMON1)
      new_aDate%end(1:3) = AMONTH(JMON)
      write(year_fmt,'("(i",i1.1,".",i1.1,")")')
     &     ndigits_year,ndigits_year

      yr1=JYR1 ! label uses the end-year of a period, e.g. dec1949-feb1950=DJF1950
      JMONL=JMON1+months-1                ! JMONL is the month before jmon
      if(JMONL > INT_MONTHS_PER_YEAR) then
         yr1=yr1+1
         JMONL=JMONL-INT_MONTHS_PER_YEAR
      end if

      write(new_aDate%begin(4:),year_fmt) Jyr1
      write(new_aDate%end(4:),year_fmt) jyr

      aDATE(1:18)=' '
      aDATE(1:3)=AMONTH(JMON1)  ! letters 1-3 of month IF months=1

      write(aDATE(4:),year_fmt) yr1
      if(years.gt.1) then
         write(aDATE(5+ndigits_year:),year_fmt) jyr
         if(jmon==1) write(aDATE(5+ndigits_year:),year_fmt) jyr-1
         aDATE(4+ndigits_year:4+ndigits_year) = '-'
      end if

      if(months.gt.INT_MONTHS_PER_YEAR) then ! should not happen
         aDATE(1:1)='x'
         new_aDate%begin(1:1)='x' ; return
      end if

      if(months.le.1) return
...
!****    =FnL where n=length of period if n>3         4or6 month periods
      aDATE(3:3)=AMONTH(JMONL)(1:1)            ! we know: months>1
      IF (months.eq.2) then
        aDATE(2:2)='+'
      else if (months.eq.3) then
        JMONM = JMONL-1
        if (jmonm.le.0 ) jmonm = jmonm + INT_MONTHS_PER_YEAR
        aDATE(2:2)=AMONTH(JMONM)(1:1)
      else if (months.ge.4.and.months.le.9) then ! here 4 or 6
         write (aDATE(2:2),'(I1)') months
         IF (months.eq.6) THEN                    !  2 exceptions:
            IF (JMON1.eq. 5) then         ! NH warm season May-Oct
              aDATE(1:3)='NHW'
            else IF (JMON1.eq.11) then    ! NH cold season Nov-Apr
              aDATE(1:3)='NHC'
            end if
         END IF
!!       IF (months.eq.7) THEN                    !    to avoid ambiguity:
!!         IF (JMON1.eq. 1) then    ! Jan-Jul J7J->J7L
!!           aDATE(1:3)='J7L'
!!         end if
!!         IF (JMON1.eq. 7) then     ! Jul-Jan J7J->L7J
!!           aDATE(1:3)='L7J'
!!         end if
!!       END IF
!!    else IF (months.eq.10) then         ! roman 10
!!      aDATE(2:2)='X'
!!    else IF (months.eq.11) then         ! hex   11
!!      aDATE(2:2)='B'
!!    IF (months.eq.7) THEN                    !    to avoid ambiguity:
!!       IF (JMON1.eq. 1) then    ! Jan-Jul J7J->J7L
!!         aDATE(1:3)='J7L'
...
!!      end if
!!    END IF
      else IF (months.eq.INT_MONTHS_PER_YEAR) THEN
C****    beg=ANn where the period ends with month n if n<10 (except 4)
         aDATE(1:3)='ANN'                ! regular annual mean, JMONL=12
         IF (JMONL.le. 9) then           ! AN1=Feb-Jan,...,AN9=Oct-Sep
            WRITE(aDATE(3:3),'(I1)') JMONL
         end if
         IF (JMONL.eq. 4) then      ! NH warm+cold seasons
            aDATE(1:3)='W+C'              ! May-Oct+Nov-Apr
         else IF (JMONL.eq.10) then      ! NH cold+warm seasons
            aDATE(1:3)='C+W'              ! Nov-Apr+May-Oct
          else IF (JMONL.eq.11) then           ! meteor. annual mean
            aDATE(1:3)='ANM'              ! Dec-Nov = DJF+MAM+JJA+SON
          end if
      END IF
      new_aDate%begin(1:3) =  aDATE(1:3) ! probably incorrect (probably not used)
      return
...
!@auth Gavin Schmidt
      ...
      Jyr=Iyr0+It/(Nday*calendar%getDaysInYear(year=1))
      Jd=1+It/Nday-(Jyr-Iyr0)*calendar%getDaysInYear(year=1)
      Jmn=1
