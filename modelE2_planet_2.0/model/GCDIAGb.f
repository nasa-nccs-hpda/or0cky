...
!@+   JL/JK refer to model versus constant-pressure levels
     ...
     *  jk_del_qgpv,jk_nt_lh_se,jk_wstar,jk_vstar
     ...
     &     jl_nt_lh_e,jl_vt_lh_e,jl_dp1,jl_dpb,jl_epflx_div
...
c
      k=k+1
      jl_dp1 = k ! only used as a denominator
      sname_gc(k) = 'jl_dp1'
c      lname_gc(k) = 'MASS AT PRIMARY LATITUDES' ! in DIAGA
c      units_gc(k) = 'mb'
      scale_gc(k) = 1.
      jgrid_gc(k) = 1
c
...
c local vars for transplanted DIAGA calculations
     ...
     &               HAVE_NORTH_POLE=HAVE_NORTH_POLE)
      ...
      do j=j_0,j_1
      do l=1,lm
        agc(j,l,jl_dp1) = agc(j,l,jl_dp1) + sum(pdsig(l,:,j))
      enddo
      enddo
...
c      IF (DTHDP.LT.SMALL) WRITE (6,999) J,L,DTHDP,SMALL
     ...
     *   -U(I,J,L)+UMN)
  866 I=Ip1
  868 AGC(J,L,JL_EPFLXN)=AGC(J,L,JL_EPFLXN)+FPHI*kg2mb
...
C**** SPECTRAL ANALYSIS OF AVAILABLE POTENTIAL ENERGY
      ...
      DO L = 1, LM
        ...
        DO J=J_0,J_1
          ...
          if(m5.eq.7) then
            AGC(J,L,JL_APE)=AGC(J,L,JL_APE)+SUM(X*X)*GMEAN(L)
          endif
...
C     agc(1,l,jk_dyn_conv_eddy_geop)=-(AGC(2,l,JK_EDDNTGEO))
     ...
     &      +(AGC(J,L,JK_EDDVTPT)-AGC(J,L-1,JK_EDDVTPT))*PKM(L)*IM
      ...
      do j=2,jm-1
        do l=1,lm
          agc(j,l,n) = bydapo(j)*
     &         (agc(j+1,l,jl_epflxn)*dxv(j+1)
     &         -agc(j  ,l,jl_epflxn)*dxv(j  ))
        enddo
        agc(j, 1,n) = agc(j,1,n) +.5*agc(j,1,jl_epflxv)
        do l=2,lm
          agc(j,l,n) = agc(j,l,n) +.5*
     &         (agc(j,l,jl_epflxv)-agc(j,l-1,jl_epflxv))
        enddo
      ENDDO
