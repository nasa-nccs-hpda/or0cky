...
!@auth I. Alienov, N. Kiang, Y. Kim
      ...
      use ent_mod, only: N_COVERTYPES, N_OTHER,COVER_SAND,COVER_DIRT !ykim - use ent_const to accomodate GISS and Ent PFTs.
...
cddd     &     /)
     ...
     &             0d0,1d30,'none',year,day)
      ...
      if ( have_veg_file .and. is_timestream ) then
        ...
        vdata(:,:,:) = 0.
        call sync_param("fsand",fsand) 
        vdata(:,:,COVER_SAND)=fsand ! bright soil fraction
        vdata(:,:,COVER_DIRT)=1d0 - fsand ! dark soil fraction
      endif
