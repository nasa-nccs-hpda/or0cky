...
C**** ADVECT ICE
     ...
     &     do_IC_fixups,is_coldstart)
      ...
      use domain_decomp_atm, only : grid ! not fundamentally needed
      implicit none
!@var istart start(1-8)/restart(>8)  option
      ...
      ! If an ocean restart file is provided during a cold start,
      ! use it instead of the default cold-start ICs.
      IF(iniOCEAN .and. file_exists('Orsf')) then
        iniOCEAN = .false.
        fid = par_open(grid,'Orsf','read')
...
C**** Initialize sea ice
      if(istart<=2 .and. iniOCEAN) call read_seaice_ic
