...
!@var nt3Dsrcmax maximum number of 3D tracer sources/sinks
      integer, parameter :: NT3DSRCMAX=11
...
!@var nsrc number of source to define ntsurfsrc(n)
    ...
    loop_n: do
      ...
      if (file_exists(trim(fname))) then ! includes directories
        nsrc=nsrc+1
        if (nsrc > ntsurfsrcmax) then
          write(out_line,*)'Too many surface sources found for '//&
                           trim(getName(trcer))//' in findSurfaceSources.'
  ...
  subroutine addSurfaceSource(this, sourceName, sourceLname, skipReason)
    ...
    if (this%ntSurfSrc==ntsurfsrcmax) then
      write(out_line,*)'Cannot add '//trim(sourceName)//' in '//&
                       'addSurfaceSource, exceeded ntsurfsrcmax.'
      call write_parallel(trim(out_line))
      call stop_model(trim(out_line),255)
    endif
