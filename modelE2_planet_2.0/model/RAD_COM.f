...
!@auth Original Development Team
      ...
      USE RADPAR, only : S0,nraero_aod=>NTRACE,kyearvm
...
!@var scatau_as Same as tau_as for scattering
!@var g_as aerosol asymmetry parameter
      ...
      REAL*8,ALLOCATABLE,DIMENSION(:,:,:,:) :: scatau_as
      REAL*8,ALLOCATABLE,DIMENSION(:,:,:,:) :: g_as
#ifdef CACHED_SUBDD
...
!@var swftoa Shortwave aerosol radiative forcing at TOA
!@var swfsfc Shortwave aerosol radiative forcing at SFC
!@var lwfrc Shortwave aerosol radiative forcing
      ...
      REAL*8,ALLOCATABLE,DIMENSION(:,:,:) :: swftoa,swfsfc,lwfrc
#endif  /* CACHED_SUBDD */
#endif
#endif
...
!@+   as defined in RADN7, nominally volcanic aerosols in the stratosphere.
...
!@+   special cases: Volc_yr=-1    : mean as defined by volc_mean.
!@+                                  Default is the CMIP6 range, 1850-2014.
...
!@dbparam volc_mean Range of years to be used when volc_yr=-1
      integer, dimension(2) :: volc_mean = (/1850,2014/)
...
!@dbparam cldX cloud scaling factor
      REAL*8 :: cldX = 1.
...
!@dbparam cloud_rad_forc = 1 for calculation of cloud + aerosol + O3
!@+   radiative forcing
      INTEGER :: cloud_aer_o3_rad_forc = 1
...
!@auth Rodger Abel
     ...
     *     chem_tracer_save(2,LM, I_0:I_1, J_0:J_1),
     *     rad_to_chem(LM, I_0:I_1, J_0:J_1),
...
!@ver  beta
     ...
     &       'tau_cs(dist_im,dist_jm,lm,nraero_aod)')
        call defvar(grid,fid,scatau_as,
     &       'scatau_as(dist_im,dist_jm,lm,nraero_aod)')
        call defvar(grid,fid,g_as,
     &       'g_as(dist_im,dist_jm,lm,nraero_aod)')
     ...
     &         'abstau_dry(dist_im,dist_jm,lm,nraero_aod)')
        ...
        if (nraero_rf>0) then
        call defvar(grid,fid,swftoa,'swftoa(dist_im,dist_jm,nraero_rf)')
        call defvar(grid,fid,swfsfc,'swfsfc(dist_im,dist_jm,nraero_rf)')
...
!@ver  beta new_ prefix avoids name clash with the default version
     ...
     &       'rad_to_chem', rad_to_chem, jdim=3)
        ...
        if (nraero_aod > 0) then
          ...
          call write_dist_data(grid,fid,'scatau_as',scatau_as)
          call write_dist_data(grid,fid,'g_as',g_as)
          if (save_dry_aod>0) then
          ...
          if (nraero_rf>0) then
            call write_dist_data(grid,fid,'swftoa',swftoa)
            call write_dist_data(grid,fid,'swfsfc',swfsfc)
     ...
     &                   bcast_all=.true.)
          if (nraero_aod_rsf /= 0) then
            ...
            allocate(scatau_as(I_0H:I_1H,J_0H:J_1H,lm,nraero_aod_rsf))
            allocate(g_as(I_0H:I_1H,J_0H:J_1H,lm,nraero_aod_rsf))
     ...
     &                     bcast_all=.true.)
            if (nraero_rf_rsf>0) then
              allocate(swftoa(I_0H:I_1H,J_0H:J_1H,nraero_rf_rsf))
              allocate(swfsfc(I_0H:I_1H,J_0H:J_1H,nraero_rf_rsf))
        ...
        if (allocated(tau_as)) then ! needs to be separate from previous if
          ...
          call read_dist_data(grid,fid,'scatau_as',scatau_as)
          call read_dist_data(grid,fid,'g_as',g_as)
          if (save_dry_aod_rsf>0) then
          ...
          if (nraero_rf_rsf>0) then
            call read_dist_data(grid,fid,'swftoa',swftoa)
            call read_dist_data(grid,fid,'swfsfc',swfsfc)
