...
#ifndef ODIFF_FIXES_2022
#define ODIFF_FIXES_2022
#endif

#ifdef TRACERS_OCEAN_INDEP
#undef TRACERS_WATER /* temporary workaround */
#endif
...
C**** Calculate KH=rho_0 BETA (sqrt(3) L_Munk/pi)^3, L_Munk=min(DX,DY)
        ...
        KHP(J)=AKHFAC*2d0*RHOWS*ABS(OMEGA)*COSPO(J)*(DSP**3)/RADIUS ! A-grid
        KHV(J)=AKHFAC*2d0*RHOWS*ABS(OMEGA)*COSVO(J)*(DSV**3)/RADIUS ! V-grid
...
C**** Add glacial melt from Antarctica and Greenland
        CALL GLMELT(CalendarSecondsPerDay)
     ...
     &     HAVE_NORTH_POLE = HAVE_NORTH_POLE  )
      ...
      Do J=max(J_0,2),min(J_1,JMO-1)
         Do I=2,IMO
            ...
            UMLx2 = 0.
            Do L=1,LMOU(I,J)
               UMLx2 = UMLx2 + MO(I,J,L)*(UO(I-1,J,L)+UO(I,J,L))
            EndDo
...
c     I=1 here
         ...
         UMLx2 = 0.
         Do L=1,LMOU(1,J)
            UMLx2 = UMLx2 + MO(1,J,L)*(UO(IMO,J,L)+UO(1,J,L))
         EndDo
...
!@auth Gary Russell/Gavin Schmidt
     ...
     *     ,ij_srwflxi,ij_srsflxi,ij_ervr,ij_mrvr,ij_ocnfr,IJ_dEPO_Geoth
#ifndef STANDALONE_OCEAN
      use diag_com, only : aij=>aij_loc, ij_geotherm
      USE MODEL_COM, only : ngrids
#endif
#ifdef TRACERS_OCEAN
      USE OCN_TRACER_COM, only : tracerlist, ocn_tracer_entry
      Use OCEAN, Only: TRMO
...
C**** set mass & energy fluxes (incl. river/sea ice runoff + basal flux)
        ...
        G0ML(:) =  G0M(I,J,:)                              !  J
!**** add geothermal heat flux to lowest layer
        G0ML(LMM(i,j)) = G0ML(LMM(i,j))
     &                 + DTs*fgeotherm(i,j)*dxypo(j)  
        GZML(:) = GZMO(I,J,:)
...
!**** Geothermal heat flux => bottom layer (upward +ve)
        OIJ(I,J,IJ_dEPO_Geoth)  = OIJ(I,J,IJ_dEPO_Geoth)  +
     *       fgeotherm(i,j)*Focean(i,j)    ! also under the ocean ice
#ifndef STANDALONE_OCEAN
        if(ngrids==1) aij(i,j,ij_geotherm) =
     *       aij(i,j,ij_geotherm) + fgeotherm(i,j)*Focean(i,j)
#endif
...
#ifdef ODIFF_FIXES_2022 /* for hemispheric symmetry */
     *           + 0.5*(TANP(J)*FVY(I,J-1) + TANP(J+1)*FVY(I,J)))
#else
...
C**** Need dv/dy,tv,dv/dx for u equation, du/dy,tu,du/dx for v equation
      ...
      FVY=0             ! flux in V equation at the y_+ boundary
      DO J=J_0,J_1S
#ifdef ODIFF_FIXES_2022 /* calculate quantities for I=1,IM instead of I=1,IM-1 */
        IM1=IM
#else
        IM1=IM-1
#endif
...
!@auth Gavin Schmidt
     ...
     *     ,OMASS
      ...
      do i=1,im
        if(focean(i,j).gt.0.) then
          osalt(i,j) = osalt(i,j)*oxyp(i,j)
          omass(i,j) = omass(i,j)*oxyp(i,j)
        else
          osalt(i,j) = 0.
          omass(i,j) = 0.
        endif
