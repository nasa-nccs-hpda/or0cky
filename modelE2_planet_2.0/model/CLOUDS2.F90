...
To-dos:
(1) The post-entrainment pre-subsidence negative-q fixup for tracers also.
#endif
...
!@+    CONVECTIVE_MICROPHYSICS,MC_PRECIP_PHASE,MASS_FLUX,PRECIP_MP
  ...
  use cld_aer_cdnc_mod, only: sntm
#ifdef TRACERS_AMP
...
!@var PLAND land fraction
  ...
  real*8 :: RTEMP,CMX,WMUIX,CONTCE1,CONTCE2,TNX,QNX

!@var RCLDIX, RCLDLX multipliers for ice particle/droplet radius
  real*8 :: RCLDIX, RCLDLX
...
!@dbparam radiusl_multiplier liquid cloud particle radius multiplier
!@dbparam radiusi_multiplier ice cloud particle radius multiplier
!@dbparam wmu_multiplier critical liquid cloud water multiplier
...
!@dbparam do_blU00 =1 if boundary layer U00 is treated differently
  ...
  real*8 :: radiusl_multiplier=1.d0  ! default
  real*8 :: radiusi_multiplier=1.d0  ! default
  real*8 :: wmu_multiplier=1.d0      ! default
...
!@var CNVMMRL convective cloud condensate mass mixing ratio
  real*8, dimension(LM) :: CNVMMRL
!@var CLDSAL Fractional stratiform cloud area used to derive in-cloud MMR
  real*8, dimension(LM) :: CLDSAL
#endif
...
!@var CONDMMR condensate mass mixing ratio for planet radiation scheme (kg/kg)
...
!@var fqaerfac for nucleation scavenging in updraft, per-distance -> per-layer
!@+   scaling factor
    real*8 WMXTR, precip_mm, fqaerfac
...
!@var MCDNCW,MCDNCI cloud droplet # for warm,cold moist conv clouds (cm^-3)
    ...
    FMC1=0.
    FSSL=1.
    RCLDLX=radiusl_multiplier  ! for liquid clouds
    RCLDIX=radiusi_multiplier  ! for ice clouds
...
CLOUD_TOP:  do L=LMIN+1,LM
            ...
            COND(L)=DQSUM
#ifdef USE_PLANET_RAD
!           Save condensate mass mixing ratio
...
#ifdef TRACERS_WATER
            do N=1,NTX
              TRCOND(N,L) = TRCOND(N,L) + TRCONDV(N,L-1)
            enddo
#endif
   ...
   (defined(TRACERS_AEROSOLS_Koch) || defined(TRACERS_AEROSOLS_SEASALT) || \
    ...
    defined(TRACERS_AEROSOLS_OCEAN) || defined(TRACERS_AEROSOLS_VBS))
      !**** DO and SELECT CASE loops continue through several tracer situations that include CLD_AER_CDNC & (Koch or SEASALT)
            ...
            call GET_SULFATE(PL(L),TPOLD(L),1d0,WA_VOL,WMXTR,SULFIN, &
                 SULFINOM,SULFINC,SULFOUT,TR_LEFT,TMP,TRCOND(:,L), &
                 mplume,LHX,DT_SULF_MC(:,L,1),0.d0,dt_get_sulf) ! cldsavt is 0 so finc can be 1

            do iaqch=1,aqchem_count
              n = aqchem_list(iaqch)
              TMP(N)=TMP(N)+SULFIN(iaqch)
              TMOMP(xymoms,N)= TMOMP(xymoms,N)*(1.+SULFINOM(iaqch))
              TRCOND(N,L) = TRCOND(N,L)+SULFOUT(iaqch)+SULFINC(iaqch)
#ifdef TRACERS_AMP 
            ...
            ENDDO
#endif
            fqaerfac = airm(l)/50d0 ! 100% application of fq_aer per 50 hPa
            call GET_COND_FACTOR( &
                 NTX,WMXTR,TPOLD(L),TPOLD(L-1),LHX,1d0 &
                 ,FQCOND,FQCONDT,.true.,fqaerfac,TRCOND(:,L),TM_dum,THLAW,TR_LEF,PL(L) &
                 ,ntix,1d0)
            ...
            if(ENT(L).gt.0.D0) then    ! non-zero entrainment
              ...
              if(eplume .gt. teeny) then
                ...
                fplume = min(mplume*byam(l),1d0) ! not really needed for anything
#else
...
EVAP_PRECIP: do L=LMAX-1,1,-1
          ...
          if (DQSUM.gt.0.) then       ! to avoid possibility of roundoff error
            ...
            WMXTR = PRCP*BYAM(L)/fevap

#if defined(TRACERS_AEROSOLS_Koch) || defined(TRACERS_AMP) || defined(TRACERS_TOMAS)
            if (prcp > teeny) then
              !**** Sulfate chemistry in the cloud environment (and below cloud base)
              ...
              TM_dum(:) = TM(L,:)*fevap
              call GET_SULFATE(PL(L),TOLD,1d0,WA_VOL,WMXTR,SULFIN,SULFINOM, &
                   SULFINC,SULFOUT,TR_LEFT,TM_dum,TRPRCP,AIRM(L)*FEVAP,LHX, &
                   DT_SULF_MC(:,L,2),0d0,dt_get_sulf) ! cldsavt is 0 so finc can be 1

              do iaqch=1,aqchem_count
                n = aqchem_list(iaqch)
                TM(L,N)=TM(L,N)+SULFIN(iaqch)
                TMOM(xymoms,L,N)=TMOM(xymoms,L,N)*(1.+FEVAP*SULFINOM(iaqch))
                TRPRCP(N)=TRPRCP(N)+SULFINC(iaqch)
                ! why is sulfout temporarily added to trcond, instead of to trprcp?
            ...
            if (prcp > teeny) then
              ! Washout in the cloud environment (and below cloud base)
              TM_dum(:) = TM(L,:)*fevap
              call GET_WASH_FACTOR(NTX,fevap,precip_mm/fevap,FWASHT, &
                   TOLD,LHX,wmxtr,1d0,TM_dum,TRPRCP,THWASH,pl(l),ntix,.true. &
#ifdef TRACERS_TOMAS
                   ,i_debug,j_debug,L &
#endif
                   )
              dtr(1:ntx) = fwasht(1:ntx)*tm(L,1:ntx)
...
OPTICAL_THICKNESS: do L=1,LMCMAX
      ...
      if(SVWMXL(L).gt.0.) then
        ...
        call ANVIL_OPTICAL_THICKNESS(SVLATL(L),RCLDLX,RCLDIX,MCDNCW,MCDNCI, &
             RIMAX,BYBR,FCLD,TEM,WTEM,RCLD,TAUMCL(L))
...
#ifdef USE_PLANET_RAD
!   Save condensate mixing ratios from CONDMMR to arrays seen by CLOUDS2_DRV
    DO L=1,LM
      IF (CLDMCL(L) > 0.0D+0) THEN
!       Non-zero cloud cover, set convective cloud mixing ratio
        CNVMMRL(L) = CONDMMR(L)
      ELSE
!       Zero cloud cover, zero condensate amount
        CNVMMRL(L) = 0.0D+0
      END IF
    END DO
#endif
...
!         WTEM=1d5*WMX(L)*PL(L)/(FCLD*TL(L)*RGAS+teeny)
          ...
          if(LHX.eq.LHE)  then
            ...
            RCLD=RCLDLX*1d-6*100.d0*(WTEM/(2.d0*BY3*TWOPI*SCDNCW))**BY3
#ifdef AIE_DIAG_FIX_MET
            ...
            RCLD=RCLDIX*100.d-6*(WTEM/(2.d0*BY3*TWOPI*SCDNCI))**BY3
            RCLD=min(RCLD,RIMAX)
#ifdef AIE_DIAG_FIX_MET
...
!yunha          FWTOQ=min(1d0,FWTOQ)
    ...
    (defined TRACERS_TOMAS)
      ...
      call GET_SULFATE(PL(L),TL(L),FCLD,WA_VOL,WMXTR,SULFIN,SULFINOM &
           ...
           ,LHX_WA,DT_SULF_SS(:,L,2),CLDSAVT,dtsrc)
      ...
      if(BELOW_CLOUD.and.QCX.lt.teeny) then
       ...
       ,i_debug,j_debug,L &
        ...
        call GET_COND_FACTOR( &
             ...
             ,FQTOWT,.false.,1d0,TRWML(:,L),TM_dum,THLAW,TR_LEF,PL(L) &
             ,ntix,CLDSAVT)
#ifdef TRACERS_AEROSOLS_OCEAN
        if (trm(i_debug,j_debug,L,n_ococean) .gt. 0.d0) then
          do n=1,ntx
...
!         WMX(L)=WMX(L)+DQSUM*FSSL(L)
    ...
    (defined TRACERS_TOMAS)
          ...
          call GET_COND_FACTOR(NTX,WMXTR,TL(L),TL(L),LHX,FCLD,FCOND &
               ,FQCONDT,.false.,1d0,TRWML(:,L),TM_dum,THLAW,TR_LEF,pl(l) &
               ,ntix,CLDSAVT)
#ifdef TRACERS_AEROSOLS_OCEAN
          if (trm(i_debug,j_debug,L,n_ococean) .gt. 0.d0) then
            do n=1,ntx
...
!     WTEM=1.d5*WMX(L)*PL(L)/(FCLD*TL(L)*RGAS+teeny)
      ...
      if(LHX.eq.LHE) then
        ...
        RCLD=RCLDLX*100.d0*(WTEM/(2.d0*BY3*TWOPI*SCDNCW))**BY3
        ...
        if(use_vmp .and. lhp(l).eq.lhs .and. wmpr(l).gt.0.) then
          ...
          RCLDE1 =RCLDIX*100.d0*(RCLDE1/(2.d0*BY3*TWOPI*SCDNCI))**BY3
...
!@auth Menon    end of addition  comment out the RCLDE definition below
        ...
        RCLD=RCLDIX*100.d0*(WTEM/(2.d0*BY3*TWOPI*SCDNCI))**BY3
        RCLD=min(RCLD,RIMAX)
        RCLDE=RCLD/BYBR
        RCLDE1 = RCLDE
...
!     if(WMX(L).le.0.) SVLHXL(L)=0.
  ...
  subroutine ANVIL_OPTICAL_THICKNESS(SVLATL,                         & ! input
       RCLDLX,RCLDIX,MCDNCW,MCDNCI,RIMAX,BYBR,FCLD,TEM,WTEM,         & ! input
       RCLD,TAUMC)                                                      ! output
...
!@var RCLDLX,RCLDIX RCLD multiplier
    real*8, intent(IN) :: RCLDLX,RCLDIX  ! RCLDX
...
!@var FCLD,TEM,WTEM cloud cover and dummy variables
    ...
    if(SVLATL.eq.LHE)  then
      RCLD=RCLDLX*100.d0*(WTEM/(2.d0*BY3*TWOPI*MCDNCW))**BY3
    else
      RCLD=RCLDIX*100.d0*(WTEM/(2.d0*BY3*TWOPI*MCDNCI))**BY3
      RCLD=min(RCLD,RIMAX)
    end if
    RCLDE=RCLD/BYBR       !  effective droplet radius in anvil
...
#ifndef NO_CLD_AER_CDNC_IN_MCMICRO
#ifdef CLD_AER_CDNC
