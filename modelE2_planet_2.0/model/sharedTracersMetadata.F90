...
!@auth NCCS ASTG
  ...
  use OldTracer_mod, only: set_KH_298,set_deltaH_R
  use OldTracer_mod, only: set_K1_298,set_deltaH1_R
  use OldTracer_mod, only: set_K2_298,set_deltaH2_R
...
!------------------------------------------------------------------------------
  ...
  subroutine DMS_setSpec(name)
    ...
    if (tracers_drydep) then
       call set_KH_298(n, 5.4d-1)
       call set_deltaH_R(n,3460.d0)
    endif 
  ...
  subroutine SO2_setSpec(name)
    ...
    if (tracers_drydep) then
       call set_KH_298(n, 1.36d0)
       call set_deltaH_R(n,3100.d0)
       call set_K1_298(n,1.30d-2)
       call set_deltaH1_R(n,1960.d0)
       call set_K2_298(n,6.6d-8)
       call set_deltaH2_R(n,1500.d0)
    endif 
    call set_has_chemistry(n, .true.)
  ...
  subroutine H2O2_setSpec(name)
    ...
    if (tracers_drydep) then
       call set_KH_298(n, 8.7d4)
       call set_deltaH_R(n,7320.d0)
       call set_K1_298(n,2.2d-12)
       call set_deltaH1_R(n,-3730.d0)
       call set_F0(n,  1.d0)
    endif
    call set_has_chemistry(n, .true.)
...
!    call set_tr_RKD(n, 0.7303d0   ) !tr_RKD=74 M/atm
    ...
    call set_tr_wd_type(n, ngas)
    if (tracers_drydep) then 
       call set_KH_298(n, 6.02d1)
       call set_deltaH_R(n, 4160.d0)
       call set_K1_298(n,1.7d-5)
       call set_deltaH1_R(n,-4325.d0)
       call set_K2_298(n,1.0d-14)
       call set_deltaH2_R(n,-6716.d0)
    endif
    call set_has_chemistry(n, .true.)
