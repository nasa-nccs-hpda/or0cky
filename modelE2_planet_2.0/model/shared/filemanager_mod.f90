...
!@ver  2.0
  ...
  public openunit, closeunit, print_open_units, findunit, nf
  public nameunit
...
! Wrapper routine for calls to netCDF functions with error handling.
  SUBROUTINE nf(status)
    ...
    IF (status /= NF_NOERR) THEN
      CALL stop_model('netCDF-ERROR: ' // nf_strerror(status), 255)
    END IF
