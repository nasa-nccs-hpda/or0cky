...
! Public entities
  ...
  ! modulo(q,r) returns   q - r * floor(q/r)
  public :: modulo
  ! fraction(q) returns   q - floor(q)
  public :: fraction
...
!---------------------------------------------------------------------------
  ...
  contains
    ...
    generic :: operator(/=) => not_equals_fraction
    ...
    procedure :: not_equals_fraction   ! r /= p
...
contains
  ...
  function getFraction(q) result(fraction)
    type (Rational) :: fraction
    class (Rational), intent(in) :: q

    fraction = modulo(q, Rational(1))

 end function getFraction
...
#ifdef COMPILER_ifx
    c = a / newRational_real_dp(x, x*1.d-10)
#else
    c = a / Rational(x, 1.d-6)
#endif
...
! Divide fractions by an integer and reduce
  ...
  logical function not_equals_fraction(a, b) result(not_equals)
    class (Rational), intent(in) :: a
    class (Rational), intent(in) :: b

    not_equals = .not. (a == b)

  end function not_equals_fraction
...
! Citation - Euclid
  ...
  function abs_rational(this)
    type (Rational) :: abs_rational
    class (Rational), intent(in) :: this

    if (this%whole < 0) then
       abs_rational%whole = -this%whole
       abs_rational%numerator = -this%numerator
       abs_rational%denominator = this%denominator
    else
       abs_rational%whole = this%whole
       abs_rational%numerator = this%numerator
       abs_rational%denominator = this%denominator
