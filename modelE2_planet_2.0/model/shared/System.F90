...
subroutine sys_alarm (timesec, prog)
#if defined(COMPILER_Intel8)
  use IFPORT
#endif
  implicit none
  integer, intent(in) :: timesec ! delay in seconds
  external :: prog ! subroutine to execute
  integer :: status
  
#if defined(COMPILER_Intel8)
  status = alarm (timesec, prog)
#elif defined(COMPILER_G95)
  call alarm(timesec, prog, status)
#else
  write(6,*) "ALARM not supported for surrent architecture. Will ignore"
#endif
  
end subroutine sys_alarm
