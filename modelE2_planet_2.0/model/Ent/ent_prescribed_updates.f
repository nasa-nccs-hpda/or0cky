...
!@sum Routines for updating prescribed vegetation. 
...
!@sum Given new LAI, update cohort biomass pools, litter, senescefrac.
      ...
      real*8 :: C_lab_old, Clossacc_old, dC, dC_tol
      real*8 :: dC_fol,dC_froot,dC_hw,dC_croot,dC_sw
      real*8 :: dC_lab, dClossacc
...
!     &       cop%C_fol,cop%C_froot,cop%C_hw,cop%C_sw,cop%C_croot
      ...
      dC_lab = cop%C_lab-C_lab_old
      dClossacc = sum(Clossacc(CARBON,SURFMET:CWD,1))-Clossacc_old

      dC = dC_fol + dC_sw + dC_hw + dC_froot + dC_croot + dC_lab +
     &     dClossacc/cop%n

      dC_tol = (sum(Clossacc(CARBON,SURFMET:CWD,1))/cop%n
     &         + abs(cop%C_lab-C_lab_old))*1d-11 + 1d-12

      if( abs(dC) > dC_tol ) then
        write(99,*) "litter", dC, dC_sw, dC_hw, dC_froot,
     &       dC_croot, dC_lab, dClossacc/cop%n, cop%n, cop%pft 
      endif
