...
!@+   level.
      ...
      public entcell_extract_canopy_cond
...
!*********************************************************************
      subroutine entcell_extract_canopy_cond(ecp, canopy_conductance)
!@sum return canopy_conductance per pft
      type(entcelltype) :: ecp
      real*8 :: canopy_conductance(:)
      !---
      type(patch), pointer :: pp
      type(cohort),pointer :: cop
      real*8 :: area(size(canopy_conductance))

      canopy_conductance(:) = 0.d0
      area(:) = 0.d0
      pp => ecp%oldest
      do while( associated(pp) )
        cop => pp%tallest
        if( associated(cop) ) then
          canopy_conductance(cop%pft + COVEROFFSET) = 
     &         canopy_conductance(cop%pft + COVEROFFSET)
     &         + cop%GCANOPY*pp%area
          area(cop%pft + COVEROFFSET) = area(cop%pft + COVEROFFSET)
     &         + pp%area
      ...
      where( area(:) > 0.d0 ) canopy_conductance(:) = 
     &                    canopy_conductance(:)/area(:)

      end subroutine entcell_extract_canopy_cond
!*********************************************************************
