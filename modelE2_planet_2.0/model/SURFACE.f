...
!@auth Nobody will claim responsibilty
     ...
     *     n_Be7, n_Be10
...
#endif
#ifdef TRACERS_WATER
      use OldTracer_mod, only: nWATER,trname
#endif
#endif
     ...
     &     ,ij_frac_for,ij_frac_cro,ij_frac_gra
     &     ,ij_frac_shr,ij_frac_bar
#endif
#endif
     ...
     *      tij_drydep,tij_vd,
#ifdef TRACERS_DRYDEP_DIAG
     *      tij_vd_for,tij_vd_cro,
     *      tij_vd_gra, tij_vd_bar, tij_vd_shr,
     *      tij_egs_for, tij_egs_cro, tij_egs_gra,
     *      tij_egs_shr,tij_egstom1,tij_egcut_dry1,
     *      tij_egcut_wet1,tij_egcut_snow1,
     *      tij_eggro_dry1,tij_eggro_wet1,
     *      tij_eggro_snow1,
#endif
     *      itcon_dd
#endif
...
#endif
#ifdef TRACERS_DRYDEP
      real*8 tdryd, tdd, td1, rtsdt, rts, vd
#endif
#endif
C****
...
C**** final fluxes
      ...
      EKT(I,J) = EKT(I,J) + (DMUA_IJ*US + DMVA_IJ*VS)*DTSURF
...
#ifdef TRACERS_DRYDEP_DIAG
! archive fraction per LUT (no multiplication by ptype)
          aij(i,j,ij_frac_for)=aij(i,j,ij_frac_for) +
     &         (asflx(ipatch)%frac_for(i,j))*byNIsurf
          aij(i,j,ij_frac_cro)=aij(i,j,ij_frac_cro) +
     &         (asflx(ipatch)%frac_cro(i,j))*byNIsurf
          aij(i,j,ij_frac_gra)=aij(i,j,ij_frac_gra) +
     &         (asflx(ipatch)%frac_gra(i,j))*byNIsurf
          aij(i,j,ij_frac_shr)=aij(i,j,ij_frac_shr) +
     &         (asflx(ipatch)%frac_shr(i,j))*byNIsurf
          aij(i,j,ij_frac_bar)=aij(i,j,ij_frac_bar) +
     &         (asflx(ipatch)%frac_bar(i,j))*byNIsurf
#endif
        do nx=1,ntx
           n=ntix(nx)
        if(dodrydep(n))then
          vd = asflx(ipatch)%vd1(n,i,j)           ! m/s
          tdryd = asflx(ipatch)%drydflx(n,i,j)           ! kg/m2; negative
          rtsdt = -tdryd/(vd+teeny)                  ! kg s/m3; positive
          rts = rtsdt/dtsurf                             ! kg/m^3
          tdd = tdryd                                    ! kg/m2
          td1 = (asflx(ipatch)%trsrfflx(n,i,j)
     &          +asflx(ipatch)%trflux_prescr(n,i,j)
     &         )*dtsurf         ! kg/m2
          ! can't deposit more mass than we have in near-surface air
...
! archive dry dep related variables for taijn
! archive flux
          taijn(i,j,tij_drydep,n)=taijn(i,j,tij_drydep,n) +
     &         ptype*rtsdt*vd
! archive deposition velocity
          taijn(i,j,tij_vd,n)=taijn(i,j,tij_vd,n) +
     &         ptype*vd*byNIsurf
#ifdef TRACERS_DRYDEP_DIAG
! archive deposition velocity per LUT (no multiplication by ptype)
          taijn(i,j,tij_vd_for,n)=taijn(i,j,tij_vd_for,n) +
     &         (asflx(ipatch)%vd_for(n,i,j))*byNIsurf
          taijn(i,j,tij_vd_cro,n)=taijn(i,j,tij_vd_cro,n) +
     &         (asflx(ipatch)%vd_cro(n,i,j))*byNIsurf
          taijn(i,j,tij_vd_gra,n)=taijn(i,j,tij_vd_gra,n) +
     &         (asflx(ipatch)%vd_gra(n,i,j))*byNIsurf
          taijn(i,j,tij_vd_shr,n)=taijn(i,j,tij_vd_shr,n) +
     &         (asflx(ipatch)%vd_shr(n,i,j))*byNIsurf
          taijn(i,j,tij_vd_bar,n)=taijn(i,j,tij_vd_bar,n) +
     &         (asflx(ipatch)%vd_bar(n,i,j))*byNIsurf
! archive effective conductances for different pathways for GASES ONLY
          IF (tr_wd_TYPE(n) /= nPART) THEN
           taijn(i,j,tij_egstom1,n)=taijn(i,j,tij_egstom1,n) +
     &         ptype*(asflx(ipatch)%egs1(n,i,j))*byNIsurf
           taijn(i,j,tij_egcut_dry1,n)=taijn(i,j,tij_egcut_dry1,n) +
     &         ptype*(asflx(ipatch)%egcut_dry1(n,i,j))*byNIsurf
           taijn(i,j,tij_eggro_dry1,n)=taijn(i,j,tij_eggro_dry1,n) +
     &         ptype*(asflx(ipatch)%eggro_dry1(n,i,j))*byNIsurf
           taijn(i,j,tij_egcut_wet1,n)=taijn(i,j,tij_egcut_wet1,n) +
     &         ptype*(asflx(ipatch)%egcut_wet1(n,i,j))*byNIsurf
           taijn(i,j,tij_eggro_wet1,n)=taijn(i,j,tij_eggro_wet1,n) +
     &         ptype*(asflx(ipatch)%eggro_wet1(n,i,j))*byNIsurf
           taijn(i,j,tij_egcut_snow1,n)=taijn(i,j,tij_egcut_snow1,n) +
     &         ptype*(asflx(ipatch)%egcut_snow1(n,i,j))*byNIsurf
           taijn(i,j,tij_eggro_snow1,n)=taijn(i,j,tij_eggro_snow1,n) +
     &         ptype*(asflx(ipatch)%eggro_snow1(n,i,j))*byNIsurf
! archive effective stomatal conductance per LUT 
           taijn(i,j,tij_egs_for,n)=taijn(i,j,tij_egs_for,n) +
     &         (asflx(ipatch)%egs_for(n,i,j))*byNIsurf
           taijn(i,j,tij_egs_cro,n)=taijn(i,j,tij_egs_cro,n) +
     &         (asflx(ipatch)%egs_cro(n,i,j))*byNIsurf
           taijn(i,j,tij_egs_gra,n)=taijn(i,j,tij_egs_gra,n) +
     &         (asflx(ipatch)%egs_gra(n,i,j))*byNIsurf
           taijn(i,j,tij_egs_shr,n)=taijn(i,j,tij_egs_shr,n) +
     &         (asflx(ipatch)%egs_shr(n,i,j))*byNIsurf
          ENDIF
#endif
     ...
     *         *vd
     ...
     &     ptype*rtsdt*vd,itcon_dd(n,1),n)

c**** for subdaily diagnostics, archive in global arrays
          depo_flux_glob(i,j,n) = depo_flux_glob(i,j,n)
     &         + ptype*rts*vd*byNIsurf ! kg/m2
#ifdef TRACERS_DRYDEP_DIAG_SUBDD
! archive instantaneous deposition velocities & effective conductances 
! want both half hours included in the average 
! for non-LUT specific, want ptype 
          egcut_wet_glob(i,j,n) = egcut_wet_glob(i,j,n)
     &         +  ptype*(asflx(ipatch)%egcut_wet1(n,i,j))*byNIsurf ! m/s
! for LUT specific, do not want ptype
          vd_glob_for(i,j,n) = vd_glob_for(i,j,n)
     &         +  asflx(ipatch)%vd_for(n,i,j)*byNIsurf ! m/s
          vd_glob_cro(i,j,n) = vd_glob_cro(i,j,n)
     &         +  asflx(ipatch)%vd_cro(n,i,j)*byNIsurf ! m/s
          vd_glob_gra(i,j,n) = vd_glob_gra(i,j,n)
     &         +  asflx(ipatch)%vd_gra(n,i,j)*byNIsurf ! m/s
          vd_glob_shr(i,j,n) = vd_glob_shr(i,j,n)
     &         +  asflx(ipatch)%vd_shr(n,i,j)*byNIsurf ! m/s  
          egs_glob_for(i,j,n) = egs_glob_for(i,j,n)
     &         +  asflx(ipatch)%egs_for(n,i,j)*byNIsurf ! m/s
          egs_glob_cro(i,j,n) = egs_glob_cro(i,j,n)
     &         +  asflx(ipatch)%egs_cro(n,i,j)*byNIsurf ! m/s
          egs_glob_gra(i,j,n) = egs_glob_gra(i,j,n)  
     &         +  asflx(ipatch)%egs_gra(n,i,j)*byNIsurf ! m/s
          egs_glob_shr(i,j,n) = egs_glob_shr(i,j,n) 
     &         +  asflx(ipatch)%egs_shr(n,i,j)*byNIsurf ! m/s
#endif 
      endif ! tracer dry dep
      enddo ! tracer
      enddo ! ipatch
      enddo ! i
      enddo ! j
...
C
      case ('qs')
        ...
        call inc_subdd(subdd,k,sddarr2d)
!
!***** pw calc is here to include q from evap.
!
      case ('pw')
        do j=j_0,j_1; do i=i_0,imaxj(j)
          sddarr2d(i,j) = sum(q(i,j,:)*MA(:,i,j))
        enddo;        enddo
        call inc_subdd(subdd,k,sddarr2d)
C
      case ('ustar')
...
C****
       ...
       case ('N_SSA_1')
          ...
          trc_flux=pbl_args%ss1_flux*mass_to_number*1.d9 
        if (ijts_ampp(3,n)>0)
