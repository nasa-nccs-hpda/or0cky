#include "rundeck_opts.h"
      ...
      SUBROUTINE masterchem_prep
!@sum masterchem main chemistry routine
!@auth Drew Shindell (modelEifications by Greg Faluvegi)
!@calls Crates,Oxfam,HOxfam,NOxfam,chemstep
C
...
c
     ...
     &                 bygrav,lhe,undef,teeny,byavog,loschmidt_constant
      ...
      USE RAD_COM, only     : COSZ1,alb,SRDN
...
! calculate Isoprene zonal mean, to be used for acetone
     ...
     &         byMA(1,i,j)
      ...
      PRES2(1:LM) = PMIDL00(1:LM) ! duplicate in masterchem: fix that
...
!!    use precision_mod, only : reduce_precision 
     ...
     &     ,ijlt_COp,ijlt_COd,ijlt_NOxd,ijlt_Oxd,ijlt_Oxp,ijlt_pO1D
     ...
     &     ,jls_ClOcon,jls_H2Ocon,ijlt_JacetA,ijlt_JacetB
...
#endif  /* TRACERS_dCO */
      use photolysis, only: fastj2_drv,o3_fastj,so2_fastj,get_sza
     &                     ,szamax,zj,jpnl,sf3_fact,sf2_fact
#ifdef CACHED_SUBDD
      use trchem_shindell_com, only : mrno,mrno2,mro3,OH_conc,HO2_conc,
     &                                JO1D_rate,JNO2_rate
#endif
#ifdef TRACERS_AEROSOLS_VBS
      use aerosol_sources, only: vbs_sets
...
!@var ratioN variables for nitrogen conservation
!@var L,N,igas,inss,LL,L2,n2 dummy loop variables
...
!@var bysumOx reciprocal of sum of regional Ox tracers
     ...
     &  FASTJ_PFACT,bydtsrc,CH4FACT,r179,
     &  ratioN,pfactor,bypfactor,gwprodHNO3,
#ifdef TRACERS_dCO
...
#endif  /* TRACERS_dCO */
     &  gwprodN2O5,wprod_sulf,wprodCO,dNO3,wprodHCHO,
     ...
     &  changeHOCl,changeHCl,changehetClONO2,chgHT3,sza,
     &  chgHT4,chgHT5,rmrClOx,rmrBrOx,rmv,rmrOx,
     &  bHNO3,mHNO3,HNO3_thresh,Ttemp,changeBrOx,changeBrONO2,
     ...
     & index2
      ...
      real*8 :: AnthroCl,AnthroBr,ch3cl_pppv,ch3br_pppv
...
!@var ilon72 lon index relative to the rad code 72x46 grid
      ...
      integer :: latIndex, pressureIndex1, pressureIndex2
      real*8 :: coeff1, coeff2, lndist
#endif
#ifdef TRACERS_AEROSOLS_VBS
      integer :: v
#endif  /* TRACERS_AEROSOLS_VBS */
...
#ifdef EESC_BASED_CLTOT_BRTOT
      ! Components are time- but not space-dependant.
      ! Timestreams updated in daily tracer routine. Convert them here
      ! from ppbv input to part-per-part(ppp):
      AnthroCl = 1.d-9*(ccl4+f114+hcfc141b+hcfc22+f113+ch3ccl3+f12+f11)
      AnthroBr = 1.d-9*(halon2402+halon1301+halon1211)
      ch3cl_pppv = 1.d-9*ch3cl
      ch3br_pppv = 1.d-9*ch3br
#endif /* EESC_BASED_CLTOT_BRTOT */
...
C**** Add water to relevant tracers as well
             do n=1,ntm
               ...
               case (nWater)       ! water: initialise tracers
                 trm_col(L,n) = trm_col(L,n)*fraQ
                 if(fraQ < 1.)trmom_col(:,L,n) = trmom_col(:,L,n)*fraQ
               end select
             end do
#endif
           end if
...
c - set reactive species for use in family chemistry & nighttime NO2:
       ...
       y(nNO3,L)   =pNO3(L,I,J)*y(nn_NOx,L)
       y(nNO2,L)     =y(nn_NOx,L)*pNOx(L,I,J)
       y(nNO,L)      =y(nn_NOx,L)-(y(nNO2,L)+y(nNO3,L))
...
#ifdef SOLAR_ENERGETIC_PARTICLES
       ! Lookup daily ion pair production rate for closest
       ! magnetic latitude and for log-linear interpolation
       ! between two nearest pressure levels. Convert from pair g-1 s-1
       ! (iprpToday) to pair cm-3 s-1 (IonProdRate variable):
       latIndex=minloc(abs(ionGlat-qdlat3D(L,i,j)),1)
       pressureIndex1=minloc(abs(ionPlev-pl(L)),1)
       ! Note that pressure increases with increasing index in ionPlev
       ! array, which is not the case for model pl() array:
       if( pl(L)==ionPlev(pressureIndex1) ) then
         coeff1=0.5d0 ; coeff2=0.5d0 ; pressureIndex2=pressureIndex1
       else if(pl(L) < ionPlev(pressureIndex1)) then
         if( pressureIndex1 == 1 ) then
           coeff1=0.5d0 ; coeff2=0.5d0 ; pressureIndex2=pressureIndex1
         else
           pressureIndex2=pressureIndex1-1
           lndist=LNionPlev(pressureIndex1)-LNionPlev(pressureIndex2)
           coeff1=(LOG(pl(L))-LNionPlev(pressureIndex2))/lndist
           coeff2=1.d0-coeff1
         end if
       else ! pl(L) > ionPlev(pressureIndex1)
         if( pressureIndex1 == size(ionPlev) ) then
           coeff1=0.5d0 ; coeff2=0.5d0 ; pressureIndex2=pressureIndex1
         else
           pressureIndex2=pressureIndex1+1
           lndist=LNionPlev(pressureIndex2)-LNionPlev(pressureIndex1)
           coeff2=(LOG(pl(L))-LNionPlev(pressureIndex1))/lndist
           coeff1=1.d0-coeff2
         end if
       end if
       IonProdRate(L)=(coeff1*iprpToday(latIndex,pressureIndex1) +
     &                 coeff2*iprpToday(latIndex,pressureIndex2))
     &                * y(nM,L) * byavog * mair
...
c Pass O3 & SO2 arrays (in ppmv; here seems to be ppv) to fastj. Above these
C levels fastj2 uses Nagatani climatological O3, read in by chem_init: 
        DO L=1,topLevelOfChemistry
          O3_FASTJ(L)=y(nO3,L)/y(nM,L)
          SO2_FASTJ(L)=yso2(L)/y(nM,L)
        END DO

! calculate photolysis rates
        call fastj2_drv(I,J, tl(1:topLevelOfChemistry), rh)
#ifndef TRACERS_ACETONE /* NOT */
...
! Apply some alterations to the fastj photolysis rates just calculated: 
     ...
     &      +zj(L,rj%H2O2__OH_OH)
          ...
          taijls(i,j,L,ijlt_JacetA)=taijls(i,j,L,ijlt_JacetA)
     &      +zj(L,rj%Acetone__CH3O2_C2O3)
          taijls(i,j,L,ijlt_JacetB)=taijls(i,j,L,ijlt_JacetB)
     &      +zj(L,rj%Acetone__CH3O2_CH3O2)
#endif
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      ! Check for PSC's existence. 
      ! Note that in Crates the pressure bounds are 245 and 5, not 250 and 3.
      pscX(:)=.false.
      do L=1,topLevelOfChemistry
...
! calculate voc2nox for SOA precursor chemistry
     ...
     &                 rr(rrbi%XO2_XO2__M_M,L)*yXO2(L,I,J))
...
CCCCCCCCCCCCCCCC NIGHTTIME CCCCCCCCCCCCCCCCCCCCCC
     ...
     &              (I,J)
      ...
      DO L=1,topLevelOfChemistry
        ...
        call updateSulfate(i,j,L,rh(L),bythick(L),sulfate,rgammasulf)

        pfactor=ma(L)/y(nM,L)
        bypfactor=1.D0/pfactor
        RVELN2O5=mean_molecular_speed(tl(l),tr_mm(n_N2O5)*1.d-3)*100.d0 ! cm/s
C       Calculate sulfate sink, and cap it at 20% of N2O5:
c       in troposphere loss is rxn on sulfate, in strat rxn w PSC or sulfate
        wprod_sulf=
     &  dt2*sulfate*y(nn_N2O5,L)*RGAMMASULF*RVELN2O5*0.25d0
...
c       calculate NO3 vs NO2 (assume no NO at night)
     ...
     &      *pOx(L,I,J)
          rlossNO3=(2.d0*rr(rrbi%NO3_NO3__NO2_NO2,L)*y(nNO3,L))
     &        *y(nNO3,L)
     ...
     &    call stop_model('(rlossNO3+rprodNO3)=0',255)
          ...
          y(nNO3,L)=pNO3temp*y(nn_NOx,L)
          y(nNO2,L)=y(nn_NOx,L)-y(nNO3,L)
        end do
...
c       paths if lead to negative conc:
     ...
     &      *y(nNO3,L)*dt2
#endif  /* TRACERS_dCO */
        rAldplusNO3=2.5d-15*yAldehyde(L,I,J)*y(nNO3,L)*dt2
#ifdef TRACERS_dCO
        rd17OaldplusNO3=2.5d-15*yd17Oald(L,I,J)*y(nNO3,L)*dt2
        rd18OaldplusNO3=2.5d-15*yd18Oald(L,I,J)*y(nNO3,L)*dt2
        rd13CaldplusNO3=2.5d-15*yd13Cald(L,I,J)*y(nNO3,L)*dt2
#endif  /* TRACERS_dCO */
        rIsopplusNO3=rr(rrbi%Isoprene_NO3__HO2_Alkenes,L)
     &    *y(nn_Isoprene,L)*y(nNO3,L)*dt2
#ifdef TRACERS_TERP
        rTerpplusNO3=
     ...
     &    y(nClO,L)*rr(rrtri%ClO_NO2__ClONO2_M,L)*y(nNO2,L)*dt2
        rDMSplusNO3=ydms(L)*rsulf3(L)*y(nNO3,L)*dt2
...
C Alkenes, Isoprene, Terpenes (if used) and AlkylNit:
     ...
     &        *0.12d0
#endif  /* TRACERS_TERP */
     &      -2.5d-15*yAldehyde(L,I,J)
     &    )*y(nNO3,L)*dt2
        if(-changeAldehyde > 0.75d0*yAldehyde(L,I,J))changeAldehyde=
     &  -0.75d0*yAldehyde(L,I,J)
     ...
     &        *0.12d0
#endif  /* TRACERS_TERP */
     &      -2.5d-15*yd17Oald(L,I,J)
     &    )*y(nNO3,L)*dt2
        if(-changed17Oald > 0.75d0*yd17Oald(L,I,J))changed17Oald=
     &  -0.75d0*yd17Oald(L,I,J)
     ...
     &        *0.12d0
#endif  /* TRACERS_TERP */
     &      -2.5d-15*yd18Oald(L,I,J)
     &    )*y(nNO3,L)*dt2
        if(-changed18Oald > 0.75d0*yd18Oald(L,I,J))changed18Oald=
     &  -0.75d0*yd18Oald(L,I,J)
     ...
     &        *0.12d0
#endif  /* TRACERS_TERP */
     &      -2.5d-15*yd13Cald(L,I,J)
     &    )*y(nNO3,L)*dt2
        if(-changed13Cald > 0.75d0*yd13Cald(L,I,J))changed13Cald=
     &  -0.75d0*yd13Cald(L,I,J)
#endif  /* TRACERS_dCO */
...
! This should be improved in the future.
     ...
     &    -0.75d0*y(nn_apinp2g,L)
        ...
        changeIsoprene=
     &    -(rr(rrbi%Isoprene_NO3__HO2_Alkenes,L)*y(nNO3,L)
     &    +rr(rrbi%Isoprene_O3__HCHO_Alkenes,L)*y(nO3,L))
     &      *y(nn_Isoprene,L)*dt2
...
#ifdef TRACERS_TERP
        changeTerpenes=-(
     &      rr(rrbi%Terpenes_NO3__HO2_Alkenes,L)*y(nNO3,L)
     &      +rr(rrbi%Terpenes_O3__HCHO_Alkenes,L)*y(nO3,L)
     &    )*y(nn_Terpenes,L)*dt2
...
C       Include reactions on dust for HNO3:
        changeHNO3 = changeHNO3 - krate(L,1,1)*y(nn_HNO3,L)*dt2
        changeN_d1 = krate(L,2,1) * y(nn_HNO3,L) *dt2
        changeN_d2 = krate(L,3,1) * y(nn_HNO3,L) *dt2
        changeN_d3 = krate(L,4,1) * y(nn_HNO3,L) *dt2
#endif
...
C Apply Alkenes, AlkyNit, and Aldehyde changes here:
        ...
        yAldehyde(L,I,J)=yAldehyde(L,I,J)+changeAldehyde
#ifdef TRACERS_dCO
        yd17Oald(L,I,J)=yd17Oald(L,I,J)+changed17Oald
        yd18Oald(L,I,J)=yd18Oald(L,I,J)+changed18Oald
        yd13Cald(L,I,J)=yd13Cald(L,I,J)+changed13Cald
#endif  /* TRACERS_dCO */
...
c       Gas phase NO3 + HCHO -> HNO3 + CO yield of HCHO & CO:
        ...
        if(-changeL(L,n_HCHO) > trm_col(L,n_HCHO))then
          changeL(L,n_HCHO)=-.95d0*trm_col(L,n_HCHO)
          changeHCHO=changeL(L,n_HCHO)*mass2vol(n_HCHO)*bypfactor
        endif
        IF((trm_col(l,n_HCHO)+changeL(l,n_HCHO)) < minKG) THEN
          changeL(l,n_HCHO) = minKG - trm_col(l,n_HCHO)
          changeHCHO=changeL(L,n_HCHO)*mass2vol(n_HCHO)*bypfactor
        ENDIF
        wprodHCHO=changeHCHO
#ifdef TRACERS_dCO
        changeL(L,n_dHCH17O)=changedHCH17O*pfactor*vol2mass(n_dHCH17O)
        if(-changeL(L,n_dHCH17O) > trm_col(L,n_dHCH17O))then
          changeL(L,n_dHCH17O)=-.95d0*trm_col(L,n_dHCH17O)
          changedHCH17O=changeL(L,n_dHCH17O)*mass2vol(n_dHCH17O)*
     &                  bypfactor
        endif
        IF((trm_col(l,n_dHCH17O)+changeL(l,n_dHCH17O)) < minKG) THEN
          changeL(l,n_dHCH17O) = minKG - trm_col(l,n_dHCH17O)
          changedHCH17O=changeL(L,n_dHCH17O)*mass2vol(n_dHCH17O)*
     &                  bypfactor
        ENDIF
        changeL(L,n_dHCH18O)=changedHCH18O*pfactor*vol2mass(n_dHCH18O)
        if(-changeL(L,n_dHCH18O) > trm_col(L,n_dHCH18O))then
          changeL(L,n_dHCH18O)=-.95d0*trm_col(L,n_dHCH18O)
          changedHCH18O=changeL(L,n_dHCH18O)*mass2vol(n_dHCH18O)*
     &                  bypfactor
        endif
        IF((trm_col(l,n_dHCH18O)+changeL(l,n_dHCH18O)) < minKG) THEN
          changeL(l,n_dHCH18O) = minKG - trm_col(l,n_dHCH18O)
          changedHCH18O=changeL(L,n_dHCH18O)*mass2vol(n_dHCH18O)*
     &                  bypfactor
        ENDIF
        changeL(L,n_dH13CHO)=changedH13CHO*pfactor*vol2mass(n_dH13CHO)
        if(-changeL(L,n_dH13CHO) > trm_col(L,n_dH13CHO))then
          changeL(L,n_dH13CHO)=-.95d0*trm_col(L,n_dH13CHO)
          changedH13CHO=changeL(L,n_dH13CHO)*mass2vol(n_dH13CHO)*
     &                  bypfactor
        endif
        IF((trm_col(l,n_dH13CHO)+changeL(l,n_dH13CHO)) < minKG) THEN
          changeL(l,n_dH13CHO) = minKG - trm_col(l,n_dH13CHO)
          changedH13CHO=changeL(L,n_dH13CHO)*mass2vol(n_dH13CHO)*
     &                  bypfactor
        ENDIF
#endif  /* TRACERS_dCO */
C -- CO --
        ...
        if((trm_col(l,n_CO)+changeL(l,n_CO)) < minKG)then
          changeL(l,n_CO) = minKG - trm_col(l,n_CO)
          changeCO=changeL(L,n_CO)*mass2vol(n_CO)*bypfactor
        endif
...
!       endif
        IF((trm_col(l,n_N_d1)+changeL(l,n_N_d1)) < minKG) THEN
          changeL(l,n_N_d1) = minKG - trm_col(l,n_N_d1)
          changeN_d1=changeL(L,n_N_d1)*mass2vol(n_N_d1)*bypfactor
        END IF
        changeL(L,n_N_d2)=changeN_d2*pfactor*vol2mass(n_N_d2)
        IF((trm_col(l,n_N_d2)+changeL(l,n_N_d2)) < minKG) THEN
          changeL(l,n_N_d2) = minKG - trm_col(l,n_N_d2)
          changeN_d2=changeL(L,n_N_d2)*mass2vol(n_N_d2)*bypfactor
        END IF
        changeL(L,n_N_d3)=changeN_d3*pfactor*vol2mass(n_N_d3)
        IF((trm_col(l,n_N_d3)+changeL(l,n_N_d3)) < minKG) THEN
          changeL(l,n_N_d3) = minKG - trm_col(l,n_N_d3)
...
C -- N2O5 --  (N2O5 from gas and het phase rxns)
        ...
        IF((trm_col(l,n_N2O5)+changeL(l,n_N2O5)) < minKG) THEN
...
c -- NOx --   (NOx from gas phase rxns)
        ...
        IF((trm_col(l,n_NOx)+changeL(l,n_NOx)) < minKG) THEN
...
C -- Alkenes --  (Alkenes from gas phase rxns)
     ...
     &  changeAlkenes*pfactor*vol2mass(n_Alkenes)
        IF((trm_col(l,n_Alkenes)+changeL(l,n_Alkenes)) < minKG)THEN
          changeL(l,n_Alkenes) = minKG - trm_col(l,n_Alkenes)
...
C -- d13Calke --  (d13Calke from gas phase rxns)
     ...
     &  changed13Calke*pfactor*vol2mass(n_d13Calke)
        IF((trm_col(l,n_d13Calke)+changeL(l,n_d13Calke)) < minKG)THEN
          changeL(l,n_d13Calke) = minKG - trm_col(l,n_d13Calke)
...
C -- isopp1g --  (isopp1g from gas phase rxns)
     ...
     &  changeisopp1g*pfactor*vol2mass(n_isopp1g)
        IF((trm_col(l,n_isopp1g)+changeL(l,n_isopp1g)) < minKG)THEN
          changeL(l,n_isopp1g) = minKG - trm_col(l,n_isopp1g)
...
C -- isopp2g --  (isopp2g from gas phase rxns)
     ...
     &  changeisopp2g*pfactor*vol2mass(n_isopp2g)
        IF((trm_col(l,n_isopp2g)+changeL(l,n_isopp2g)) < minKG)THEN
          changeL(l,n_isopp2g) = minKG - trm_col(l,n_isopp2g)
...
C -- apinp1g --  (apinp1g from gas phase rxns)
     ...
     &  changeapinp1g*pfactor*vol2mass(n_apinp1g)
        IF((trm_col(l,n_apinp1g)+changeL(l,n_apinp1g)) < minKG)THEN
          changeL(l,n_apinp1g) = minKG - trm_col(l,n_apinp1g)
...
C -- apinp2g --  (apinp2g from gas phase rxns)
     ...
     &  changeapinp2g*pfactor*vol2mass(n_apinp2g)
        IF((trm_col(l,n_apinp2g)+changeL(l,n_apinp2g)) < minKG)THEN
          changeL(l,n_apinp2g) = minKG - trm_col(l,n_apinp2g)
...
c -- Isoprene -- (Isoprene from gas phase rxns)
     ...
     &  changeIsoprene*pfactor*vol2mass(n_Isoprene)
        IF((trm_col(l,n_Isoprene)+changeL(l,n_Isoprene)) < minKG)
     &  THEN
          changeL(l,n_Isoprene) = minKG - trm_col(l,n_Isoprene)
...
c -- Terpenes -- (Terpenes from gas phase rxns)
     ...
     &  changeTerpenes*pfactor*vol2mass(n_Terpenes)
        IF((trm_col(l,n_Terpenes)+changeL(l,n_Terpenes)) < minKG)
     &  THEN
          changeL(l,n_Terpenes) = minKG - trm_col(l,n_Terpenes)
...
c -- AlkylNit -- (AlkylNit from gas phase rxns)
     ...
     &  changeAlkylNit*pfactor*vol2mass(n_AlkylNit)
        IF((trm_col(l,n_AlkylNit)+changeL(l,n_AlkylNit)) < minKG)
     &  THEN
...
c --  Ox --   ( Ox from gas phase rxns)
     ...
     *         *cpd/DTsrc
        ...
        if(changeL(L,n_NOx)< 0.) then
          taijls(i,j,L,ijlt_NOxd)=taijls(i,j,L,ijlt_NOxd)+changeNOx
     *         *cpd/DTsrc
        end if
c -- ClONO2 --   (ClONO2 from gas and het phase rxns)
        changeL(L,n_ClONO2)=changeClONO2*pfactor*
     &  vol2mass(n_ClONO2)
        IF((trm_col(l,n_ClONO2)+changeL(l,n_ClONO2)) < minKG) THEN
...
c -- ClOx --   (ClOx from gas and het phase rxns)
        ...
        IF((trm_col(l,n_ClOx)+changeL(l,n_ClOx)) < minKG) THEN
          changeL(l,n_ClOx) = minKG - trm_col(l,n_ClOx)
...
c -- HOCl --   (HOCl from het phase rxns)
          ...
          IF((trm_col(l,n_HOCl)+changeL(l,n_HOCl)) < minKG) THEN
            changeL(l,n_HOCl) = minKG - trm_col(l,n_HOCl)
...
c -- HCl --   (HCl from het phase rxns)
          ...
          IF((trm_col(l,n_HCl)+changeL(l,n_HCl)) < minKG) THEN
            changeL(l,n_HCl) = minKG - trm_col(l,n_HCl)
...
c -- HBr --   (HBr from gas phase rxns)
          ...
          IF((trm_col(l,n_HBr)+changeL(l,n_HBr)) < minKG) THEN
            changeL(l,n_HBr) = minKG - trm_col(l,n_HBr)
...
c -- BrOx --   (BrOx from gas phase rxns)
          ...
          IF((trm_col(l,n_BrOx)+changeL(l,n_BrOx)) < minKG) THEN
            changeL(l,n_BrOx) = minKG - trm_col(l,n_BrOx)
...
c -- BrONO2 --   (BrONO2 from gas phase rxns)
          ...
          IF((trm_col(l,n_BrONO2)+changeL(l,n_BrONO2)) < minKG) THEN
...
C       ACCUMULATE 3D NO3 diagnostic: 
        if (y(nNO3,L) > 0.d0 .and. y(nNO3,L) < 1.d20)
     &  taijls(i,j,L,ijlt_NO3)=taijls(i,j,L,ijlt_NO3)+y(nNO3,L)
...
C Lower limit on HO2NO2 : 
        if(trm_col(l,n_HO2NO2)+changeL(l,n_HO2NO2) < minKG)
...
c Tropospheric halogen sink Br & Cl :
        if(y(nH2O,L)/y(nM,L) > 10.d-6)then ! sink by wet removal in trop
          rmv=0.5d0
          changeL(L,n_ClOx)  =-trm_col(L,n_ClOx)  *rmv
          changeL(L,n_HCl)   =-trm_col(L,n_HCl)   *rmv
          changeL(L,n_HOCl)  =-trm_col(L,n_HOCl)  *rmv
          changeL(L,n_ClONO2)=-trm_col(L,n_ClONO2)*rmv
          changeL(L,n_BrOx)  =-trm_col(L,n_BrOx)  *rmv
          changeL(L,n_HBr)   =-trm_col(L,n_HBr)   *rmv
          changeL(L,n_HOBr)  =-trm_col(L,n_HOBr)  *rmv
          changeL(L,n_BrONO2)=-trm_col(L,n_BrONO2)*rmv
        else
#ifdef EESC_BASED_CLTOT_BRTOT
          ! If using EESC, CLTOT is based on Ch3Cl + Anthropogenic Cl
          ! gases, using GSFC EESC (Paul Newman) and using vertical
          ! distribution due to photolysis via CFCs and methane:
#else /* default case: */
          ! If not reading in EESC, set CLTOT based on CFCs (3.0 ppbv
          ! yield from complete oxidation of 1.8 ppbv CFC) plus 0.8 ppbv
          ! background; vertical distribution of both (due to
          ! photolysis) via CFCs & methane:
#endif
          !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
#ifdef EESC_BASED_CLTOT_BRTOT
          CLTOT=AnthroCl*
     &         ( y(nn_CFC,1)/y(nM,1) - y(nn_CFC,L)/y(nM,L) ) /
     &         ( y(nn_CFC,1)/y(nM,1) )
          CLTOT=CLTOT+ch3cl_pppv*
     &         ( y(nn_CH4,1)/y(nM,1) - y(nn_CH4,L)/y(nM,L) ) /
     &         ( y(nn_CH4,1)/y(nM,1) )
#else /* default case: */
     ...
     &      trm_col(L,n_ClOx)*(CLTOT-1.D0)
            changeL(L,n_HCl)=changeL(L,n_HCl)*CLTOT+
     &      trm_col(L,n_HCl)*(CLTOT-1.D0)
            changeL(L,n_HOCl)=changeL(L,n_HOCl)*CLTOT+
     &      trm_col(L,n_HOCl)*(CLTOT-1.D0)
c           Conserve N wrt ClONO2 once inital Cl changes past:
     ...
     &        (trm_col(L,n_ClONO2)+changeL(L,n_ClONO2))*
     &        (CLTOT-1.D0)*tr_mm(n_NOx)/tr_mm(n_ClONO2)
              if(-changeL(L,n_NOx) > trm_col(L,n_NOx))changeL(L,n_NOx)=
     &        -0.8d0*trm_col(L,n_NOx)
            endif
            changeL(L,n_ClONO2)=changeL(L,n_ClONO2)*CLTOT+
     &      trm_col(L,n_ClONO2)*(CLTOT-1.D0)
          END IF

#ifdef EESC_BASED_CLTOT_BRTOT
          ! If using EESC, BRTOT is based on Ch3Br + Anthropogenic Br
          ! gases, using GSFC EESC (Paul Newman) and using vertical
          ! distribution due to photolysis via CFCs and methane, and
          ! converting from EESC to Br:
#else /* default: */
          ! If not reading in EESC, set BRTOT based on CFCs (4.5 pptv
          ! yield from complete oxidation of 1.8 ppbv CFC) plus 0.5 ppbv
          ! background; vertical distribution of both (due to
          ! photolysis) via CFCs & methane:
#endif
          !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &         ( y(nn_CFC,1)/y(nM,1) - y(nn_CFC,L)/y(nM,L) ) /
     &         ( y(nn_CFC,1)/y(nM,1) )
          BRTOT=BRTOT+ch3br_pppv*
     &         ( y(nn_CH4,1)/y(nM,1) - y(nn_CH4,L)/y(nM,L) ) /
     &         ( y(nn_CH4,1)/y(nM,1) )
          BRTOT=BRTOT*5.d-3 ! convert from EESC to Br
#else /* default: */
...
c           Conserve N wrt BrONO2 once inital Br changes past:
     ...
     &      trm_col(L,n_BrONO2)*(BRTOT-1.D0)
        ...
        do v=1,vbs_sets
          vbs_conc(v)%gas=trm_col(l,vbs_conc(v)%igas)*kg2ugm3
          vbs_conc(v)%aer=trm_col(l,vbs_conc(v)%iaer)*kg2ugm3
        enddo
#endif  /* TRACERS_AEROSOLS_VBS */
...
#ifdef TRACERS_HETCHEM
#ifdef TRACERS_NITRATE
        tr3Dsource(l,nChemistry,n_N_d1) = changeL(l,n_N_d1) *bydtsrc
        tr3Dsource(l,nChemistry,n_N_d2) = changeL(l,n_N_d2) *bydtsrc
        tr3Dsource(l,nChemistry,n_N_d3) = changeL(l,n_N_d3) *bydtsrc
#endif  /* TRACERS_NITRATE */
#endif  /* TRACERS_HETCHEM */
...
C to initial conditions and set the chemistry change to zero...
      if(fix_CH4_chemistry == 1)then
        tr3Dsource(:,nChemistry,n_CH4)=0.d0 ! forget CH4 chemistry changes
...
CCCCCCCCCCCCC PRINT SOME CHEMISTRY DIAGNOSTICS CCCCCCCCCCCCCCCC
     ...
     &    pBrOx(ijlprn(3),I,J)
         call write_parallel(trim(out_line),crit=jay)
         write(out_line,*)
     &   'sun, sza,I,J,Itime= ',sza,I,J,Itime
         call write_parallel(trim(out_line),crit=jay)
       end if
      end if
     ...
     &     J==ijlprn(2).and.I==ijlprn(1).and.L==ijlprn(3))then
          jay = .true.!(J >= J_0 .and. J <= J_1)
          write(out_line,*)
     &    'dark, sza,I,J,L,Itime= ',sza,I,J,L,Itime
          call write_parallel(trim(out_line),crit=jay)
          if(pscX(L))then
     ...
     &    100.d0*(changeAldehyde)/yAldehyde(L,I,J),' percent of'
     &    ,yAldehyde(L,I,J),'(',1.d9*yAldehyde(L,I,J)/y(nM,L),' ppbv)'
          call write_parallel(trim(out_line),crit=jay)
#ifdef TRACERS_dCO
          write(out_line,198) 'd17Oald ',': ',
     &    changed17Oald,' molecules produced; ',
     &    100.d0*(changed17Oald)/yd17Oald(L,I,J),' percent of'
     &    ,yd17Oald(L,I,J),'(',1.d9*yd17Oald(L,I,J)/y(nM,L),' ppbv)'
          call write_parallel(trim(out_line),crit=jay)
          write(out_line,198) 'd18Oald ',': ',
     &    changed18Oald,' molecules produced; ',
     &    100.d0*(changed18Oald)/yd18Oald(L,I,J),' percent of'
     &    ,yd18Oald(L,I,J),'(',1.d9*yd18Oald(L,I,J)/y(nM,L),' ppbv)'
          call write_parallel(trim(out_line),crit=jay)
          write(out_line,198) 'd13Cald ',': ',
     &    changed13Cald,' molecules produced; ',
     &    100.d0*(changed13Cald)/yd13Cald(L,I,J),' percent of'
     &    ,yd13Cald(L,I,J),'(',1.d9*yd13Cald(L,I,J)/y(nM,L),' ppbv)'
          call write_parallel(trim(out_line),crit=jay)
#endif  /* TRACERS_dCO */
     ...
     &    ,y(nn_HBr,L),'(',1.d9*y(nn_HBr,L)/y(nM,L),' ppbv)'
          ...
          write(out_line,199) 'NO2, NO3  = ',y(nNO2,L),y(nNO3,L)
          call write_parallel(trim(out_line),crit=jay)
...
C**** GLOBAL parameters and variables:
      USE RESOLUTION, only  : LM
      USE MODEL_COM, only: Itime,ItimeI
      USE TRACER_COM, only: coupled_chem,trm_col,nn_N2O5,n_N2O5,n_SO4
      USE TRACER_COM, only: n_ClONO2
      USE TRACER_COM, only: n_HOCl
      USE TRDIAG_COM, only: jls_N2O5sulf
      USE OldTracer_mod, only: vol2mass,tr_mm
     ...
     & cboltz,r1,sb,nst,y,nM,nH2O,ro,sn,which_trop,dt2,
     & RGAMMASULF,pscX,topLevelOfChemistry,rh,bythick,rrbi,rrhet,
     & so4_offline
...
!@var wprod_sulf N2O5 change by sulfate reactions in molecules/cm3/timestep
      REAL*8:: bytl,dd,pp,fw,rkp,rk2,rk3M,rrrr,temp,beta,pcon,waterPPMV
      real*8 :: associationReaction, activationReaction,pfactor,
     & k0T,k0TM,kinfT,kinfTbyM,wprod_sulf
      real*8 :: RVELN2O5,RVELClONO2,RVELHOCl
      real*8 :: mean_molecular_speed
      INTEGER             :: L,jj
      INTEGER, INTENT(IN) :: I,J
...
!@+   RAD-code aerosol extinction 
      ...
      INTEGER               :: LAXt,LAXb
      real*8, allocatable, dimension(:) :: rkext
      real*8 :: pscEx,sulfate
...
#ifdef TRACERS_dCO
          else if (jj==rrbi%XO2N_HO2__dMe17OOH_O2) then
            rr(jj,L)=rr(rrbi%XO2_HO2__dMe17OOH_O2,L)
     &        *rr(rrbi%XO2N_NO__AlkylNit_M,L)
     &        /rr(rrbi%XO2_NO__NO2_M,L)
          else if (jj==rrbi%XO2N_HO2__dMe18OOH_O2) then
            rr(jj,L)=rr(rrbi%XO2_HO2__dMe18OOH_O2,L)
     &        *rr(rrbi%XO2N_NO__AlkylNit_M,L)
     &        /rr(rrbi%XO2_NO__NO2_M,L)
          else if (jj==rrbi%XO2N_HO2__d13MeOOH_O2) then
            rr(jj,L)=rr(rrbi%XO2_HO2__d13MeOOH_O2,L)
     &        *rr(rrbi%XO2N_NO__AlkylNit_M,L)
     &        /rr(rrbi%XO2_NO__NO2_M,L)
#endif  /* TRACERS_dCO */
...
! Note that in nighttime the pressure bounds are 250 and 3, not 245 and 5.
        if(pres(L) >= 245.d0 .or. pres(L) <= 5.d0)then 

! All heterogeneous reactions except N2O5 on sulfate are set to effectively
! zero outside the 245-5 pressures range.
...
c         Reaction rrhet%N2O5_H2O__HNO3_HNO3 on sulfate and PSCs:
          rr(rrhet%N2O5_H2O__HNO3_HNO3,L)=0.5d0*rkext(l)*RVELN2O5*0.2d0
          if(pres(l) > 31.6d0) rr(rrhet%N2O5_H2O__HNO3_HNO3,L)=
     &      rr(rrhet%N2O5_H2O__HNO3_HNO3,L)+0.25d0*pscEx*RVELN2O5*4.d-4
          wprod_sulf=dt2*y(nn_N2O5,L)*rr(rrhet%N2O5_H2O__HNO3_HNO3,L)

c         Reaction rrhet%ClONO2_H2O__HOCl_HNO3 on sulfate and PSCs:
          RVELClONO2=
     &      mean_molecular_speed(tl(l),tr_mm(n_ClONO2)*1.d-3)*100.d0 ! cm/s
          rr(rrhet%ClONO2_H2O__HOCl_HNO3,L)=
     &      0.5d0*rkext(l)*RVELClONO2*0.8d-2
          if(pres(l) > 31.6d0) rr(rrhet%ClONO2_H2O__HOCl_HNO3,L)=
     &      rr(rrhet%ClONO2_H2O__HOCl_HNO3,L)
     &      +0.25d0*pscEx*RVELClONO2*4.d-3

c         Reaction rrhet%ClONO2_HCl__Cl_HNO3 on PSCs:
          if(pres(l) > 31.6d0) then
            rr(rrhet%ClONO2_HCl__Cl_HNO3,L)=
     &        0.25d0*pscEx*RVELClONO2*0.2d0

c         Reaction rrhet%HOCl_HCl__Cl_H2O on PSCs:
            RVELHOCl=
     &        mean_molecular_speed(tl(l),tr_mm(n_HOCl)*1.d-3)*100.d0 ! cm/s
            rr(rrhet%HOCl_HCl__Cl_H2O,L)=0.25d0*pscEx*RVELHOCl*0.1d0

c         Reaction rrhet%N2O5_HCl__Cl_HNO3 on PSCs:
            rr(rrhet%N2O5_HCl__Cl_HNO3,L)=0.25d0*pscEx*RVELN2O5*0.003d0
          end if
        end if

        pfactor=ma(L)/y(nM,L)
        CALL INC_TAJLS2(I,J,L,jls_N2O5sulf,
     &                 -1.d0*wprod_sulf*pfactor*vol2mass(n_N2O5))
     ...
     &              kpart_temp_ref(whichsoa(n_apinp2a)))
      ...
      if (coupled_chem == 1) then
        ...
        ! (so4 in kg/m2 times byth in m-1 gives kg/m3)
      ...
      real*8 function mean_molecular_speed(t, mw)
!@sum Function that returs the mean molecular speed [m s-1] of a gaseous
!@+   molecule with a molecular weight of mw [kg mol-1] at temperature t [K].
!@+   The equation used is v = sqrt(8 * Kb * T / (pi * mw)).
!@auth Kostas Tsigaridis
      ...
      implicit none
!@var t Temperature [K]
!@var mw Molecular weight [kg mol-1]
      real*8, intent(in) :: t, mw
!@param kboltJ Boltzmann constant (=R/Na) [J K-1, or kg m2 s-2 K-1]
      real*8, parameter :: kboltJ=gasc/avog
!@var smw Single molecule weight [kg]
      real*8 :: smw

      smw=mw/avog
      mean_molecular_speed=sqrt(8.d0*kboltJ*t/(pi*smw))
