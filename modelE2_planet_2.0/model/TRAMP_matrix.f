#include "rundeck_opts.h"
      SUBROUTINE MATRIX(AERO,GAS,EMIS_MASS,TSTEP,TK,RH,PRES,AQSO4RATE,
     +                  WUPDRAFT,FLAND,DIAG,PM)
!-----------------------------------------------------------------------------------------------------------------------
!
...
!-----------------------------------------------------------------------------------------------------------------------
      ...
      USE AERO_PARAM,   ONLY: INUC
      USE AERO_NPF  ,   ONLY: READJTIMN 
      ...
      REAL(8) :: ANO3,ANH4                 ! temporary variables that hold AERO(MASS_NO3) and AERO(MASS_NH4)
      ...
      REAL(8) :: FSO4TMP                   ! scratch variable for computing Hamaker constant dependence
      ...
      REAL(8) :: AERO_total                ! NO3 and NH4 mass added to sulfate
      ...
      REAL(8), SAVE :: KBAR0IJH1(NWEIGHTS,NWEIGHTS)     ! as above before averaging across species by Hamaker constant
      REAL(8), SAVE :: KBAR0IJH2(NWEIGHTS,NWEIGHTS)
      REAL(8), SAVE :: KBAR3IJH1(NWEIGHTS,NWEIGHTS)
      REAL(8), SAVE :: KBAR3IJH2(NWEIGHTS,NWEIGHTS)
      ...
      LOGICAL, PARAMETER :: USE_HAMAKER = .TRUE.        ! Repeats coagulation calcs for Van der Waals influence on SO4
...
!----------------------------------------------------------------------------------------------------------------------
      ...
      IF ( FIRSTIME ) THEN
        ...
        IF ( .NOT. NO_MICROPHYSICS ) THEN
          ...
          CALL SETUP_KIJ_TABLES(USE_HAMAKER)
          IF( UPDATE_KIJ .EQ. 0 ) THEN
            DGN(:)     = DGN0(:)
            DGN_DRY(:) = DGN0(:)
            if (USE_HAMAKER) then
              CALL GET_KBARNIJ(0,TK,PRES,DGN,KBAR0IJH1,KBAR3IJH1,KBAR0IJH2,KBAR3IJH2)
            else
              CALL GET_KBARNIJ(0,TK,PRES,DGN,KBAR0IJH1,KBAR3IJH1)
            endif
          ENDIF
        ENDIF
        CALL SETUP_EMIS  
     ...
     &                                               DPCUT_IMTR, XNUM_FACTOR, X3_TERM, LNSG_AKK, LNSG_ACC
      ...
      ANO3=0.d0
      ANH4=0.d0
      if (MASS_NO3>0) ANO3=AERO(MASS_NO3)
      if (MASS_NH4>0) ANH4=AERO(MASS_NH4)
      CALL AERO_THERMO(TOT_SULF,ANO3,ANH4,TOT_DUST,TOT_SEAS,AERO(MASS_H2O),ApH,SSH2O,
     &                 GAS(GAS_NH3),GAS(GAS_HNO3),TK,RH,RHD,RHC,
     &                 WRITE_LOG,AUNIT1)
      if (MASS_NO3>0) AERO(MASS_NO3)=ANO3
      if (MASS_NH4>0) AERO(MASS_NH4)=ANH4
      ...
      IF ( UPDATE_DP ) THEN
        ...
        DO I=1, NWEIGHTS
          ...
          if (mass_no3>0) MASS_COMP(I,nmass_spcs+mass_no3) = FTMP * AERO(mass_no3)           ! [ug/m^3] nitrate
          if (mass_nh4>0) MASS_COMP(I,nmass_spcs+mass_nh4) = FTMP * AERO(mass_nh4)           ! [ug/m^3] ammonium
     ...
     &                                   + SSH2O_PER_SSMASS * AERO(SEAS_MAP(J)) ! [ug/m^3]
        ENDDO
        DO I=1, NWEIGHTS
          DO J=1, nmass_spcs+nextra
            if (j==nmass_spcs+mass_h2o) cycle ! all components except water
...
!          ENDIF
     ...
     &                     WSTR_DDEP,USTR_DDEP,RAER_DDEP,DGN,LNSIG0,DENSPI)
      ...
      CASE ( 1 )
        if (USE_HAMAKER) then
          CALL GET_KBARNIJ(1,TK,PRES,DGN,KBAR0IJH1,KBAR3IJH1,KBAR0IJH2,KBAR3IJH2)
        else
          CALL GET_KBARNIJ(1,TK,PRES,DGN,KBAR0IJH1,KBAR3IJH1)
        endif
      END SELECT

      ! For mass fraction that is SO4, enhance coagulation according to Hamaker constant
      IF (USE_HAMAKER) THEN
        DO I=1, NWEIGHTS
          FSO4TMP = min(max(AERO(SULF_MAP(I))/(TOT_MASS(I)+teeny),0.d0),1.d0)
          DO J=1, NWEIGHTS
            if (I==J) then
              KBAR0IJ(I,J) = KBAR0IJH1(I,J)*(1.d0-FSO4TMP)+KBAR0IJH2(I,J)*FSO4TMP
              KBAR3IJ(I,J) = KBAR3IJH1(I,J)*(1.d0-FSO4TMP)+KBAR3IJH2(I,J)*FSO4TMP
            else
              KBAR0IJ(I,J) = KBAR0IJH1(I,J)
              KBAR3IJ(I,J) = KBAR3IJH1(I,J)
            endif
          ENDDO
        ENDDO
      ELSE
...
! send output back to MATRIX
     ...
     &                 WRITE_LOG,AUNIT1)
      ...
      !----------------------------------------------------------------------------------------------------------------
      AEROTMP2(:) = AERO(:) 
      if (mass_dd1_dust>0) AERO(MASS_DD1_DUST) = MAX( AERO(MASS_DD1_DUST), TINYNUMER )
      if (mass_bc1_bcar>0) AERO(MASS_BC1_BCAR) = MAX( AERO(MASS_BC1_BCAR), TINYNUMER )
      if (mass_bc2_bcar>0) AERO(MASS_BC2_BCAR) = MAX( AERO(MASS_BC2_BCAR), TINYNUMER )
      if (mass_dd2_dust>0) AERO(MASS_DD2_DUST) = MAX( AERO(MASS_DD2_DUST), TINYNUMER )

      TOT_SULF = SUM( AERO(SULF_MAP(:)) ) + TINYNUMER
#ifndef TRACERS_AMP_M11 /* not */
      OPTOT_NO3NH4H2O_TO_SULF = 1.0D+00 + (AERO(MASS_NO3)+AERO(MASS_NH4)+AERO(MASS_H2O)) / TOT_SULF
#else
      OPTOT_NO3NH4H2O_TO_SULF = 1.0D+00 + (AERO(MASS_H2O)) / TOT_SULF
#endif  /* not TRACERS_AMP_M11 */

        !--------------------------------------------------------------------------------------------------------------
        ! Transfer mode DD1 to mode DS1.
        !--------------------------------------------------------------------------------------------------------------
      if (mass_dd1_dust>0) then
...
c Diagnostics for PM1 PM(1), PM2.5 PM(2) and PM10 PM(3)
        ...
        DO I=1, NWEIGHTS        ! loop over modes (quadrature points) 
           DO Q=1, NM(I)        ! loop over species defined for mode I 
#ifndef TRACERS_AMP_M11 /* not */
          IF (Q.eq.1) THEN ! hack that assumes that sulfate is species 1
              AERO_total= AERO(MASS_MAP(I,Q))+(AERO(MASS_MAP(I,Q))*(AERO(MASS_NO3)+AERO(MASS_NH4))/TOT_SULF)
           ELSE
              AERO_total= AERO(MASS_MAP(I,Q))
           ENDIF
#else
              AERO_total= AERO(MASS_MAP(I,Q))
#endif  /* not TRACERS_AMP_M11 */
               PM(1) = PM(1) + AERO_total*
     &          0.5d0*(erf(log(1.d0/DGN(I))/(sqrt(2.d0)*log(sig0(I)))) 
     &              -erf(log(0.001d0/DGN(I))/(sqrt(2.d0)*log(sig0(I)))))

               PM(2) = PM(2) + AERO_total*
     &          0.5d0*(erf(log(2.5d0/DGN(I))/(sqrt(2.d0)*log(sig0(I)))) 
     &              -erf(log(0.001d0/DGN(I))/(sqrt(2.d0)*log(sig0(I)))))

               PM(3) = PM(3) + AERO_total*
