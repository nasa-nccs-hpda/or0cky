...
#undef TRACERS_WATER
#endif
#ifndef SEAICE_FIXES_2022
#define SEAICE_FIXES_2022
#endif
      MODULE SEAICE
...
!@dbparam minIceTemperature minimum allowed sea/lake ice temperature (C)
       real*8 :: minIceTemperature = -100.d0
...
!@var TSIL temperature of ice layers (C)
     ...
     $     ,SMELTI,HMELTI,HC,dFdTI,F
      ...
      REAL*8 MSI1, MELTS, MELTS2, DEW, CMPRS, DEWS, HCMPRS, SNOWX
...
C****
      SNOWX = SNOWL(1) + DEWS-MELTS
      IF (SNOWX.GT.0d0) THEN
        CMPRS = MIN(dSNdML*MELTS, SNOWX) ! > 0.
        HCMPRS = HSNOW(1)*CMPRS/SNOWX
#ifdef TRACERS_WATER
        TRMELTS(:)= MELTS*TRSNOW(:,1)/(SNOWL(1)+DEWS)
        TRCMPRS(:)= CMPRS*(TRSNOW(:,1)-TRMELTS(:))/SNOWX
...
C**** flushing (30% of MELT12 pushes out an equivalent mass of brine)
#ifdef SEAICE_FIXES_2022
            IF(MSI2.GT.AC2OIM.AND.brine_frac.NE.0.) THEN
            IF(brine_frac.gt.0.05d0.AND.MSI2.GT.2d0*RHOI) THEN
              rate = min(1d0,0.3d0*MELT12/(MICE(L)*brine_frac)) ! fractional loss
            ELSE
              rate = min(0.3d0,0.3d0*MELT12/(MICE(L)*brine_frac)) ! fractional loss
            ENDIF
            ENDIF
#else
            ...
            ENDIF
#endif
C**** basic gravity drainage (30 day timescale)
#ifdef SEAICE_FIXES_2022
C****  correction to the factor 100 in the previous version
C****  in oreder to match a 30 days timescale for GD 
C****  as well as a new minimimum for of 5% for the brine fraction
            if (brine_frac.gt.0.05d0) rate =
     *           min(rate + DT*BYDTSSI*5d0*(brine_frac-0.05d0),1d0)
#else
...
#ifdef SEAICE_FIXES_2022
            if (SICE(L).lt.ssimin*MICE(L)) then
               rate=1d0
               DMSI(L) = brine_frac*MICE(L)
               DHSI(L) = max(-mu*1d3*SICE(L)*shw,
     *           HICE(L)-(TSIL(L)*shi-lhm)*(mice(l)-dmsi(l)))
               DSSI(L) = rate*SICE(L)
            else
...
#ifdef SEAICE_FIXES_2022
        IF(Ti1.LT.0d0.AND.Ti1.GT.-1d-15) Ti1=0d0 ! clean up roundoff errors
#endif
...
#ifdef SEAICE_FIXES_2022
C**** make sure that imprecision in Ti does not cause TSNOW > 0.
        IF (Ti1 < 0 .and. SNOWL(1) > 0
     *       .AND.HICE(1).NE.MICE(1)*Em(Si1)) THEN
...
#ifdef SEAICE_FIXES_2022
C**** make sure that imprecision in Ti does not cause TSNOW > 0.
        IF (Ti1 < 0 .and. SNOWL(2) > 0
     *       .AND.HICE(2).NE.MICE(2)*Em(Si1)) THEN
          HSNOW(2)=MIN(HSNOW(2),(Ti1*shi-LHM)*SNOWL(2))
        ENDIF
#endif
...
#ifdef SEAICE_FIXES_2022
      IF(dabs(FMSI1).LT.1.d-14) FMSI1=0.
#else
...
#ifdef TRACERS_WATER
            FTRSI(:,L) = TRICE(:,L)+(FMSI(L)-MICE(L))*TRICE(:,L-1)
     $           /MICE(L-1)
#endif
          ELSE
...
c**** solve Ei = shi*(Ti+mu*Si)-lhm*(1+mu*Si/Ti)-mu*Si*shw
          ...
          if(Ei .ge. shw*Tm) then ! at or above melting point
            ...
            c=lhm*Tm
            det=b*b-4q0*shi*c   ! > 0
            Ti=-5q-1*(b + sqrt(det))*byshi
          end if
        else
          Ti=(Ei+lhm)*byshi  ! pure ice case
          IF (abs(Ei+lhm).LT.1q-10) Ti=0q0 ! clean up roundoff errors
        end if
      end select
...
c**** so it has to be multiplied by MICE/(MICE+SNOWL)
        Ti2b=(Eit+lhm*(1q0-1q-3*Si*(MICE/(MICE+SNOWL))))*byshi
      case ("BP")               ! Brine pocket formulation
        if (Si.gt.0q0) then
c**** solve Ei = shi*(Ti+mu*Si)-lhm*(1+mu*Si/Ti)-mu*Si*shw
          Tm=-mu*Si
          ...
          det=b*b-4q0*shi*c   ! > 0
          Ti2b=-5q-1*(b + sqrt(det))*byshi
        else
          Ti2b=(Eit+lhm)*byshi  ! pure ice case
          IF (abs(Eit+lhm).LT.1q-10) Ti2b=0q0 ! clean up roundoff errors
        end if
      end select
