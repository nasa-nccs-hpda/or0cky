...
!@ver  1.0
     ...
     &               J_STRT=J_0, J_STOP=J_1)
      ...
      allocate( FLASH_DENS(I_0:I_1,J_0:J_1) )
      FLASH_DENS = 0d0

      allocate(    CG_DENS(I_0:I_1,J_0:J_1) )
      CG_DENS = 0d0

#ifdef AUTOTUNE_LIGHTNING
      allocate( FLASH_UNC(I_0:I_1,J_0:J_1) )
      FLASH_UNC = 0d0
#endif
      !---------------------------------------------
      ...
      allocate(      ENOx_lgt(I_0:I_1,J_0:J_1) )
      allocate(      CLDTOPL(I_0:I_1,J_0:J_1)  )
...
!@auth Colin Price / Greg Faluvegi / Lee Murray
      ...
      !if ( sum(dZ) == 0 ) return ! This may happen on the first call.
      ...
      do L=1,levtrop
        tr3Dsource(L,nOther,n_NOx) = srclight(L)
      end do
      do L=levtrop+1,LM
