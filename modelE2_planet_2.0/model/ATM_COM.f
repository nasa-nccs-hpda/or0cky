...
!@var P surface pressure (hecto-Pascals - PTOP)
      ...
      Real*8 :: temperature_istart1=250.
...
!@+    run time
!@param Tinit,Qinit initial T,Q for istart=1, defaults 250,3d-6
!@auth NCCS (Goddard) Development Team
     ...
     &     DPDX_BY_RHO,DPDY_BY_RHO,DPDX_BY_RHO_0,DPDY_BY_RHO_0
      ...
      INTEGER :: IER, dim, L
      REAL*8, allocatable :: Tinit(:),Qinit(:)
      character*1 :: ptype
...
C****
      ...
      U(:,:,:)=0.
      V(:,:,:)=0.
!     T in K will be changed to pot. temperature later
      T(:,:,:)=temperature_istart1
      if (is_set_param('Tinit')) then
        call query_param ('Tinit',dim,ptype)
        if(dim>LM) dim=LM
        allocate (Tinit(LM))
        call sync_param('Tinit',Tinit(1:dim),dim)
        if (dim<LM) Tinit(dim+1:LM) = Tinit(dim)
        temperature_istart1 = Tinit(1)
        do L=1,LM
          T(:,:,L) = Tinit(L)
        end do
        deallocate (Tinit)
      ...
      Q(:,:,:)=3.D-6
      if (is_set_param('Qinit')) then
        call query_param ('Qinit',dim,ptype)
        if(dim>LM) dim=LM
        allocate (Qinit(LM))
        call sync_param('Qinit',Qinit(1:dim),dim)
        if (dim<LM) Qinit(dim+1:LM) = Qinit(dim)
        do L=1,LM
          Q(:,:,L) = Qinit(L)
        end do
        deallocate (Qinit)
      end if
!     P will be set from Psf and topography later
      qcl(:,:,:)=0.
      qci(:,:,:)=0.
      ZATMO(:,:)=0.
