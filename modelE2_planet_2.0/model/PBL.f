...
c**** Do not use global variables for that purpose !
      type t_pbl_args
        ...
        real*8 canopy_temperature,fw,SoilTemp,SoilSat
...
!@var mcfrac fractional area with moist convection in grid cell
        REAL(KIND=8) :: mcfrac
!@var snowfr snow fraction on land
        real(kind=8) :: snowfr
!@var rhosrf surface atmospheric density
        real(kind=8) :: rhosrf
...
#ifdef TRACERS_DRYDEP
        real*8, pointer, dimension(:) :: vd1=>null()
#ifdef TRACERS_DRYDEP_DIAG
        real*8, pointer, dimension(:) :: egs1=>null()
        real*8, pointer, dimension(:) :: egcut_dry1=>null()
        real*8, pointer, dimension(:) :: eggro_dry1=>null()
        real*8, pointer, dimension(:) :: egcut_wet1=>null()
        real*8, pointer, dimension(:) :: eggro_wet1=>null()
        real*8, pointer, dimension(:) :: egcut_snow1=>null()
        real*8, pointer, dimension(:) :: eggro_snow1=>null()
        real*8, pointer, dimension(:) :: vd_for=>null()
        real*8, pointer, dimension(:) :: vd_cro=>null()
        real*8, pointer, dimension(:) :: vd_gra=>null()
        real*8, pointer, dimension(:) :: vd_shr=>null()
        real*8, pointer, dimension(:) :: vd_bar=>null()
        real*8, pointer :: frac_for=>null()
        real*8, pointer :: frac_cro=>null()
        real*8, pointer :: frac_gra=>null()
        real*8, pointer :: frac_shr=>null()
        real*8, pointer :: frac_bar=>null()
        real*8, pointer, dimension(:) :: egs_for=>null()
        real*8, pointer, dimension(:) :: egs_cro=>null()
        real*8, pointer, dimension(:) :: egs_gra=>null()
        real*8, pointer, dimension(:) :: egs_shr=>null()
#endif
#endif
...
!@var  tv    local virtual potential temperature
    ...
    (defined TRACERS_TOMAS)
      ...
      use tracer_com, only: gasex_index, n_co2n, n_cfcn
#endif
...
C**** First, define some useful quantities
      ...
      pbl_args%rhosrf=rhosrf

#ifdef TRACERS_DRYDEP
C**** Get tracer deposition velocity for all tracers
C**** Note does not account for aerodynamic transport from bottom of atm to land
      call get_dep_vel(ilong,jlat,itype,ch,ws,ustar,ts
     &     ,rhosrf,rh1,visc
     &     ,pbl_args%canopy_temperature,pbl_args%fw
     &     ,pbl_args%SoilTemp,pbl_args%SoilSat
...
#ifdef TRACERS_DRYDEP_DIAG
     &     ,pbl_args%egs1
     &     ,pbl_args%egcut_dry1, pbl_args%eggro_dry1
     &     ,pbl_args%egcut_wet1, pbl_args%eggro_wet1
     &     ,pbl_args%egcut_snow1, pbl_args%eggro_snow1
     &     ,pbl_args%vd_for, pbl_args%vd_cro
     &     ,pbl_args%vd_gra, pbl_args%vd_shr
     &     ,pbl_args%vd_bar
     &     ,pbl_args%egs_for, pbl_args%egs_cro
     &     ,pbl_args%egs_gra, pbl_args%egs_shr
     &     ,pbl_args%frac_for,pbl_args%frac_cro
     &     ,pbl_args%frac_gra,pbl_args%frac_shr
     &     ,pbl_args%frac_bar
#endif
     &     )
#endif
...
C**** Tracer Dry Deposition boundary condition for dry dep tracers:
        if(dodrydep(pbl_args%ntix(itr))) then
          trsf=pbl_args%trsfac(itr)*pbl_args%vd1(pbl_args%ntix(itr))
        end if
#endif
...
#ifdef TRACERS_SPECIAL_O18
      real*8, intent(out) :: fac_cq_tr(NTM)
      real*8 :: cq_tr(NTM),z0q_tr(NTM),Sc_tr,get_diff_rel,dpsiq_dum
      integer :: itr
#endif
...
c *********************************************************************
     ...
     *       call getchq(zgs,z0q_tr(itr),lmonin,dm,dpsiq_dum,cq_tr(itr))
      end do
      do itr=1,NTM
        if (tr_wd_TYPE(itr).eq.nWater)
...
#ifdef TRACERS_DRYDEP
      allocate(pbl_args%vd1(maxNTM))
      pbl_args%vd1 = 0.d0
#ifdef TRACERS_DRYDEP_DIAG
      allocate(pbl_args%egs1(maxNTM))
      allocate(pbl_args%egcut_dry1(maxNTM))
      allocate(pbl_args%eggro_dry1(maxNTM))
      allocate(pbl_args%egcut_wet1(maxNTM))
      allocate(pbl_args%eggro_wet1(maxNTM))
      allocate(pbl_args%egcut_snow1(maxNTM))
      allocate(pbl_args%eggro_snow1(maxNTM))
      allocate(pbl_args%vd_for(maxNTM))
      allocate(pbl_args%vd_cro(maxNTM))
      allocate(pbl_args%vd_gra(maxNTM))
      allocate(pbl_args%vd_shr(maxNTM))
      allocate(pbl_args%vd_bar(maxNTM))
      allocate(pbl_args%egs_for(maxNTM))
      allocate(pbl_args%egs_cro(maxNTM))
      allocate(pbl_args%egs_gra(maxNTM))
      allocate(pbl_args%egs_shr(maxNTM))
      allocate(pbl_args%frac_for)
      allocate(pbl_args%frac_cro)
      allocate(pbl_args%frac_gra)
      allocate(pbl_args%frac_shr)
      allocate(pbl_args%frac_bar)
      pbl_args%egs1 = 0.d0
      pbl_args%egcut_dry1 = 0.d0
      pbl_args%eggro_dry1 = 0.d0
      pbl_args%egcut_wet1 = 0.d0
      pbl_args%eggro_wet1 = 0.d0
      pbl_args%egcut_snow1 = 0.d0
      pbl_args%eggro_snow1 = 0.d0
      pbl_args%vd_for = 0.d0 
      pbl_args%vd_cro = 0.d0
      pbl_args%vd_gra = 0.d0
      pbl_args%vd_shr = 0.d0
      pbl_args%vd_bar = 0.d0
      pbl_args%frac_for = 0.d0
      pbl_args%frac_cro = 0.d0
      pbl_args%frac_gra = 0.d0
      pbl_args%frac_shr = 0.d0
      pbl_args%frac_bar = 0.d0
      pbl_args%egs_for = 0.d0
      pbl_args%egs_cro = 0.d0
      pbl_args%egs_gra = 0.d0
      pbl_args%egs_shr = 0.d0
#endif
#endif
#ifdef TRACERS_WATER
...
#ifdef TRACERS_DRYDEP
      deallocate(pbl_args%vd1)
#ifdef TRACERS_DRYDEP_DIAG
      deallocate(pbl_args%egs1)
      deallocate(pbl_args%egcut_dry1)
      deallocate(pbl_args%eggro_dry1)
      deallocate(pbl_args%egcut_wet1)
      deallocate(pbl_args%eggro_wet1)
      deallocate(pbl_args%egcut_snow1)
      deallocate(pbl_args%eggro_snow1)
      deallocate(pbl_args%vd_for)
      deallocate(pbl_args%vd_cro)
      deallocate(pbl_args%vd_gra)
      deallocate(pbl_args%vd_shr)
      deallocate(pbl_args%vd_bar)
      deallocate(pbl_args%frac_for)
      deallocate(pbl_args%frac_cro)
      deallocate(pbl_args%frac_gra)
      deallocate(pbl_args%frac_shr)
      deallocate(pbl_args%frac_bar)
      deallocate(pbl_args%egs_for)
      deallocate(pbl_args%egs_cro)
      deallocate(pbl_args%egs_gra)
      deallocate(pbl_args%egs_shr)
#endif
#endif
#ifdef TRACERS_WATER
