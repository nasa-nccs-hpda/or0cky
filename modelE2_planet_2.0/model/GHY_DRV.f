...
ccc tracers variables
    ...
    (defined TRACERS_AMP)  || (defined TRACERS_TOMAS)
      ...
      pbl_args%snowfr=atmlnd%snowfr(i,j)
c**** prescribed dust emission
      pbl_args%d_dust(1:nDustBins) = d_dust(i, j, 1:nDustBins) / ptype
...
cddd#endif
    ...
    (defined TRACERS_AMP)  || (defined TRACERS_TOMAS)
      ...
      integer n,nx
#ifdef TRACERS_DRYDEP
      real*8 tdryd,tdd,td1,rtsdt,rts,depvel
#endif 
#ifdef TRACERS_AMP
      real*8 :: mass_to_number
#endif  /* TRACERS_AMP */
...
cddd     &     (arauto+asoilresp-agpp)/dtsurf
     ...
     &     + (arauto+asoilresp-agpp+excess_C_delta)*ptype
     &     *44.d0/12.d0
#endif
...
#ifdef TRACERS_AMP
       case ('N_SSA_1')
!     Should be coded using Matrix variables, but as they are constant for now hardcoded. (Sorry)          
!       mass_to_number=1.0D+00/(1.0D+12*EMIS_DENS(n)*PI6*DP0_EMIS(n)**3 )   
       mass_to_number=273457.275999069  ![1/ug]  
          trc_flux=pbl_args%ss1_flux*mass_to_number*1.d9 !trc_flux [kg/m2/s] * [1/kg] -> to get number   
        if (ijts_ampp(3,n)>0) 
     +    taijs(i,j,ijts_AMPp(3,n)) =taijs(i,j,ijts_AMPp(3,n)) +trc_flux  
        case ('N_SSC_1')
       mass_to_number=12690.8900124837
          trc_flux=pbl_args%ss2_flux*mass_to_number*1.d9
        if (ijts_ampp(3,n)>0) 
     +    taijs(i,j,ijts_AMPp(3,n)) =taijs(i,j,ijts_AMPp(3,n)) +trc_flux  
      case ('N_SSS_1')
         mass_to_number=273457.275999069 
      trc_flux=(pbl_args%ss1_flux+pbl_args%ss2_flux)*mass_to_number*1.d9
        if (ijts_ampp(3,n)>0) 
     +    taijs(i,j,ijts_AMPp(3,n)) =taijs(i,j,ijts_AMPp(3,n)) +trc_flux  
#endif
...
#ifdef TRACERS_AMP
        case ('N_DD1_1')
       mass_to_number=99414.1247027830  ![1/ug]
           trc_flux=sum(pbl_args%dust_flux(1:2))*mass_to_number*1.d9  !trc_flux [kg/m2/s] * [1/kg] -> to get number
        if (ijts_ampp(3,n)>0) 
     +    taijs(i,j,ijts_AMPp(3,n)) =taijs(i,j,ijts_AMPp(3,n)) +trc_flux  
        case ('N_DD2_1')
       mass_to_number=19396.8886990094
          trc_flux=sum(pbl_args%dust_flux(3:4))*mass_to_number*1.d9
        if (ijts_ampp(3,n)>0) 
     +    taijs(i,j,ijts_AMPp(3,n)) =taijs(i,j,ijts_AMPp(3,n)) +trc_flux  
        case ('N_DDD_1')
       mass_to_number=273457.275999069 
          trc_flux=sum(pbl_args%dust_flux(1:4))*mass_to_number*1.d9
        if (ijts_ampp(3,n)>0) 
     +    taijs(i,j,ijts_AMPp(3,n)) =taijs(i,j,ijts_AMPp(3,n)) +trc_flux  
        case ('M_DD1_DU')
...
c****
     ...
     *     ,sha,tf,rhow,deltx,gasc,stbo,mair
      use resolution, only : im,jm
      use model_com, only : modelEclock
      use model_com, only : dtsrc,nday,itime, orbit
      use BaseTime_mod
...
!!!     &   ,vs,vs0,tprime,qprime
     ...
     *     ,top_index_ij,top_dev_ij
     ...
     &     ,hsn_ij,fr_snow_ij,shc_soil_texture,mCO2cond_ij
     &     ,Qf_ij
#ifdef TRACERS_ON
!!! need for Ca hack
      use tracer_com, only : trm
...
c**** call tracers stuff
     ...
     &     )
      ...
      call get_canopy_temperature_fw(
     &     pbl_args%canopy_temperature, pbl_args%fw, i, j)
      call get_soil_temperature_sat(
     &     pbl_args%SoilTemp, pbl_args%SoilSat, i, j)
...
!!! changed units of Ca from mol/m^3 to ppm (also in advnc() )
      ...
      case(2)
#ifdef TRACERS_GASEXCH_land_CO2
        Ca = trm(i,j,1,n_CO2n)/ma1*mair/tr_mm(n_CO2n)*1.d6
!     &       *ps*100.0/gasc/ts
#else
...
!      endif
      ...
      mCO2cond_old = mCO2cond_ij(i,j)
      t = newBaseTime( modelEClock%getCurrentTime()
     &     + newTimeInterval(0.d0) )
      call advnc(
     &     entcells(i,j), Ca,
     &     cos_zen_angle, vis_rad, direct_vis_rad,
     &     Qf_ij(i,j),
     ...
     &     srheat,
     &     trheat,
     &     fgeotherm(i,j), ! geothermal heat flux (W/m^2)
     &     pbl_args%tsv/(1.+qs*xdelt),
     &     pbl_args%qsrf,
     &     ps - dmCO2cond(i,j)*grav * 0.01d0  ,
     &     rhosrf,
     &     cdh,
     &     q1*ma1,
     &     pbl_args%ws,
     ...
     &     ,ghy_tr
#endif
     &     ,orbit%getTrueAnomaly(t)/3.14159265358979d0*180.d0
     &     )

#ifdef DO_CO2_CONDENSATION
      dmCO2cond(i,j) = dmCO2cond(i,j) + mCO2cond_ij(i,j) - mCO2cond_old
      if ( dmCO2cond(i,j) > 0.d0 ) then
        snoage(3,i,j) =
     &       snoage(3,i,j)*(1.d0 - .2d0*min(1.d0, dmCO2cond(i,j)/.1d0) )
      endif
#ifdef DO_CO2_CONDENSATION_DEBUG
      if ( j==45 .and. i==20 ) then
        write(897,*) i, j, mCO2cond_ij(i,j), dmCO2cond(i,j)
      endif
#endif
#endif
...
#ifdef DO_CO2_CONDENSATION
      ! if have CO2 condensation - add it to bare soil
      ! (actually assume that thicker snowpack absorbs thinner one)
      snowd_ij(1,i,j) = max( snowd_ij(1,i,j),
     &     mCO2cond_ij(i,j)/1400.d0 ) ! i.e. mass(kg/m^2) / density(kg/m^3)
      atmlnd%fr_snow_rad(1,i,j) = max( atmlnd%fr_snow_rad(1,i,j),
     &     min(.6d0, mCO2cond_ij(i,j)/200.d0) )
! all these should be eliminated, but while they are present,
! adding CO2 snow to these vars
      atmlnd%snowe(i,j)=max( atmlnd%snowe(i,j), mCO2cond_ij(i,j) )
      atmlnd%snow(i,j) = atmlnd%snowe(i,j)
      atmlnd%snowfr(i,j) = max( atmlnd%snowfr(i,j),
     &     atmlnd%fr_snow_rad(1,i,j) )
      atmlnd%snowdp(i,j) = max( atmlnd%snowdp(i,j),
     &     mCO2cond_ij(i,j)/1.d3 )
#endif
...
c**** accumulate surface fluxes and prognostic and diagnostic quantities
      ...
      ! shdt sensible heat: positive down
      shdt=-ashg
      ! trying to add energy flux due to mass flux to sensible heat:
      ! condensation/sublimation of CO2 happen at ground temperature,
      ! but the gas is removed/added from/to first atmosphere layer -
      ! compensating for the related heat flux here
      shdt = shdt + (mCO2cond_ij(i,j) - mCO2cond_old)*sha*
     &     (pbl_args%tsv/(1.+qs*xdelt) - th1*psk)
      evhdt=-alhg
...
c****
      atmlnd%e0(i,j)=atmlnd%e0(i,j) + ae0 - eprec(i,j)/nisurf
...
! hack to fill polar rows
      if ( imaxj(j) < I_1 ) then
        dmCO2cond(imaxj(j)+1:I_1,j) = dmCO2cond(imaxj(j),j)
      endif
      end do loop_j
...
C
      case ('frlsm')
        do j=j_0,j_1; do i=i_0,imaxj(j)
          sddarr2d(i,j) = fearth(i,j)
        enddo;        enddo
        call inc_subdd(subdd,k,sddarr2d)
      case ('evaplsm')
        do j=j_0,j_1; do i=i_0,imaxj(j)
          sddarr2d(i,j) = atmlnd%evapor(i,j)*fearth(i,j)
        enddo;        enddo
        call inc_subdd(subdd,k,sddarr2d)
      end select
      enddo
...
c***********************************************************************
     ...
     *     ,tf_day1,tf_last
     ...
     &     ,ij_evapsn,ij_irrW, ij_irrE, ij_CO2cond, ij_geotherm
     &     ,ij_gbsif, ij_gvsif, ij_landcarbon
     ...
     &    ,airrig,aeirrig,alandC
      ...
      use ghy_com, only : gdeep, gsaveL, fearth, mCO2cond_ij, fgeotherm
      use ent_com, only : excess_C
      use ghy_com, only : gdeep, gsaveL, fearth, fgeotherm
...
ccc the following values are returned by PBL
     ...
     &     + ( (alandC+excess_C(i,j) )/nisurf)*ptype
      ...
      aij(i,j,ij_geotherm)=aij(i,j,ij_geotherm)+fgeotherm(i,j)*ptype/
     &   nisurf
      if (moddsf.eq.0) then
...
!      Ice fraction of bare soil layers
        do k=1,6
          aij(i,j,ij_gbsif+k-1)=aij(i,j,ij_gbsif+k-1)+fice(k,1)*ptype*fb
        end do
!      Ice fraction of canopy and vegetated soil layers
        do k=0,6
          aij(i,j,ij_gvsif+k)=aij(i,j,ij_gvsif+k)+fice(k,2)*ptype*fv
        end do
!     Water table depth
        aij(i,j,ij_gwtbl)=aij(i,j,ij_gwtbl)+(fb*zw(1)+fv*zw(2))*ptype
#ifdef DO_CO2_CONDENSATION
!     CO2 condensate
        aij(i,j,ij_CO2cond)=aij(i,j,ij_CO2cond)+mCO2cond_ij(i,j)*ptype
#endif
      end if
...
c****
!@dbparam top_dev_mean standard deviation of surface elevation
      real*8 :: top_dev_mean = 100d0
...
!**** sanity check
      if ( wsn_max > 0.d0 .and. .not. file_exists('GLMELT') ) then
        call stop_model("You can't remove extra snow if you don't"//
     &       " have ocean or you have not set a proper GLMELT file."//
     &       " To disable snow removal set wsn_max=0. (the default"//
     &       " is 2.).", 255)
      endif
...
c**** read topmodel parameters
        call sync_param('top_dev_mean', top_dev_mean)
        top_dev_ij = top_dev_mean
...
!******************************************************************
  ...
  !!  call  get_param( "init_flake", init_flake )
      ...
      if (istart == 8 .and. keep_params == 1) do_IC_fixups = 0
...
c**** fix initial conditions for soil heat if necessry
     ...
     &           q_ij(i,j,:,:), dz_ij(i,j,:), i, j )
          enddo
        enddo
!!!!  end if ! data read from GIC need to be fixed
...
c initialize soil (w, ht) from earth_*
        do k=1,ngm
          w(k,ibv) = max( thets(k,ibv)*earth_sat(k,ibv), thetm(k,ibv) )
     &         *dz(k)
          call temperature_to_heat(ht(k,ibv),
...
#ifdef BIOGENIC_EMISSIONS
! Update leaf area indices & land fractions for biogenic emissions
      CALL RDLAI
...
!     bare soil layers 1-6
     ...
     &         *pearth*fv
        ...
        do k=1,ngm
          aij(i,j,ij_wlay1+k-1) = aij(i,j,ij_wlay1+k-1) +
     &    flake(i,j)*w_ij(k,3,i,j)*rhow
        end do
      end if
c****
      end do
...
!@dbparam roughl_lice (m) hack to reset roughl for non-standard land ice fractions
!@dbparam roughl_bare_soil (m) for uniform conditions (ROUGHL file not needed)
      real*8 :: roughl_lice = -1.d30 , roughl_bare_soil = -1.d30
     ...
     *               I_STRT=I_0, I_STOP=I_1 )
      ...
      call sync_param( "roughl_bare_soil", roughl_bare_soil )
      ...
      if(file_exists('ROUGHL')) then
        ...
        call par_close(grid,fid)
      else if(roughl_bare_soil>0.d0) then
        rrr = roughl_bare_soil
      else
...
!! set above
     ...
     &               ) / (wsoil_tot) * w_ij(k,3,i,j) !!! removed /rhow
      ...
      subroutine get_canopy_temperature_fw(CanTemp, fw, i, j)
!@sum returns canopy temperature and wetness for the cell i,j
!@+   returns -1d30 for a cell with no vegetation
      use ghy_com, only : w_ij, ht_ij, fearth
      use ent_com, only : entcells
      use ent_mod, only : ent_get_exports
      real*8, intent(out) :: CanTemp, fw
      integer, intent(in) :: i, j
      real*8 :: fb, fv, fice, can_ht_cap, ws_can
     ...
     &     ht_ij(0,2,i,j), w_ij(0,2,i,j), can_ht_cap)
      if ( ws_can > 0.d0 ) then
        fw = (w_ij(0,2,i,j)/ws_can)**(2.d0/3.d0)
      else
        fw = 0.d0
      endif

      end subroutine get_canopy_temperature_fw


      subroutine get_soil_temperature_sat(SoilTemp, SoilSat, i, j)
!@sum returns soil temperature and saturation at the surface for the cell i,j
!@    area weighted average for bare and vegetated soil 
      use ghy_com, only : w_ij, ht_ij, fearth, q_ij, dz_ij, ngm
      use sle001, only : get_soil_properties
      real*8, intent(out) :: SoilTemp, SoilSat
      integer, intent(in) :: i, j
      real*8 :: fice, t,fbv(2)
      real*8 :: thets(ngm), thetm(ngm), shc(ngm)
      integer :: ibv 
     ...
     &     thets, thetm, shc )
      ...
      SoilSat = 0.d0     
      do ibv=1,2
         if (fbv(ibv) > 0.d0) then 
          call heat_to_temperature( t, fice,
     &     ht_ij(1,ibv,i,j), w_ij(1,ibv,i,j), shc(1))
          SoilTemp = SoilTemp + fbv(ibv)*t
          SoilSat =  SoilSat + fbv(ibv)*
     &        w_ij(1,ibv,i,j) / (thets(1)*dz_ij(i,j,1))
         endif
      enddo

      end subroutine get_soil_temperature_sat
