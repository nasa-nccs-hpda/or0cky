...
!@auth Jan Perlwitz
    ...
    (defined TRACERS_AMP) || (defined TRACERS_TOMAS)
     ...
     &     tracers_tomas
      ...
      use domain_decomp_atm, only: am_i_root, dread_parallel
     ...
     &     write_dist_data, variable_exists
...
c**** read in rundeck parameters
      ...
      call sync_param( 'wsubtke_dust_on', wsubtke_dust_on )
      call sync_param( 'wsubwd_dust_on', wsubwd_dust_on )
      call sync_param( 'wsubwm_dust_on', wsubwm_dust_on )
...
!@auth jan perlwitz
     ...
     &         j_0:j_1 ), 0.d0 )
        ...
        do n = 1,Ntm_dust
          ...
          if ( tracers_amp .or. tracers_tomas ) then
            ...
            nn = 0
            do n1 = 1,nDustBins
              if ( dustBinNamesIn( n1 ) == dust_names( n ) ) nn = n1
            end do
            if ( nn == 0 ) call stop_model
     &           (' Error in read_mineralfractions_netcdf:' //
     &           'No matching bin name found', 255 )
          else
     ...
     &           // 'why did it get here?', 255 )
          ...
          do m = 1,nMinerals
            ...
            varName='frac' // trim( dustBinNamesIn( nn ) ) //
     &           trim( mineralNames( m ) )

            if ( am_i_root()) write( 6, * ) '  read mineral fraction: ',
     &           trim( varName )

            is_mineralfraction = variable_exists( grid, fid,
     &           trim( varName ) )

            if ( .not. is_mineralfraction ) call stop_model
     &           ( 'Read error: ' // trim( varName ) //
     &           ' is not in input file. ', 255 )

            call read_dist_data( grid, fid, trim( varName ), work )

            mineralFractions( i_0:i_1, j_0:j_1, n ) = mineralFractions(
     &           i_0:i_1, j_0:j_1, n ) + max( work( i_0:i_1, j_0:j_1 ),
     &           0.d0 )
