#include "rundeck_opts.h"
      ...
      public init_ATMDYN, DYNAM, FILTER, COMPUTE_DYNAM_AIJ_DIAGNOSTICS, AFLUX, COMPUTE_MASS_FLUX_DIAGS, SDRAG,shap1,shap1d &
     &     ,fcuva,fcuvb

!**** Variables used in DIAG5 calculations
...
!@+       1 global patch with mode=1 is assumed.
      ...
      integer, dimension(:,:), allocatable :: ipatch_aflux_topo_adj,jpatch_aflux_topo_adj
...
!**** NIdyn=dtsrc/dt(dyn) has to be a multiple of 2
      NIdyn = 2*nint(.5*dtsrc/dt)
      if (is_set_param("DT") .and. nint(DTsrc/dt).ne.NIdyn) then
        if (AM_I_ROOT())  write(6,*) 'DT=',DT,' has to be changed to',DTsrc/NIdyn
...
!      call setDtParam('dt', dt, dtSrcUsed)
      ...
      if(is_set_param('mincolmass'))  call get_param('mincolmass',mincolmass)
      if(is_set_param('maxcolmass'))  call get_param('maxcolmass',maxcolmass)
...
!**** Determine if FLTRUV is called.
      QUVfilter = .false.
      if (DT_XUfilter>0. .or. DT_XVfilter>0. .or. DT_YUfilter>0. .or. DT_YVfilter>0.)  QUVfilter = .true.
      if (QUVfilter) then
...
! Warn if polar fixes requested for a model not having a half polar box
!     if(do_polefix.eq.1 .and. jm.ne.46) then
!        do_polefix = 0
!        write(6,*) 'Polar fixes are currently applicable only to models having a half polar box; no fixes applied'
!    endif
      ...
      if(is_set_param('i1_aflux_topo_adj')) then
        ...
        call get_param('md_aflux_topo_adj',md_aflux_topo_adj, npatch_aflux_topo_adj)
      else ! use defaults
...
!@auth Original development team
      ...
      Use DYNAMICS,   Only: MU,MV,MW, pu,pv,sd,dut,dvt,cos_limit,nidyn,dt,mrch,nstep,quvfilter,USE_UNR_DRAG
      Use DIAG_COM,   Only: MODD5K,NDA5K,NDAA
      USE DOMAIN_DECOMP_ATM, only: grid
      Use DOMAIN_DECOMP_1D,  Only: GetDomainBounds, GLOBALSUM, SOUTH, HALO_UPDATE, HALO_UPDATE_COLUMN
      USE MOMENTS, only : advecv
      IMPLICIT NONE
!**** Local variables
      Integer :: NLFAtm = 8  !  = twice number of even leap frog steps before restarting with forward-backward steps
      Real*8,Dimension (LM, IM, GRID%J_STRT_HALO:GRID%J_STOP_HALO) :: MEVEN,MODD1,MODD3
      Real*8,Dimension (IM,grid%J_STRT_HALO:grid%J_STOP_HALO) :: MSUMODD, FPEU,FPEV, AM1,AM2
...
!**** Leap-frog re-initialization: IF (NS.LT.NIdyn)
  300 CONTINUE
...
!**** ACCUMULATE MASS FLUXES FOR TRACERS and Q
      MUs(:,:,:) = MUs(:,:,:) + PU(:,:,:)
      MVs(:,:,:) = MVs(:,:,:) + PV(:,:,:)
...
!**** Check for end of Leap-Frog time scheme
      If (NS <= 1)  GoTo 400
      If (Mod(NIDyn-NS,NLFAtm) < NLFAtm-1)  GoTo 340
      NS = NS - 1  ;  GoTo 300

  400 If (MODDA >= 2)  Call MAtoPMB
...
! apply east-west filter to U and V once per physics timestep
      Call FLTRUV
! apply north-south filter to U and V once per physics timestep
...
!     Call HALO_UPDATE (GRID, GZ, From=SOUTH)   haloed in PGF
      ...
      DO L=1,LM
         I=IM
         DO IP1=1,IM
            AIJ(I,J,IJ_FGZU) = AIJ(I,J,IJ_FGZU) + (GZ(I,J,L)+GZ(IP1,J,L))*MU(I,J,L)*DT ! use DT=DTLF/2
            I=IP1
         END DO
      END DO
      END DO
      ...
      DO L=1,LM
         DO I=1,IM
            AIJ(I,J,IJ_FGZV) = AIJ(I,J,IJ_FGZV) + (GZ(I,J-1,L)+GZ(I,J,L))*MV(I,J,L)*DT ! use DT=DTLF/2
         END DO
      END DO
      END DO
      ...
      use DIAG_COM, only: AIJ => AIJ_loc, IJ_FGZU, IJ_FGZV, IJ_FMV, IJ_FMU
      use RESOLUTION, only: IM,JM,LM
...
!!@auth Original development team
      ...
      Use DOMAIN_DECOMP_ATM, Only: GRID
      Use DOMAIN_DECOMP_1D,  Only: GetDomainBounds, SOUTH,NORTH, HALO_UPDATE
      IMPLICIT NONE
...
!**** Local variables
      ...
      Real*8  :: DUMMYS(IM),MUS,MVS,PBS,MVSA(LM), zNSxDT, &
                 DUMMYN(IM),MUN,MVN,PBN,MVNA(LM), USV0(IM,2,LM),VSV0(IM,2,LM), M,CONVs,MVARs
      real*8 :: mdn,mup,xx
...
!**** Compute MU (kg/s) = eastward mass flux at non-polar points
      Do 2166 L=1,LM
      Do 2154 J=J1XP,JNXP
      DO 2154 I=1,IM
 ...
 2165 I=IP1
 2166 CONTINUE
!**** Compute MV (kg/s) = northward mass flux
      Do 2172 L=1,LM
      IM1=IM
      DO 2172 J=J1V,JNV
      DO 2170 I=1,IM
      MV(I,J,L) = .25*DXV(J)*(V(I,J,L)+V(Im1,J,L)) * (MA(L,I,J)+MA(L,I,J-1))
 2170 IM1=I
 2172 CONTINUE
...
! To maintain consistency with subroutine ADVECV, adjust pu at the pole if no corner fluxes are used there in ADVECV.
         If (QSP)  MU(:,1 ,:) = MU(:,1 ,:)*TWOby3
         If (QNP)  MU(:,JM,:) = MU(:,JM,:)*TWOby3  ;  EndIf

      if(aflux_topo_adjustments) then
!
! modify uphill air mass fluxes around topography
!
      do adjmode=0,1
...
!**** Compute MW (kg/s) = downward vertical mass flux
      DO 2435 J=J1,JN
      Do 2435 I=1,IMAXJ(J)
         CONVs = Sum(CONV(I,J,:))
         MVARs = MESUM(I,J)
         ...
         MW(I,J,LM-1) = CONV(I,J,LM) - CONVs*MFRAC(LM) + (ME(LM,I,J) - (MFIX(LM)+MVARs*MFRAC(LM)) )*DXYP(J)*zNSxDT
         do L=LM-2,1,-1
           MW(I,J,L) = MW(I,J,L+1) + CONV(I,J,L+1) - CONVs*MFRAC(L+1) + (ME(L+1,I,J) - (MFIX(L+1)+MVARs*MFRAC(L+1)) )*DXYP(J)*zNSxDT
         enddo
 2435 CONTINUE
      Do L=1,LM-1
...
!@auth Original development team
      ...
      USE DOMAIN_DECOMP_ATM, only: grid
      Use DOMAIN_DECOMP_1D,  Only: GetDomainBounds, HALO_UPDATE_COLUMN, HALO_UPDATE, GLOBALMAX, NORTH,SOUTH
      IMPLICIT NONE
      Real*8,Intent(In) ::  MOLD(LM,IM,GRID%J_STRT_HALO:GRID%J_STOP_HALO), DT1
...
!**** Local variables
      ...
      Call GetDomainBounds (GRID, J_STRT = J1, J_STOP = JN, &  !  primary row limits
         HAVE_SOUTH_POLE = QSP, HAVE_NORTH_POLE = QNP)
...
!**** Compute MNEW and MSUM, the new atmospheric mass distribution
      ...
      Do J=J1,JN
      Do I=1,IMAXJ(J)
         MNEW(LM,I,J) = MOLD(LM,I,J) + DT1*(CONV(I,J,LM) - MW(I,J,LM-1))*byDXYP(J)
         MSUM(I,J) = MNEW(LM,I,J)
         Do L=LM-1,2,-1
            MNEW(L,I,J) = MOLD(L,I,J) + DT1*(CONV(I,J,L) + MW(I,J,L) - MW(I,J,L-1))*byDXYP(J)
            MSUM(I,J) = MSUM(I,J) + MNEW(L,I,J)  ;  EndDo
         MNEW(1,I,J) = MOLD(1,I,J) + DT1*(CONV(I,J,1) + MW(I,J,1))*byDXYP(J)
         MSUM(I,J) = MSUM(I,J) + MNEW(1,I,J)
         if(MSUM(I,J)+MTOP > MAXCOLMASS .or. MSUM(I,J)+MTOP < MINCOLMASS) then
           n_exception = 1
           if(MSUM(I,J)+MTOP > MAXCOLMASS*(1200d0/1160d0) .or. MSUM(I,J)+MTOP < MINCOLMASS*(250d0/350d0)) then
          ...
          if(MSUM(I,J)+MTOP > MAXCOLMASS .or. MSUM(I,J)+MTOP < MINCOLMASS) then
            ...
            Write (6,990) I,J,MRCH,ZATMO(I,J),DT1, &
                 (L,U(IM1,J,L),U(I,J,L),U(IM1,J+1,L),U(I,J+1,L), &
                    ...
                    MNEW(L,I,J),MOLD(L,I,J),T(I,J,L),Q(I,J,L)*1000, L=LM,1,-1)
            Write (6,*) "Pressure diagnostic error"
          endif
        EndDo  ;  EndDo
      endif

      if(n_exception_all==2)  call stop_model('ADVECM: Mass diagnostic error',11)
      ...
      Return
  990 Format (// 'PRESSURE DIAGNOSTIC  I,J,MRCH,ZATMO,DT=',3I4,2F10.2 / &
      ...
      Subroutine PGF (DT1, U,V,MAM, UT,VT,MAFTER, S0,mSZ)
...
!****     S0,mSZ = potential temperature and minus half of vertical gradient (K)
!****     MAFTER = mass distribution at end of time step (kg/m^2)
!**** Output: UT,VT = velocity updated by pressure gradient force (m/s)

!**** G  (m/s^2)   =  uniform downward acceleration due to gravity  =  GRAV
!**** M  (kg/m^2)  =  variable vertical coordinate  =  air mass above the level
!**** M0           =  mean vertical coordinate of layer  =  (Md + Mu) / 2
!**** DM (kg/m^2)  =  layer air mass difference  =  Md - Mu  =  MAM
!**** Q0 (kg/kg)   =  mean water vapor mixing ratio of layer  =  Q
!**** QZ (kg/kg)   =  downward vertical gradient of water vapor mixing ratio of layer  =  - 2*QMOM(MZ)
!**** QC (kg/kg)   =  mean water concentration mixing ratio of layer  =  QCL + QCI
!**** Q  (kg/kg)   =  variable water vapor mixing ratio  =  Q0 + QZ*(M-M0)/DM  =  Q0 + QZ*(P-P0)/DP
!**** RD (J/kg*C)  =  dry air gas constant  =  RGAS
!**** RV (J/kg*C)  =  water vapor gas constant  =  RVAP
!**** R (J/kg*C)   =  gas constant  =  RD + (RV-RD)*Q  =  RD + (RV-RD)*Q0 + (RV-RD)*QZ*(P-P0)/DP  =
!****              =  RD+(RV-RD)*(Q0-QZ*P0/DP) + (RV-RD)*QZ*P/DP  =  Ro + Rz*P
!**** C (J/kg*C)   =  specific heat capacity  =  SHC + (SHV-SHC)*Q
!**** K            =  uniform exponent of exner function  =  R/C  =  KAPA
!**** K1, K2, K3   =  K+1, K+2, K+3
!**** P  (Pa)      =  pressure = M*G
!**** P100 (Pa)    =  initial reference pressure for potential temperature  =  100 (Pa)
!**** P0 (Pa)      =  mean pressure of layer  =  G*M0  =  (Pd + Pu) / 2
!**** PK (1)       =  exner function of P referenced to 1 (Pa)  =  (P/P1)^K  =  P^K  =  PK
!**** DP (Pa)      =  layer pressure difference  =  Pd - Pu  =  G*DM
!**** S0 (K)       =  mean potential temperture of layer referenced to 1 (Pa)  =  S0 / P100^K
!**** SZ (K)       =  downward vertical gradient of layer referenced to 1 (Pa)  =  - 2*mSZ / P100^K
!**** S  (K)       =  variable potential temperature referenced to 1 (Pa)  =  S0 + SZ*(M-M0)/DM  =  S0 + SZ*(P-P0)/DP  =
!****              =  S0-SZ*P0/DP + SZ*P/DP  =  So + Sz*P
!**** T  (K)       =  temperature  =  S*(P/P1)^K  =  S*P^K  =  So*P^K + Sz*P*P^K  =  So*P^K + Sz*P^K1
!**** AG (m^3/kg)  =  gaseous specific volume  =  R*T/P  =  (Ro + Rz*P)*(So*P^K + Sz*P^K1) / P  =
!****              =  Ro*So*P^(K-1) + (Ro*Sz + Rz*So)*P^K + Rz*Sz*P^K1
!**** A  (m^3/kg)  =  specific volume  =  (1 - QC) * [Ro*So*P^(K-1) + (Ro*Sz + Rz*So)*P^K + Rz*Sz*P^K1]  =
!****              =  Am*P^(K-1) + Ao*P^K + Ap*P^K1
!**** Z  (m)       =  variable altitude measured from mean sea level
!**** GZ (m^2/s^2) =  gravity times variable altitude measured from mean sea level  =  G*Z

!**** Compute DZ  =  altitude thickness as a function of P inside a layer measured from Pd to P:
!**** DZ  =  - Int[A dP] / G  =  - Int[Am*P^(K-1) + Ao*P^K + Ap*P^K1] dP / G  =
!****     =  [Am(Pd^K - P^K)/K + Ao(Pd^K1 - P^K1)/K1 + Ap[Pd^K2 - P^K2)/K2] / G

!**** Compute DZu  =  altitude thickness as a function of P in an entire layer measured from Pd to Pu:
!**** DZu  =  [Am(Pd^K - Pu^K)/K + Ao(Pd^K1 - Pu^K1)/K1 + Ap(Pd^K2 - Pu^K2)/K2] / G

!**** Compute mass weighted value DZmean in an entire layer from Mu to Md or from Pu to Pd:
!**** DZmean  =  Int[DZ dM] / DM  =  Int[DZ dP] / DP  =
!****         =  Int{[Am(Pd^K - P^K)/K + Ao(Pd^K1 - P^K1)/K1 + Ap(Pd^K2 - P^K2)/K2] dP} / G*DP  =
!****         =  [Am*Pd^K/K + Ao*Pd^K1/K1 + Ap*Pd^K2/K2] / G - Int{[Am*P^K/K + Ao*P^K1/K1 + Ap*P^K2/K2] dP} / G*DP  =
!****         =  [Am*Pd^K/K + Ao*Pd^K1/K1 + Ap*Pd^K2/K2] / G -
!****            - [Am(Pd^K1 - Pu^K1)/K*K1 + Ao(Pd^K2 - Pu^K2)/K1*K2 + Ap(Pd^K3 - Pu^K3)/K2*K3] / G*DP

!**** Zmean(L) = ZATMO/G + Sum[DZup(1:L-1)] + DZmean(L)

!**** Compute mass weighted value Amean in layer from Mu to Md or from Pu to Pd:
!**** Amean  =  Int[A dM] / DM  =  Int[A dP] / DP  =  Int{[Am*P^(K-1) + Ao*P^K + Ap*P^K1] dP} / DP  =
!****        =  [Am(Pd^K - Pu^K)/K + Ao(Pd^K1 - Pu^K1)/K1+ Ap(Pd^K2 - Pu^K2)/K2] / DP
!**** Amean*DM  =  AdM  =  [Am(Pd^K - Pu^K)/K + Ao(Pd^K1 - Pu^K1)/K1+ Ap(Pd^K2 - Pu^K2)/K2] / G  =  DZu

!**** Acceleration of velocity by pressure gradient force (m/s^2) =  Amean dP/dX + d(G*Zmean)/dX
!**** PGFU (kg*m/s^2)  =  [Amean dP/dX + d(G*Zmean)/dX] * DM * DX*DY  =  [Aver(Amean*DM) dP + Aver(DM) d(G*Zmean)] * DY
!**** DUT (kg*m/s) = dTIME * PolarFiltered(PGFU)

      Use CONSTANT,   Only: GRAV,RGAS,RVAP,KAPA,byGRAV, yK=>byKAPA, yK1=>byKAPAp1, yK2=>byKAPAp2
      Use RESOLUTION, Only: IM,JM,LM, MTOP
      Use ATM_COM,    Only: ZATMO, Q,QCL,QCI
      Use SOMTQ_COM,  Only: MZ,QMOM
      ...
      IMPLICIT NONE
      Real*8 :: DT1
      Real*8,Dimension(IM,grid%J_STRT_HALO:grid%J_STOP_HALO,LM) :: U,V, UT,VT, S0,mSZ
      Real*8,Dimension(LM,IM,grid%J_STRT_HALO:grid%J_STOP_HALO) :: MAM,MAFTER
!**** Local variables
      Real*8,Dimension(IM,GRID%J_STRT_HALO:GRID%J_STOP_HALO,LM) :: P,PGFU
      Real*8 :: DT4, GDZu(LM),GDZmean(LM), yP100K,yK3, Pu,PKu,PKPu,PKPPu,PKPPPu, Pd,PKd,PKPd,PKPPd,PKPPPd, DP,yDP,P0, Q0,QZ,QC, &
                Ro,Rz, So,Sz, Am,Ao,Ap, GZd, FACTOR,FLUX, VMASS
...
!**** Integrate pressures from the top down
         Pu = MTOP*GRAV  ;  PKu = Pu**KAPA  ;  PKPu = PKu*Pu  ;  PKPPu = PKPu*Pu  ;  PKPPPu = PKPPu*Pu
         Do L=LM,1,-1
            DP = MAM(L,I,J)*GRAV  ;  yDP = 1 / DP
            Pd = Pu + DP  ;  PKd = Pd**KAPA  ;  PKPd = PKd*Pd  ;  PKPPd = PKPd*Pd  ;  PKPPPd = PKPPd*Pd
            P0 = .5*(Pu + Pd)
            Q0 = Q(I,J,L)  ;  QZ = - 2 * QMOM(MZ,I,J,L)    ;  QC = QCL(I,J,L) + QCI(I,J,L)
!!!         Q0 = 0         ;  QZ = 0                       ;  QC = 0
            Ro = RGAS + (RVAP - RGAS) * (Q0 - QZ*P0*yDP)   ;  Rz = (RVAP - RGAS) * QZ * yDP
            So = (S0(I,J,L) + 2*mSZ(I,J,L)*P0*yDP)*yP100K  ;  Sz = - 2*mSZ(I,J,L)*yDP*yP100K
            Am = (1 - QC) * Ro*So  ;  Ao = (1 - QC) * (Ro*Sz + Rz*So)  ;  Ap = (1 - QC) * Rz*Sz
            GDZu(L) = Am*(PKd-PKu)*yK + Ao*(PKPd-PKPu)*yK1 + Ap*(PKPPd-PKPPu)*yK2
            GDZmean(L) = Am*PKd*yK + Ao*PKPd*yK1 + Ap*PKPPd*yK2 &
                         - (Am*(PKPd-PKPu)*yK*yK1 + Ao*(PKPPd-PKPPu)*yK1*yK2 + Ap*(PKPPPd-PKPPPu)*yK2*yK3) * yDP
            AdM(I,J,L) = GDZu(L) * byGRAV
              P(I,J,L) = P0
            Pu = Pd  ;  PKu = PKd  ;  PKPu = PKPd  ;  PKPPu = PKPPd  ;  PKPPPu = PKPPPd  ;  EndDo

!**** Integrate altitude from the bottom up
         GZd = ZATMO(I,J)
         Do L=1,LM
            GZ(I,J,L) = GZd + GDZmean(L)
            GZd = GZd + GDZu(L)  ;  EndDo  ;  EndDo  ;  EndDo
...
!
      ...
      IM1=IM
      DO 3234 I=1,IM
      FLUX = ((AdM(I,J,L)+AdM(I,J-1,L))*(P(I,J,L)- P(I,J-1,L)) + (MAM(L,I,J)+MAM(L,I,J-1))*(GZ(I,J,L)-GZ(I,J-1,L))) * FACTOR
...
!
      DO L=1,LM
        ...
        DO J=Max(2,J_0STG-1),J_1STG
          DO IP1=1,IM
            PGFU(I,J,L) = (AdM(Ip1,J,L)+AdM(I,J,L))*( P(Ip1,J,L)- P(I,J,L)) + (MAM(L,Ip1,J)+MAM(L,I,J))*(GZ(Ip1,J,L)-GZ(I,J,L))
            I=IP1
          END DO
        END DO
...
! correct for erroneous dxyv at the poles
      if(do_polefix.eq.1) then
         do ipole=1,2
...
!****
      ...
      DO 3525 IP1=1,IM
      Do L=1,LM
         VMASS = .5*((MAFTER(L,I,J-1)+MAFTER(L,Ip1,J-1))*DXYN(J-1) + (MAFTER(L,I,J)+MAFTER(L,Ip1,J))*DXYS(J))
...
!**** THIS VERSION OF AVRX DOES SO BY TRUNCATING THE FOURIER SERIES.
      USE DOMAIN_DECOMP_ATM, only: grid
      USE DOMAIN_DECOMP_1D, Only : getDomainBounds
      USE MOMENTS, only : moment_enq_order
      USE constant, only : byrt2
      IMPLICIT NONE
      REAL*8, INTENT(INOUT), optional :: X(IM,grid%J_STRT_HALO:grid%J_STOP_HALO)
      Integer, Intent(In), optional :: jrange(2)
      REAL*8, ALLOCATABLE, SAVE  :: DRAT(:)
      REAL*8, SAVE ::  BYSN(IMH)
      REAL*8, DIMENSION(0:IMH) :: AN,BN
!!!   INTEGER, SAVE :: NMIN(grid%J_STRT_HALO:grid%J_STOP_HALO)
...
!@calls SHAP1D
!****
!**** MFILTR=1  SMOOTH P USING SEA LEVEL PRESSURE FILTER
!****        2  SMOOTH T USING TROPOSPHERIC STRATIFICATION OF TEMPER
!****        3  SMOOTH P AND T
!****
      ...
      Real*8,Dimension(IM,grid%J_STRT_HALO:grid%J_STOP_HALO) :: PEDNOLD,X,Y
      Real*8,Dimension(LM,IM,GRID%J_STRT_HALO:GRID%J_STOP_HALO) :: MABEF,PKOLD
...
!****
!**** SEA LEVEL PRESSURE FILTER ON P
!****
      Do J=J1P,JNP
        DO I=1,IM
          ZS=ZATMO(I,J)*BYGRAV
...
!**** reduce large variations (mainly due to topography)

#ifdef V2_PSURF_FILTER /* only for comparing to V2 runs */
          PEDN(1,I,J) = ptop + Max (PEDN(1,I,J)-ptop, 0.99d0*(PEDNOLD(I,J)-ptop))
          PEDN(1,I,J) = ptop + Min (PEDN(1,I,J)-ptop, 1.01d0*(PEDNOLD(I,J)-ptop))
#else
...
!**** Conserve column mass for present J latitude row
      ...
      END DO
        ...
        do j=j_0s,j_1s
          ...
          enddo
          do i=1,im-1
            pgfx(i) = PEDNOLD(I+1,J) - PEDNOLD(I,J) + .5*(rhosrf(i+1)+rhosrf(i))*(zatmo(i+1,j)-zatmo(i,j))
          enddo
          i=im
             pgfx(i) = PEDNOLD(1,J) - PEDNOLD(I,J) + .5*(rhosrf(1)+rhosrf(i))*(zatmo(1,j)-zatmo(i,j))
...
!**** Compute new MA from filtered PEDN(1) array
      Do J=J1P,JNP  ;  Do I=1,IM
         ...
         MA(LMFRAC1:LMFRACM,I,J) = MFIX(LMFRAC1:LMFRACM) + MVAR*MFRAC(LMFRAC1:LMFRACM)
         EndDo  ;  EndDo
      Call MAtoPMB

!**** Scale mixing ratios (incl moments) to conserve mass/heat
      DO L=LMFRAC1,LMFRACM
      DO J=J_0S,J_1S
      DO I=1,IM
         zMRAT = MABEF(L,I,J) / MA(L,I,J)
! adjust pot. temp. to maintain unchanged absolute temp.
...
#ifdef TRACERS_ON
!**** In general, only an air tracer is affected by the filter
!**** This fix conserves tracer concentration, BUT NOT MASS!
!****   It is not wanted for most tracers.
!**** Thus, this code slows model and should be removed if not wanted
!**** Instead of if(trname...) could use n=n_air and avoid the IF-test
!**** But if n_air=0 this will cause problems...
      do n=1,ntm
...
!****
!**** TEMPERATURE STRATIFICATION FILTER ON T
!****
...
!@auth Original development team
      ...
      USE DYNAMICS, only : dt,mrch,ang_uv, COS_LIMIT,do_polefix,DT_XUfilter,DT_XVfilter,DT_YVfilter,DT_YUfilter
!**********************************************************************
!**** FILTERING IS DONE IN X-DIRECTION WITH A 8TH ORDER SHAPIRO
!**** FILTER. THE EFFECT OF THE FILTER IS THAT OF DISSIPATION AT
!**** THE SMALLEST SCALES.
!**********************************************************************
      USE DOMAIN_DECOMP_ATM, only: grid
      USE DOMAIN_DECOMP_1D, only : getDomainBounds
      IMPLICIT NONE
      REAL*8, DIMENSION(IM,grid%J_STRT_HALO:grid%J_STOP_HALO,LM) :: DUT,DVT,USAVE,VSAVE
...
!****
!**** Filtering in east-west direction
!****
      DO 350 L=1,LM
!**** Filter U component of velocity
...
!**** Filter V component of velocity
...
! This routine is now called by subroutine DYNAM
!      if(do_polefix.eq.1) then
!         call isotropuv(u,v,COS_LIMIT)
!      endif

!**** Conserve angular momentum along latitudes
      DO L=1,LM
        DO J=J_0STG,J_1STG
          ANGM=0.
          MMUVs = 0
          I=IM
          DO IP1=1,IM
             MMUV(I) = .5*((MA(L,Ip1,J-1)+MA(L,I,J-1))*DXYN(J-1) + (MA(L,Ip1,J  )+MA(L,I,J  ))*DXYS(J))
             MMUVs = MMUVs + MMUV(I)
...
!@auth M. Kelley
      ...
      REAL*8, DIMENSION(IM,grid%J_STRT_HALO:grid%J_STOP_HALO,LM) :: U, V
      real*8 :: coscut,fac,k
...
! convert xy velocities back to polar coordinates
          do i=1,im
...
!@var NORDER order of shapiro filter (must be even)
      ...
      REAL*8, INTENT(INOUT), DIMENSION(IM,grid%J_STRT_HALO:grid%J_STOP_HALO) :: X
...
!@auth Original Development Team
      ...
      USE GEOM, only : cosv,imaxj,kmaxj,idij,idjj,rapj,dxyv,dxyn,dxys,rapvs,rapvn
      USE DIAG_COM, only : ajl=>ajl_loc,jl_dudtsdrg
      USE DYNAMICS, only : x_sdrag,csdragl,lsdrag,lpsdrag,ang_sdrag,Wc_Jdrag,wmax,vsdragl
...
!**** SDRAG_CONST is applied in stratosphere below the SDRAG_LIN
!**** regime (but not above P_CSDRAG)
      Real*8 :: X,MAUV,WL,TL,RHO,CDN,MMUV(LM),DU
!@var DUT,DVT change in momentum (kg*m/s)
      REAL*8, DIMENSION(IM,grid%J_STRT_HALO:grid%J_STOP_HALO,LM) :: DUT,DVT
      INTEGER I,J,L,IP1,K,Lmax
      logical cd_lin
!@var ang_mom is the sum of angular momentun at layers LS1 to LM
      REAL*8, DIMENSION(IM,grid%J_STRT_HALO:grid%J_STOP_HALO)    :: ANG_MOM, SUM_MMUV
...
!**** Extract domain decomposition info
      ...
      if(linear_sdrag) then
        ...
        i=im
        do ip1=1,im
           MAUV = (MA(L,Ip1,J-1)+MA(L,I,J-1))*RAPVN(J-1) + (MA(L,Ip1,J  )+MA(L,I,J  ))*RAPVS(J)
!**** adjust diags for possible difference between dt1 and dtsrc
!        call inc_ajl(i,j,l,jl_dudtsdrg,-u(i,j,l)*x) ! for a-grid only
      ...
      DO J=J_0STG, J_1STG
      cd_lin=.false.
      IF( L.ge.LSDRAG .or. (L.ge.LPSDRAG.and.COSV(J).LE..15) ) cd_lin=.true.
      wmaxj=wmax
      if(COSV(J).LE..15) wmaxj=wmaxp
      I=IM
      DO IP1=1,IM
        TL=T(I,J,L)*PK(L,I,J)   ! not quite correct - should be on UV grid
!**** check T to make sure it stayed within physical bounds
        if (TL.lt.100..or.TL.gt.373.) then
          write(99,'(a,i8,3i4,a,3f10.2)') ' SDRAG:',itime,i,j,l,'  T,U,V=',TL,U(I,J,L),V(I,J,L)
...
!**** WL is restricted to Wmax by adjusting X, if necessary;
!**** the following is equivalent to first reducing (U,V), if necessary,
!**** then finding the drag and applying it to the reduced winds
                    CDN=CSDRAGl(l)*xjud
...
!*
!***  Add the lost angular momentum uniformly back in if ang_sdrag>0
!***  only below 150mb if ang_sdrag=1, into whole column if ang_sdrag>1
!*
      if (ang_sdrag.gt.0) then
        ...
        do ip1 = 1,im
          do l = 1,lmax
            MMUV(L) = .5*((MA(L,Ip1,J-1)+MA(L,I,J-1))*DXYN(J-1) + (MA(L,Ip1,J  )+MA(L,I,J  ))*DXYS(J))
...
!            call inc_ajl(i,j,l,JL_DUDTSDRG,du) ! for a-grid only
...
!**** (technically we should use U,V from before but this is ok)
...
!**** Angular Momentum on B grid (kg m^2/s)
      Do J=J1V,JN
         I=IM
         Do Ip1=1,IM
            AM(I,J) = Sum (((MA(:,I,J-1) + MA(:,Ip1,J-1))*DXYN(J-1) + (MA(:,I,J  ) + MA(:,Ip1,J  ))*DXYS(J)) * &
...
!**** Kinetic Energy on B grid (J)
      Do J=J1V,JN
         I=IM
         Do Ip1=1,IM
            RKE(I,J) = Sum (((MA(:,I,J-1) + MA(:,Ip1,J-1))*DXYN(J-1) + (MA(:,I,J  ) + MA(:,Ip1,J  ))*DXYS(J)) * &
                            (U(I,J,:)**2 + V(I,J,:)**2)) * .25
            I=Ip1  ;  EndDo  ;  EndDo
...
!     non polar boxes
!**** Update halos of u and v. (Needed bcs. IDJJ(3:4,J_1S)=J_1S+1)
...
!****
      ...
      implicit none
      real*8, dimension(im,grid%j_strt_halo:grid%j_stop_halo)  :: u_a,v_a
      real*8, dimension(2*im*(1+grid%j_stop_stgr-grid%j_strt_stgr)), intent(out) :: uv1d
      real*8 :: hemi
      integer :: i,ip1,j,n
      ...
      implicit none
      integer :: k
      REAL*8, DIMENSION(k,LM,IM,GRID%J_STRT_HALO:GRID%J_STOP_HALO) :: UR,VR
      real*8, dimension(im,lm) :: ursp,vrsp,urnp,vrnp
      integer :: i,j,l
      integer :: J_0S,J_1S
      if(k.ne.4)  call stop_model('incorrect k in replicate_uv_to_agrid',255)
      ...
      subroutine avg_replicated_duv_to_vgrid(du,dv,k, dusp,dvsp,dunp,dvnp)
      use resolution, only : im,jm,lm
      USE ATM_COM, only : u,v
      USE DOMAIN_DECOMP_ATM, only: grid
      USE DOMAIN_DECOMP_1D, only : HALO_UPDATE_BLOCK,SOUTH, hasSouthPole, hasNorthPole
      implicit none
      integer :: k
      REAL*8, DIMENSION(k,LM,IM,GRID%J_STRT_HALO:GRID%J_STOP_HALO) :: DU,DV
      real*8, dimension(im,lm) :: dusp,dvsp,dunp,dvnp
      integer :: i,j,l
      integer :: J_0STG,J_1STG
      if(k.ne.4) call stop_model ('incorrect k in avg_replicated_duv_to_vgrid',255)
      J_0STG = GRID%J_STRT_STGR
      J_1STG = GRID%J_STOP_STGR
      CALL HALO_UPDATE_BLOCK(GRID,DU,FROM=SOUTH)
      CALL HALO_UPDATE_BLOCK(GRID,DV,FROM=SOUTH)
!
! copy circumpolar data into the appropriate spots in du,dv
!
      if(hasSouthPole(grid)) then
        j=1
        do i=2,im
...
! compensate for the factor of 2 in ravj(1).  change ravj(1) later.
        du(3:4,:,:,j) = du(3:4,:,:,j)*.5
        dv(3:4,:,:,j) = dv(3:4,:,:,j)*.5
#endif
...
#ifndef ALT_CLDMIX_UV
! compensate for the factor of 2 in ravj(jm).  change ravj(jm) later.
        du(1:2,:,:,j) = du(1:2,:,:,j)*.5
        dv(1:2,:,:,j) = dv(1:2,:,:,j)*.5
#endif
...
!
      ...
      USE DOMAIN_DECOMP_ATM, only: grid
      USE DOMAIN_DECOMP_1D, only : HALO_UPDATE
      USE DOMAIN_DECOMP_1D, only : NORTH, hasSouthPole, hasNorthPole
      USE GEOM, only : byim
      IMPLICIT NONE
      ...
      DO I=1,IM
        XIJ = x(i,j,l)
        X(I,J,L) = .25*(XIM1J+XIJ+x(im1,j+1,l)+x(i,j+1,l))
        XIM1J = XIJ
        IM1=I
      ENDDO
      ENDDO
...
! regrids scalar x_bgrid*dxyv -> x_agrid*dxyp
      USE RESOLUTION, only : im,jm
      USE GEOM, only : rapvs,rapvn,dxyp,dxyv
      USE DOMAIN_DECOMP_ATM, only: grid
      USE DOMAIN_DECOMP_1D, only : getDomainBounds, HALO_UPDATE, NORTH, hasSouthPole, hasNorthPole
      USE GEOM, only : byim
      IMPLICIT NONE
      ...
      DO I=1,IM
        XIJ = X(I,J)
!        X(I,J) = .25*(XIM1J+X(I,J)+X(IM1,J+1)+X(I,J+1))
        X(I,J) = (XIM1J+X(I,J))*RAPVS(J) + (X(IM1,J+1)+X(I,J+1))*RAPVN(J)
        XIM1J = XIJ
        IM1 = I
      ENDDO
      ENDDO
...
!@auth Gary Russell
      ...
      IMPLICIT NONE
!****
!**** THE PARAMETER M INDICATES WHEN DIAGCD IS BEING CALLED
!**** M=1  AFTER ADVECTION IN DYNAMICS
!****   2  AFTER CORIOLIS FORCE IN DYNAMICS
!****   3  AFTER PRESSURE GRADIENT FORCE IN DYNAMICS
!****   4  AFTER STRATOS DRAG IN DYNAMICS
!****   5  AFTER FLTRUV IN DYNAMICS
!****   6  AFTER GRAVITY WAVE DRAG IN DYNAMICS
!****
...
!@var UX,VX current velocities
      REAL*8, INTENT(IN),DIMENSION(IM,GRID%J_STRT_HALO:GRID%J_STOP_HALO,LM) :: UX,VX
!@var DUT,DVT current momentum changes (kg*m/s)
      REAL*8, INTENT(IN),DIMENSION(IM,GRID%J_STRT_HALO:GRID%J_STOP_HALO,LM) :: DUT,DVT
      REAL*8, DIMENSION(GRID%J_STRT_HALO:GRID%J_STOP_HALO) :: PI,DAMB,DKEB
      INTEGER :: I,J,L,N,IP1
      LOGICAL dopit
      REAL*8 :: DUTI,DUTIL,RKEI,RKEIL,begin
      INTEGER, DIMENSION(6) :: NAMOFM=(/2,3,4,5,6,7/), NKEOFM=(/14,15,16,17,18,19/)
...
!****
      ...
      END IF
!****
!**** CHANGE OF ANGULAR MOMENTUM AND KINETIC ENERGY BY VARIOUS
...
!****
      ...
      DO J=J_0STG,J_1STG
        ...
        DO L=1,LM
          ...
          RKEIL=RKEIL+RKEI
        END DO
        if (dopit) DUTIL = DUTIL + 2*DT1*RADIUS*OMEGA*COSV(J) * (PI(J-1)*RAVPN(J-1)+PI(J)*RAVPS(J))
...
!
      ...
      USE RESOLUTION, only : jm
      USE DOMAIN_DECOMP_ATM, only: grid
      USE DOMAIN_DECOMP_1D, only : HALO_UPDATE, NORTH, hasSouthPole, hasNorthPole
      ...
      REAL*8, DIMENSION(IM,GRID%J_STRT_HALO:GRID%J_STOP_HALO,LM) :: DUT,DVT  !  (kg*m/s)
      ...
      REAL*8, DIMENSION(IMH+1,GRID%J_STRT_HALO:GRID%J_STOP_HALO,NSPHER) :: KE_part
      REAL*8 BEGIN
...
!****  810 WRITE (6,910) M5
  810 CALL WRITE_PARALLEL(M5, UNIT=6, format="('0INCORRECT VALUE OF M5 WHEN CALLING DIAG5D.  M5=',I5)")
!****  910 FORMAT ('0INCORRECT VALUE OF M5 WHEN CALLING DIAG5D.  M5=',I5)
      call stop_model('INCORRECT VALUE OF M5 WHEN CALLING DIAG5D',255)
!****
!**** KINETIC ENERGY
!****
!**** TRANSFER RATES FOR KINETIC ENERGY IN THE DYNAMICS
  100 CALL GETTIME(BEGIN)
      ...
      DO L=1,LM
        DO J=J_0STG,J_1STG
          ...
          DO KUV=1,2 ! loop over u,v
            ...
            DO N=1,NM
               X(N) = .5*FIM * (FA(N-1)*FCUVA(N-1,J,L,KUV)+FB(N-1)*FCUVB(N-1,J,L,KUV))
            ENDDO
            X(1)=X(1)+X(1)
            X(NM)=X(NM)+X(NM)
            IF (J.NE.JEQ)  KE_part(:,J,KSPHER) = KE_part(:,J,KSPHER) + X(:)*DSIG(L)
            ...
            ELSE IF (J.EQ.JEQ) THEN ! EQUATOR
              DO N=1,NM
                ...
                KE_part(N,J,KSPHER  ) = KE_part(N,J,KSPHER  ) + .5*X(N)*DSIG(L)       ! CONTRIB TO SH
                KE_part(N,J,KSPHER+1) = KE_part(N,J,KSPHER+1) + .5*X(N)*DSIG(L)       ! CONTRIB TO NH
              ENDDO
      ...
      CALL GLOBALSUM(grid, KE_part(1:NM,:,1:NSPHER), KE(1:NM,1:NSPHER), ALL=.TRUE.)
...
!**** FOURIER COEFFICIENTS FOR CURRENT WIND FIELD
!****
      use resolution, only : im,jm,lm
      ...
      REAL*8, DIMENSION(IM,GRID%J_STRT_HALO:GRID%J_STOP_HALO,LM) :: UX,VX
      INTEGER :: J,L
      INTEGER :: J_0STG, J_1STG
      REAL*8 BEGIN
...
!@auth J. Lerner
      ...
      Use GEOM,       Only: AXYP
      USE TRACER_ADV, Only: AADVQ,AADVQ0,sbf,sbm,sfbm,scf,scm,sfcm,ncyc,scf3d
      USE DOMAIN_DECOMP_ATM, only: grid, getDomainBounds
...
!**** Extract domain decomposition info
      ...
      call getDomainBounds (grid, J_STRT = J_0, J_STOP = J_1, J_STRT_STGR = J_0STG, J_STOP_STGR = J_1STG)
...
!**** ADVECT
      ...
      CALL AADVQ (Q,QMOM, .TRUE. ,'q       ')
        ...
        AGC(:,:,jl_zmfntlh) = AGC(:,:,jl_zmfntlh) + sbm(:,:)*sfbm(:,:)*byim
        AGC(:,:,jl_totvtlh) = AGC(:,:,jl_totvtlh) + scf(:,:)
        AGC(:,:,jl_zmfvtlh) = AGC(:,:,jl_zmfvtlh) + scm(:,:)*sfcm(:,:)*byim
!****
!**** convert from mass to concentration units (using updated MMA)
!****
      DO L=1,LM
      DO J=J_0,J_1
      DO I=1,IM
...
!@auth J. Lerner
      ...
      Use TRDIAG_COM, Only: TAJLN=>TAJLN_loc, TAIJN=>TAIJN_LOC, TSCF3D=>TSCF3D_loc, &
                             jlnt_nt_tot,jlnt_nt_mm,jlnt_vt_tot,jlnt_vt_mm, tij_uflx,tij_vflx
#endif
...
!**** Flux diagnostics
        ...
        TAJLN(:,:,jlnt_nt_mm, n) = TAJLN(:,:,jlnt_nt_mm, n) + sbm(:,:)*sfbm(:,:)*byim
        TAJLN(:,:,jlnt_vt_tot,n) = TAJLN(:,:,jlnt_vt_tot,n) + scf(:,:)
        TAJLN(:,:,jlnt_vt_mm, n) = TAJLN(:,:,jlnt_vt_mm, n) + scm(:,:)*sfcm(:,:)*byim
        TSCF3D(:,:,:,n) = scf3d(:,:,:)
#ifdef TRACERS_WATER
!**** vertically integrated atmospheric fluxes
      ...
      if (maxDaysInYear == 0) then ! tidally locked
         Call Stop_Model ('init_UNRDRAG() - tidally locked not supported.', 255)
      end if
      ...
      do IT = 1, maxDaysInYear
         ...
         do J = 1, JM
            if ( LAT_DG(J,2) <= 1.0E-8 .and. x <= 0.0_r8 ) then
               Bt(J,IT) = -Bt_Smax * exp(-((LAT_DG(J,2) + 60.0_r8)/15.0_r8)**2 * aLn2 ) * x
            elseif ( LAT_DG(J,2) > 1.0E-8 .and. x >= 0.0_r8 ) then
               Bt(J,IT) =  Bt_Nmax * exp(-((LAT_DG(J,2) - 60.0_r8)/15.0_r8)**2 * aLn2 ) * x
            else
               Bt(J,IT) = 0.0_r8
            end if
         end do
      ...
      do IT = 1, maxDaysInYear
         ...
         Phi = -10.0_r8 * x
         do J = 1, JM
            Bt(J,IT) = Bt(J,IT) + Bt_Tmax * exp(-( (LAT_DG(J,2) - Phi)/5.0_r8 )**2 * aLn2 ) * 0.25_r8 * ( 3.0_r8 - x )
         end do
      end do
      ...
      implicit none
      real(r8), dimension(IM,GRID%J_STRT_HALO:GRID%J_STOP_HALO,LM) :: U, V, T, SZ, UNRDRAG_x, UNRDRAG_y
      intent(inout) :: T, SZ
      intent(in) :: U, V
      ...
      real(r8), dimension(LM,IM,GRID%J_STRT_HALO:GRID%J_STOP_HALO) :: T2, SZ2, MA2
...
!     call HALO_UPDATE(GRID, MA, from=SOUTH)
      ...
      do J = J_0S, J_1
         ...
         do IP1 = 1, IM
            do L= 1, LM
            T2(L,I,J) = 0.25_r8 * (T(I,J-1,L) + T(IP1,J-1,L) + T(I,J,L) + T(IP1,J,L))
            SZ2(L,I,J) = 0.25_r8 * (SZ(I,J-1,L) + SZ(IP1,J-1,L) + SZ(I,J,L) + SZ(IP1,J,L))
      ...
      if (h_4sq <= 0.0) then
          ...
          call orographic_drag (ue, ve, rhoe, bvfe, h_4sq, Eke_by2, drag_x, drag_y)
      end if
      ...
      do IAZ = 1, N_Az
         ...
         do Ikh = 1, N_Kh
            do IC = 1, N_C
               ...
               B(IC,IAZ,Ikh) = SGN * Bm(Ikh) * exp( -(C(IC,Ikh)/Cw(Ikh))**2 * aLn2 ) * rhoe(IZ0(J))
            end do
         end do
      end do
      ...
      do IAZ = 1, N_Az
         ...
         C_actual(:) = C(:,Ikh) * real(flag, r8) + ( C(:,Ikh) + U_Comp(IZ0(J)) ) * real(1 - flag, r8)
         call nonorographic_drag (C_actual(IC0), dc(Ikh), B(IC,IAZ,Ikh), Eps, Kh(Ikh), hb, rhoe, U_Comp, bvfe, MC, IZ0(J), GW)
      ...
      subroutine nonorographic_drag (c,dc,b,eps,kh,hb,rho,u,bf,nc,iz0, gwfrc)
