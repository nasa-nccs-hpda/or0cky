...
!@var shv heat capacity of water (J/m^3 C)
      real*8, parameter, public :: shv= shv_kg * rho_water
...
ccc include all fluxes except htpr (which is already included)
     ...
     &      -snht
     &      -evaporation*shv*tsn(1)

      flux_in_deriv =
     &     -4.d0*sigma*(tsn(1)+tfrz)**3 - lat_evap*evap_dt - snsh_dt
     &     -evap_dt*shv*tsn(1) - evaporation*shv

      radiation_out = sigma*(tsn(1)+tfrz)**4
      snht = snht + evaporation*shv*tsn(1)
...
ccc snht_out_cor, delta_tsn_impl
     ...
     &     + (evap_dt*shv*tsn(1) + evaporation*shv)*delta_tsn_impl
...
#ifdef GHY_VAPOR_SNSH_ON
        hsn(1) = hsn(1) - evap_corr*dt*(lat_evap + shv*tsn(1))
        snht = snht + evap_corr*shv*tsn(1)
#else
...
ccc repack the layers
      ...
      do n=1,nl
        if( dz(n) .gt. EPS ) then
          ...
          scale_rho = .5d-7 * grav * mass_above
...
!!!   assuming that melting happens at 0 C, so no need to include any
!!!   corrections due to the sensible heat of vapor
