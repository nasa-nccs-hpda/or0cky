...
! this should eventually come from "constant" module
!@var shv_kg specific heat of vapor, Cp (J/kg C)
#ifdef GHY_VAPOR_SNSH_ON
      real*8, parameter :: shv_kg=1.864d3 !(at 300K, engineeringtoolbox.com)
#else
      real*8, parameter :: shv_kg=0.d0 ! (disable for now)
#endif
      
!@var shv heat capacity of water (J/m^3 C)
      real*8, parameter, public :: shv= shv_kg * rhow
...
ccc   input fluxes
      real*8 :: pr,htpr,prs,htprs,srht,trht,geothermal_heat
...
ccc   i.e. they are not multiplied by any fr_...
!@var snshg sensible heat from soil to air (bare/vege) no snow (W/m^2)
!@var snshv sensible heat from vegetation to air (bare/vege) no snow (W/m^2)
!@var snshs sensible heat from snow to air (bare/vege)  (W/m^2)
!@var dsnsh_dt d(sensible heat)/d(soil temp) : same for bare/vege/snow
      real*8 :: snshg(2), snshv(2), snshs(2), dsnsh_dt
...
!@var mCO2cond amount of CO2 condensate in first soil layer (kg)
      real*8 :: mCO2cond
!@var ma1 mass of first atmosphere layer (kg/m^2)
      real*8 :: ma1
...
ccc   and scale it with espect to palnet's gravity (for Earth factor=1)
      do k=1,n
        do ibv=i_bare,i_vege
          h(k,ibv)=h(k,ibv)+zc(k)*grav/9.80665d0
        end do
      end do
      return
...
#ifdef GHY_NO_LIQUID
        ! below the tripple point or too cold:
        ! all iec is at the surface - no diffusion
        evap_max_dry(ibv) = evap_max(ibv) ! + pr ! pr may fall on snow
#else
...
#ifdef GHY_SNOW_EVAP_DRY_SNOW
      qbs = qsat(tsn1(1)+tfrz,lhe+lhm,pres) ! dry snow !!!
      qvs = qsat(tsn1(2)+tfrz,lhe+lhm,pres) ! dry snow !!!
#else
...
c     v_tprime=(vs-vs0)*tprime
      ...
      ! ground
      snshg(1)=sha*rho*ch*(vs*(tp(1,1)-ts+tfrz)-v_tprime)
      snshg(2)=sha*rho*ch*(vs*(tp(1,2)-ts+tfrz)-v_tprime)*eta
      ! canopy
      snshv(1)=0.d0
      snshv(2)=sha*rho*ch*(vs*(tp(0,2)-ts+tfrz)-v_tprime)*(1.d0-eta)
      ! snow
...
#ifdef DRIP_USES_CONSISTENT_FR_SNOW
      use snow_model, only: snow_fraction
#endif
...
#ifndef DRIP_USES_CONSISTENT_FR_SNOW
...
#ifdef DRIP_USES_CONSISTENT_FR_SNOW
        ! pre-calculate the updated snow fraction seen by flg().
        call snow_fraction(dzsn(:,2), nsn(2), snowf, dts, fr_snow(2),
     &       fr_snow_new )
        ! dew/evap flux to/from canopy
        can_evap = evapvw*fw*(1.d0-fm*fr_snow_new)
#endif
...
c**** soils28   common block     9/25/90
c**** bare soil fluxes
      real*8 :: thrm_can, thrm_soil(2),snsh_vapor(0:1,2)
...
!!!! test
 !!!     thrm_can = 0.d0
     ...
     &       + ( -htdripw(1) +
     &       evapb*(elh + shv*tp(1,1)) + snshg(1)
     &       + thrm_soil(1) - srht - trht)
     &       *(1.d0-fr_snow(1))
     &       - htirrig(1) ! adding heat of irrigation water
        snsh_vapor(1,1) = evapb*shv*tp(1,1)
      endif
      ! vegetated soil
      if ( process_vege ) then
        fh(1,2) = - fhsng(2)*fr_snow(2) - fhsng_scale(2)
     &       + ( -htdripw(2)
     &       + evapvg*(elh + shv*tp(1,2)) + snshg(2)
     ...
     &       - htirrig(2) ! adding heat of irrigation water
        snsh_vapor(1,2) = evapvg*shv*tp(1,2)
        ! canopy
        fch(0) = -htpr +
     &       (evapvw*(elh + shv*tp(0,2))*fw + snshv(2) + thrm_can
#ifdef RAD_VEG_GROUND
     &       * (1.d0-trans_sw)
     ...
     &       -srht-trht
#endif
     &       + evapvd*(elh + shv*tp(0,2))*fd)
     &       *(1.d0-fm*fr_snow(2))
        snsh_vapor(0,2) = (evapvw*fw + evapvd*fd)*shv*tp(0,2)
...
c     compute total sensible heat
      snsh_tot(1) = (snshg(1) + snsh_vapor(1,1))*(1.d0-fr_snow(1))
     &     + snshs(1)*fr_snow(1)
      snsh_tot(2) = (snshv(2) + snsh_vapor(0,2))*(1.d0-fr_snow(2)*fm)
     &     + snshs(2)*fr_snow(2)*fm
     &     + (snshg(2) + snsh_vapor(1,2))*(1.d0-fr_snow(2))
      return
      end subroutine flhg
...
c**** soils28   common block     9/25/90
      integer ibv, k, kk
! for debug
      real*8 :: tg_old, ps_old, mcond_old
      tp(:,:) = 0.d0
      fice(:,:) = 0.d0
      do ibv=i_bare,i_vege
        kk=2-ibv
        do k=kk,n
          ...
          if( fsn*w(k,ibv)+ht(k,ibv) .lt. 0.d0 ) then ! all frozen
            ...
            fice(k,ibv)=1.d0
#ifdef DO_CO2_CONDENSATION
!!! do CO2 condensation here
            if ( k==1 .and. ibv==1 ) then
              tg_old = tp(k,ibv)
              ps_old = pres
              mcond_old = mCO2cond
              call process_condensate(pres, ht(k,ibv),
     &             -w(k,ibv)*fsn, shc(k,ibv)+w(k,ibv)*shi,
     &             mCO2cond, tp(k,ibv))
cddd              if(ijdebug==71002)  write(6,*) 'CCC ', tg_old, tp(k,ibv),
cddd     &             mcond_old, mCO2cond, ps_old, pres
            endif
#endif
...
!         endif
     ...
     &   any([tp(1,1),tp(0,2)] < minGroundTemperature)) then
         ...
         write(6,*)'tp(1,1), tp(0,2) = ', tp(1,1), tp(0,2)
...
!-----------------------------------------------------------------------
     ...
     &     srht_in,trht_in,geothermal_heat_in,     ! forcing fluxes
...
!@var longi,latj corresponding coordinate of the cell
     ...
     &     ,fr_snow_in(2)
      ...
      real*8 :: srht_in,trht_in,geothermal_heat_in
      ...
      srht   = srht_in  
      trht   = trht_in 
      geothermal_heat = geothermal_heat_in 
      ts     = ts_in    
      qs     = qs_in    
      pres   = pres_in  
      rho    = rho_in   
...
#ifdef DO_CO2_CONDENSATION_DEBUG_SITES
      if ( ijdebug == 1001 ) then
        write(933,'(a,20(e12.4))') "before",srht,trht,ts,qs,pres,
     &       rho,ch,qm1,vs,vs0,mCO2cond,pr,htpr,prs,htprs,w(1,1),ht(1,1)
      endif
      if ( ijdebug == 63034 ) then ! Viking 2 ! was 26034
        write(934,'(20(e12.4))') orbit_Sl,ts,pres,mCO2cond,vs,qs
      endif
      if ( ijdebug == 64022 ) then ! Curiosity ! was 28022
        write(935,'(20(e12.4))') orbit_Sl,ts,pres,mCO2cond,vs,qs
      endif
#endif
      ...
      call init_step
       ...
       write(933,*) "srht          ", srht          
       write(933,*) "trht          ", trht          
       write(933,*) "geothermal_heat", geothermal_heat
       write(933,*) "ts            ", ts            
       write(933,*) "qs            ", qs            
       write(933,*) "pres          ", pres          
       write(933,*) "rho           ", rho           
      ...
      limit=1000   ! 200 increase to avoid a few more stops
      nit=0
      dtr=dt
...
#ifdef DO_CO2_CONDENSATION_DEBUG
      if ( ijdebug == 71044 ) then
        write(933,'(a,20(e12.4))') "during",srht,trht,ts,qs,pres,
     &       rho,ch,qm1,vs,vs0,mCO2cond,pr,htpr,prs,htprs,w(1,1),ht(1,1)
     &       ,real(nit,kind=kind(1.d0))
      endif
#endif
        ...
        call hydra
        !call qsbal
        !call sensible_heat ! - moved after evaporation
        call xklh
!!! hack to compute time step before evap_limits ...
!        evapb = 1.d0 ; epb=1.d0
!        evapvw = 1.d0 ; evapvd = 1.d0 ; epv = 1.d0
...
c**** more outw outputs
      ...
      write(ichn,*)'snsh ',snshg, snshv
      write(ichn,*)'htpr,srht,trht,geothermal_heat '
     &     ,htpr,srht,trht,geothermal_heat
      return
1000  format(' ',121('='))
1001  format('1')
...
c     write(6,*)'ij,n,k,hmat,ibv,xkl,ibv',ijdebug,n,k,hmat,ibv,xk(k,ibv)
      hmat=h(k,ibv)-zc(k)*grav/9.80665d0
...
!@+   runoff, keeping associated heat in the soil
     ...
     &       eta*(tr_wsn(1:ntg,2,ibv)+tr_wsn(1:ntg,3,ibv))
      ...
      subroutine get_condensate_properties( ps, ts, t_cond, lat_cond,
     &     sh_cond)
      use constant, only : sha
      real*8, intent(in) :: ps ! surface pressure (mbar)
      real*8, intent(in) :: ts ! surface pressure (mbar)
      real*8, intent(out) :: t_cond ! phase change temperature (K)
      real*8, intent(out) :: lat_cond ! latent heat of phase change
      real*8, intent(out) :: sh_cond ! specific heat of condensate (solid)
      !---
      !real*8, parameter :: sh_cond=47.11d0 / 0.044d0 ! heat capacity J/(kg K)
      real*8, parameter :: lat_cond_ref=(8.647 + 15.326)/44.d0*1000
      ! latent heat cond at 216.58 K (tripple point), J/kg
      ! (http://en.wikipedia.org/wiki/Carbon_dioxide_data)
!@param lhe_cond lat. heat of sublimation (J/kg) (Allison)
      real*8, parameter :: lhe_cond=5.902d5 ! ?? -check
!@param t_lhe_cond reference temperature for lhe_cond (K)
      real*8, parameter :: t_lhe_cond=150.d0 ! ?? -check
!@param sh_cond_g specific heat of condensate (gas) (J/kg/K) Wikipedia
      !real*8, parameter :: sh_cond_g=33.89d0/.044
      ! setting it to the number from PlanetParam
      real*8, parameter :: sh_cond_g=770.22727272d0
!@param sh_cond_s specific heat of condensate (solid) (J/kg/K) Wikipedia
      real*8, parameter :: sh_cond_s=47.11d0/.044

#ifdef DO_CO2_CONDENSATION_CC_FORMULA
      t_cond = -3125.d0 / ( log (ps/10.d0) - 20.5d0 )
#elif DO_CO2_CONDENSATION_CC_FORMULA_hack
      t_cond = -3125.d0 / ( log (ps/10.d0) - 20.5d0 )
      if ( ps < 0.4d0 ) then
        t_cond = max(0.d0, t_cond * (ps - 0.2d0)/(0.4d0 - 0.2d0) )
#else
      t_cond = 149.2 + 6.48 * log(0.135 *ps)
#endif
      !! just for test increase t_cond:
      !! t_cond = 170. + 6.48 * log(0.135 *ps)
      !lat_cond = lat_cond_ref - (sh_cond-sha)*(t_cond - 216.58)
      
      !lat_cond = lhe_cond + sh_cond_g*(t_cond-t_lhe_cond)
      lat_cond = lhe_cond + sh_cond_g*(ts-t_lhe_cond)
     &     - sh_cond_s*(t_cond-t_lhe_cond)

      sh_cond = sh_cond_s

      end subroutine get_condensate_properties


      subroutine process_condensate(ps, htt, htlw, hcsw, mcond, tg)
      use constant, only : grav

      real*8, intent(inout) :: ps ! input surface pressure (mbar)
      real*8, intent(inout) :: htt ! total heat in the layer (J/m^2)
      real*8, intent(in) :: htlw !latent heat of water (<0) in the layer(J/m^2)
      real*8, intent(in) :: hcsw ! heat capacity of soil+water (J/K/m^2)
      real*8, intent(inout) :: mcond ! mass of atm. condensate (kg/m^2)
      real*8, intent(out) :: tg ! temperature of the soil layer (K)
!@var a1,a2,a3 parameters for formula Tcond = a1 + a2*Ln(a3*p), 
!@+   which defines the temperature of CO2 condensate, p=pressure (mb)
!@+   (M. Allison notes, Haberle(1982), Pollack(1981)
      real*8, parameter :: a1=149.2d0, a2=6.48d0, a3=0.135d0
!@param lhe_cond lat. heat of sublimation (J/kg) (Allison)
      real*8, parameter :: lhe_cond=5.902d5 ! ?? -check
!@param t_lhe_cond reference temperature for lhe_cond (K)
      real*8, parameter :: t_lhe_cond=150.d0 ! ?? -check
...
!@param sh_cond_s specific heat of condensate (solid) (J/kg/K) Wikipedia
      real*8, parameter :: sh_cond_s=47.11d0/.044
      !---
!@param dsh_cond temperature correction to lhe_cond (J/kg/K)
      real*8, parameter :: dsh_cond = sh_cond_s-sh_cond_g
      !---
      real*8 :: t_cond, lat_cond, dht, mcond_old, dmcond
      integer :: counter
      real*8 :: dmcond_max, ddmcond, mcond_max, mcond_min
      real*8 :: sh_cond

      dmcond_max = .2 * ma1 / 3600.d0 * dts ! allow ~ 20% change per hour
      mcond_max = mcond + dmcond_max
      mcond_min = mcond - dmcond_max
 ...
 10   continue
      ...
      if (counter>50 ) call stop_model("CO2 cond doesn't converge",255)

      call get_condensate_properties(ps, ts, t_cond, lat_cond, sh_cond)

      ! estimate if there is any condensate at all
      tg = (htt-htlw)/hcsw
      if ( tg + tfrz >= t_cond ) then ! no condensate
        mcond = max(0.d0, mcond_min)
        dmcond = mcond-mcond_old
        ps = ps - dmcond*grav * 0.01d0
        htt = htt + dmcond*sh_cond_g*(ts-tfrz)
        return
      endif

      tg = t_cond-tfrz
      dht = (htt-htlw) - hcsw*tg
      mcond = -dht/(lat_cond - sh_cond_g*tg)

!!! debugging
#ifdef DO_CO2_CONDENSATION_DEBUG
      if (mcond>mcond_max .or. mcond<mcond_min ) then
        write(807,*) "DMCOND", ijdebug, counter,
     &       mcond_old, mcond-mcond_old, dmcond_max, t_cond-tfrz,
     &       htt, htlw, mcond, lat_cond, hcsw
        endif
#endif

      mcond = min(mcond, mcond_max)
      mcond = max(mcond, mcond_min)

      ! correct pressure for change in atm. mass
      dmcond = mcond-mcond_old

#ifdef RESTRICT_CONDENSATE_OLD
! trying to restrict the rate of condensation

      dmcond_max = .1 * ma1 / 3600.d0 * dts ! allow ~ 10% change per hour
cddd      if ( dmcond > dmcond_max ) then
cddd        ddmcond = dmcond - dmcond_max
cddd        dmcond = dmcond_max
cddd        mcond = mcond_old + dmcond
cddd        print *, "dmcond", dmcond, ddmcond
cddd        dht = ddmcond * lat_cond
cddd        tg = tg - dht/hcsw
cddd      endif

      if ( abs(dmcond) > dmcond_max ) then
        dmcond = min(  dmcond_max, dmcond )
        dmcond = max( -dmcond_max, dmcond )
        mcond = mcond_old + dmcond
        tg = (htt-htlw+mcond*lat_cond)/hcsw
      endif

      if ( ijdebug==71002) then
        write(6,*) 'BBB', mcond_old, dmcond, dmcond_max, t_cond-tfrz,
     &       htt, htlw, mcond, lat_cond, hcsw
      endif

      ps = ps - dmcond*grav * 0.01d0

!!      if ( dmcond > 0.01d0 ) goto 10
#else
      ps = ps - dmcond*grav * 0.01d0
      htt = htt + dmcond*sh_cond_g*(ts-tfrz)
      if ( dmcond > 0.01d0 ) goto 10
      ...
      ! total heat = htlw + (T-Tfrz)*hcsw
      !       + mcond*(lhe_cond + dsh_cond*(T-t_lhe_cond))


      end subroutine process_condensate
...
c****------------------------------------------------------------------
c**** EARLY DEVELOPMENT NOTES FOR  MODULE sle001 in GHY.f
c****
