...
c    &     pbl_args%TGV = 1.0001d0*pbl_args%TGV
      ...
      ch=atm%chgs(i,j)
      cq=atm%cqgs(i,j)
#ifdef PBL_DISABLE_HORIZONTAL_TRANSPORT
      dpdxr  = 0.d0
      dpdyr  = 0.d0
      dpdxr0 = 0.d0
      dpdyr0 = 0.d0
...
#ifdef TRACERS_DRYDEP
      atm%vd1(:,i,j)=pbl_args%vd1(:)
#ifdef TRACERS_DRYDEP_DIAG
      atm%egs1(:,i,j)=pbl_args%egs1(:)
      atm%egcut_dry1(:,i,j)=pbl_args%egcut_dry1(:)
      atm%eggro_dry1(:,i,j)=pbl_args%eggro_dry1(:)
      atm%egcut_wet1(:,i,j)=pbl_args%egcut_wet1(:)
      atm%eggro_wet1(:,i,j)=pbl_args%eggro_wet1(:)
      atm%egcut_snow1(:,i,j)=pbl_args%egcut_snow1(:)
      atm%eggro_snow1(:,i,j)=pbl_args%eggro_snow1(:)
      atm%vd_for(:,i,j)=pbl_args%vd_for(:)
      atm%vd_cro(:,i,j)=pbl_args%vd_cro(:)
      atm%vd_gra(:,i,j)=pbl_args%vd_gra(:)
      atm%vd_shr(:,i,j)=pbl_args%vd_shr(:)
      atm%vd_bar(:,i,j)=pbl_args%vd_bar(:)
      atm%egs_for(:,i,j)=pbl_args%egs_for(:)
      atm%egs_cro(:,i,j)=pbl_args%egs_cro(:)
      atm%egs_gra(:,i,j)=pbl_args%egs_gra(:)
      atm%egs_shr(:,i,j)=pbl_args%egs_shr(:)
      atm%frac_for(i,j)=pbl_args%frac_for
      atm%frac_cro(i,j)=pbl_args%frac_cro
      atm%frac_gra(i,j)=pbl_args%frac_gra
      atm%frac_shr(i,j)=pbl_args%frac_shr
      atm%frac_bar(i,j)=pbl_args%frac_bar
#endif
      do nx=1,pbl_args%ntx
        n = pbl_args%ntix(nx)
...
c**** wspdf in PBL.f for the other soil types.
     ...
     *     ,idd_qabl1,idd_zhat1,idd_e1,idd_km1,idd_ri1,idd_turb
#endif
...
#else
#ifdef TRACERS_DUST
      INTEGER,PARAMETER :: n_idxd=15+6*npbl+4*(npbl-1)
#endif
#endif
...
C**** QUANTITIES ACCUMULATED HOURLY FOR DIAGDD
...
#ifdef TRACERS_DUST
     &        ,idd_emis,   idd_emis2,  idd_turb,  idd_ws2
     ...
     *       (idd_km1+ii-1,ii=1,npbl-1), (idd_ri1+ii-1,ii=1,npbl-1)
     ...
     &                 * pbl_args%trs(nx) * pbl_args%vd1(n)
                end select
              end if
            end do
#endif
