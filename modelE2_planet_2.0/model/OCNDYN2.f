!**** USE_OPGFQ == 1 differs from USE_OPGFQ /= 1 in that
!**** ocean pressure gradient force evaluates specific volume (m^3/kg) at 3 different pressure levels instead of only 2 levels.
!**** In computing mass weighted altitude Z, dZ of lower part contributes to Z at both lower and upper part of a cell,
!**** whereas dZ of upper part contributes only to Z of upper part, it does not contribute to Z of lower part.
!**** Consequently, dZ of lower part is more heavily mass weighted than is dZ of upper part.
!**** Model E assumes uniform constant gravity, so P (Pa) = m (kg/m^2) * GRAV (m/s^2) and ZG and P are used instead of Z and m.
!**** Inside each layer:  S(h) (kg/kg) = [S0M + 2 h SZM + 6 h^2 SZZM - SZZM/2] / MO*OXYP  where  h = [-.5, .5] 
!**** Although  S(0) = [S0M - SZZM/2] / MO*OXYP , the mass weighted mean value of S is S0M/MO*OXYP over the interval [-.5, .5].
!****
!**** With USE_QUS = 1, second order moment in the vertical is added, and computation at 3 presure levels seems appropriate.         
!**** Assume specific volume V(m) is a cubic polynomial as function of mass in the vertical where m = [-M/2, M/2].
!**** V00 = V(0)  ;  VDN = V[M*Sqrt(3/20)]  ;  VUP = V[-M*Sqrt(3/20)]
!**** Mass integrated V(m) from m = -M/2 to m = M/2 divided by M is:  VBAR = V00*8/18 + VDN*5/18 + VUP*5/18
!**** Z(LMO) at bottom of deepest layer comes from TOPO_OC; at top of layer LMO: Z(LMO-1) = Z(LMO) + VBAR*MO
!**** Within layer L, Z(n) = Z(L) + integral of V(m) with respect to m from n to M/2
!**** Mass weighted Z(n) for entirety of layer L is integral of Z(n) from -M/2 to M/2 divided by M
!**** This double integral with respect to mass is:  VOO*8/36 + VDN*[5+Sqrt(15)]/36 + VUP*[5-Sqrt(15)]/36 = Vweighted/2.
!**** If V = V00 = VDN = VUP, the mass weighted Z(n) of layer L is:  Z(L) + M*V/2, the half way point of the layer.
!**** In Model E:  dZGdP = GRAV * Vweighted  and later  ZG(n) = ZG(L) + dP * dZGdP / 2
!****
!**** Before USE_QUS = 1 was available, vertical structure of ocean heat and salt had mean and vertical gradient;
!**** specific volume was computed at 2 pressure levels in a cell since there were only 2 degrees of freedom in the vertical.
!**** Assume specific volume V(m) is a quadratic polynomial as a function of mass in the vertical where m = [-M/2, M/2].
!**** VDN = V[M/Sqrt(12)]  ;  VDN = V[-M/Sqrt(12)]  ;  VBAR = VDN/2 + VUP/2
!**** Mass weighted Z(n) from -M/2 to M/2 divided by M is:  VDN*[1+Sqrt(3)]/4 + VUP*[1-Sqrt(3)]/4
...
!@+   from extensive-units state variables
      ...
      use ocean, only : GZZM=>GZZMO, SZZM=>SZZMO, USE_QUS, USE_OPGFQ
...
c
      ...
      Real*8, Parameter :: Sqrt3y20 = .387298334621d0  !  = Sqrt(3/20)
      Real*8 :: gup,gdn,sup,sdn,pm,pup,pdn,vup,vdn,bym, g00,s00,p00,v00
...
c**** Extract domain decomposition info
     ...
     &               have_south_pole = have_south_pole)
      ...
      if (use_opgfq /= 1) then
!**** Use Linear Upstream Scheme with repect to mass
      do j=j_0,j_1
      do n=1,nbyzm(j,1)
      do i=i1yzm(n,j,1),i2yzm(n,j,1)
        pe(0) = opress(i,j)
...
C**** In-situ density = 1/specific volume
      ...
      if (use_opgfq == 1) then
!**** Use Quadratic Upstream Scheme with repect to mass
!**** S(h) (kg/kg) = [S0M + 2 h SZM + 6 h^2 SZZM - SZZM/2] / MO*OXYP  where  h = [-.5, .5] 
      do j=j_0,j_1
      do n=1,nbyzm(j,1)
      do i=i1yzm(n,j,1),i2yzm(n,j,1)
        pe(0) = opress(i,j)
        do l=1,lmm(i,j)
          ...
          p3d(l,i,j) = .5*(pe(l)+pe(l-1))
!          pm = p3d(l,i,j)
          pup = p3d(l,i,j) - mo(i,j,l)*grav*Sqrt3y20
          pdn = p3d(l,i,j) + mo(i,j,l)*grav*Sqrt3y20
C**** Specific volume ref to mid-point pressure
          g00 = (g0m(i,j,l) - .5*gzzm(i,j,l)) * bym
          gup = (g0m(i,j,l) - 2*Sqrt3y20*gzm(i,j,l)
     &                      + .4d0*gzzm(i,j,l)) * bym  !  .4 = 6*3/20 - .5
          gdn = (g0m(i,j,l) + 2*Sqrt3y20*gzm(i,j,l)
     &                      + .4d0*gzzm(i,j,l)) * bym
          s00 = (s0m(i,j,l) - .5*szzm(i,j,l)) * bym
          sup = (s0m(i,j,l) - 2*Sqrt3y20*szm(i,j,l)
     &                      + .4d0*szzm(i,j,l)) * bym
          sdn = (s0m(i,j,l) + 2*Sqrt3y20*szm(i,j,l)
     &                      + .4d0*szzm(i,j,l)) * bym
          s00 = max(0d0,s00)
          sup = max(0d0,sup)
          sdn = max(0d0,sdn)
          v00 = volgsp(g00,s00,p3d(l,i,j))
          vup = volgsp(gup,sup,pup)
          vdn = volgsp(gdn,sdn,pdn)
          vbar(l,i,j) = (8*v00 + 5*vup + 5*vdn) / 18
C**** In-situ temperature
          t3d(l,i,j) = (8*temgsp(g00,s00,p3d(l,i,j)) +
     +                  5*temgsp(gup,sup,pup)+5*temgsp(gdn,sdn,pdn)) /18
C**** In-situ density = 1/specific volume
          rho(l,i,j)  = 1d0/vbar(l,i,j)
        enddo
      enddo
...
C****
     ...
     *                 G0M,GZM=>GZMO, S0M,SZM=>SZMO, DXYPO
      Use OCEAN, Only: GZZM=>GZZMO, SZZM=>SZZMO, USE_OPGFQ
      ...
      Real*8,Parameter :: z12eH=.28867513d0  !  z12eH = 1/SQRT(12)
      Real*8,Parameter :: Sqrt3y20 = .387298334621d0,  !  = Sqrt(3/20)
     &                       x8y36 = .222222222222d0,  !  = 8/36
     &                x5mSqrt15y36 = .031306018161d0,  !  = [5-Sqrt(15)]/36
     &                x5pSqrt15y36 = .246471759617d0   !  = [5+Sqrt(15)]/36
      Integer*4 I,J,L, J1,J1H,JN,J1A,JNH, N
      Logical :: QNP
      Real*8,External   :: VOLGSP
      Real*8,Dimension(IM,GRID%J_STRT_HALO:GRID%J_STOP_HALO,LMO) ::
     *          P
      Real*8 :: G00,S00, PUP,PDN, V00,VUP,VDN, smean
C****
C**** Extract domain decomposition band parameters
C****                          Band1  Band2  BandM
      J1  = GRID%J_STRT     !    1      5     JM-3   Band minimum
...
C**** Calculate pressure by integrating from the top down
      ...
      do l=lmo,1,-1
        If (USE_OPGFQ /= 1) Then
!**** Use Linear Upstream Scheme with respect to mass
        Do J=J1H,JNH
          do n=1,nbyzm(j,l)
            do i=i1yzm(n,j,l),i2yzm(n,j,l)
        ...
        If (USE_OPGFQ == 1) Then
!**** Use Quadratic Upstream Scheme with respect to mass
!**** S(h) (kg/kg) = [S0M + 2 h SZM + 6 h^2 SZZM - SZZM/2] / MO*OXYP  where  h = [-.5, .5] 
        Do J=J1H,JNH
          do n=1,nbyzm(j,l)
            do i=i1yzm(n,j,l),i2yzm(n,j,l)
              MMI(I,J,L) = MO(I,J,L)*DXYPO(J)
              G00 = (G0M(I,J,L) - .5*GZZM(I,J,L)) / MMI(I,J,L)
              GUP(I,J,L) = (G0M(I,J,L) - 2*Sqrt3y20*GZM(I,J,L)
     +                                 + .4d0*GZZM(I,J,L)) / MMI(I,J,L)  !  .4 = 6*3/20 - .5
              GDN(I,J,L) = (G0M(I,J,L) + 2*Sqrt3y20*GZM(I,J,L)
     +                                 + .4d0*GZZM(I,J,L)) / MMI(I,J,L)
              S00 = (S0M(I,J,L) - .5*SZZM(I,J,L)) / MMI(I,J,L)
              SUP(I,J,L) = (S0M(I,J,L) - 2*Sqrt3y20*SZM(I,J,L)
     +                                 + .4d0*SZZM(I,J,L)) / MMI(I,J,L)
              SDN(I,J,L) = (S0M(I,J,L) + 2*Sqrt3y20*SZM(I,J,L)
     +                                 + .4d0*SZZM(I,J,L)) / MMI(I,J,L)
              smean = s0m(i,j,l)/mmi(i,j,l)
              S00        = MAX(S00       , .5d0*smean)
              SUP(I,J,L) = MAX(SUP(I,J,L), .5d0*smean)
              SDN(I,J,L) = MAX(SDN(I,J,L), .5d0*smean)
              PUP = P(I,J,L) - MO(I,J,L)*GRAV*Sqrt3y20
              PDN = P(I,J,L) + MO(I,J,L)*GRAV*Sqrt3y20
              V00 = VOLGSP (G00       ,S00       ,P(I,J,L))
              VUP = VOLGSP (GUP(I,J,L),SUP(I,J,L),PUP)
              VDN = VOLGSP (GDN(I,J,L),SDN(I,J,L),PDN)
              dZGdP(I,J,L) = 2*(V00*x8y36 + VUP*x5mSqrt15y36 
     +                                    + VDN*x5pSqrt15y36)
              VBAR(I,J,L) = (8*V00 + 5*VUP + 5*VDN) / 18
              DH3D(I,J,L) = MO(I,J,L)*VBAR(I,J,L)
            enddo
          enddo
        enddo
