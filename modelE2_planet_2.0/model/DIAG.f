...
c stratosphere
     ...
     &       max(0.d0,log(t(i,j,lstr)/t(i,j,ls1))))
        rossl = min(nh*byfcor,sqrt(nh*bybeta))
        richn = 999d0 ! for now
        do it=1,ntype
...
! please use critically:
     ...
     &         trm(:,:,1,n_SO4_d3)
          long_name =
     &         'Layer 1 Mass of Sulfate + Sulfate Coat on Dust'
#endif
#endif
#if (defined CLD_AER_CDNC) || (defined AIE_DIAG_FIX_MET)
...
C**** accumulating/averaging mode ***
            ...
            case ("SO4_d1")
              ...
              units_of_data = 'kg'
              long_name = 'Mass of Sulfate Coat on 0.1 to 1 um Dust'
            case ("SO4_d2")
              datar8= trm(:,:,l,n_SO4_d2)
              units_of_data = 'kg'
              long_name = 'Mass of Sulfate Coat on 1 to 2 um Dust'
            case ("SO4_d3")
              datar8=trm(:,:,l,n_SO4_d3)
              units_of_data = 'kg'
              long_name = 'Mass of Sulfate Coat on 2 to 4 um Dust'
            case ("Clay")
              datar8=trm(:,:,l,n_Clay)
              units_of_data = 'kg'
              long_name = 'Mass of Clay Dust 0.1 to 1 um'
            case ("Silt1")
              datar8=trm(:,:,l,n_Silt1)
              units_of_data = 'kg'
              long_name = 'Mass of Silt Dust 1 to 2 um'
            case ("Silt2")
              datar8=trm(:,:,l,n_Silt2)
              units_of_data = 'kg'
              long_name = 'Mass of Silt Dust 2 to 4 um'
            case ("Silt3")
              datar8=trm(:,:,l,n_Silt3)
              units_of_data = 'kg'
              long_name = 'Mass of Silt Dust 4 to 8 um'
#endif
            case ("CLDSS")
...
C**** From DIAGB (PM, PMO are fixed, PL,PLO will vary)
      PM(1)=psf*1.2d0 ! ensures below surface for extrapolation
      DO L=2,LM+1
        PL(L)=pednl00(l)
        PM(L)=pednl00(l)
      END DO
...
!@auth Group
     ...
     *     ij_RTSE, ij_HWV, ij_PVS, ij_pland,
     ...
     *     ij_lwaerabs,ij_swaerabsnt,ij_lwaerabsnt
      ...
      DO I=I_0,IMAXJ(J)
        ...
        k = ij_fland
        aij(i,j,k) = (1-focean(i,j))*idacc(ia_src)-aij(i,j,ij_lk)
     ...
     &       *idacc(ia_inst)
        ...
        k = ij_pland
        aij(i,j,k) = idacc(ia_ij(k))*(1 - focean(i,j))
...
C**** Find MSU/SSU channels (simple lin.comb. of Temps)
        pland = 1-focean(i,j)-aij(i,j,ij_lk)/idacc(ia_src)
