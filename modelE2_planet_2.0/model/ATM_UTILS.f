...
!@+   (ever?), the level is set to the pressure level below ptropmin.
!@+
      USE RESOLUTION, only : klev=>lm,psf
...
!@param zplimb max pressure at which to define tropopause (mb)
!@param ptropmax max pressure at which to define tropopause (mb)
!@param ptropmin min pressure at which to define tropopause (mb)
      real*8, parameter :: zgwmo=-2d-3*psf/984.d0,
     &                     zgwmo2=-3d-3*psf/984.d0,
     &                     zdeltaz=2000.d0,
     &                     zfaktor=-GRAV/RGAS,
     &                     zplimb=500.d0*psf/984.d0,
     &                     ptropmax=600.d0*psf/984.d0,
     &                     ptropmin=30.d0*psf/984.d0
...
c**** set limits based on pressure
      do jk=2,klev-1
        if (papm1(jk-1).gt.ptropmax) then
          iplimb=jk
        else
          if (papm1(jk).lt.ptropmin) exit
        end if
      end do
      iplimt=jk
c****
...
c****
 ...
 2000 continue
      ...
      if (ltset.eq.-999) then
        ltropp=iplimt-1  ! default = last level below ptropmin
        if(trim(planet_name).eq.'Earth') then
        print*,"In tropwmo ltropp not set, using default: ltropp ="
     *       ,ltropp
        write(6,'(20(I4,5F10.5,/))') (l,ptm1(l),papm1(l),pk(l),zdtdz(l)
