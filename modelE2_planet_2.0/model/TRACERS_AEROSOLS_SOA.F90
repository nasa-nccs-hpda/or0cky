...
#endif  /* SOA_DIAGS */
  if (PCP.ne.PCP) then
    call stop_model('PCP is NaN in SOA, but search for the problem elsewhere.',255)
  endif
!
...
#endif  /* SOA_DIAGS */
  enddo
  sum_soamass=sum(soamass)
  if (sum_soamass.ne.sum_soamass) then
    call stop_model('soamass is NaN in SOA, but search for the problem elsewhere.',255)
  endif
  if (sum_soamass==0.d0) goto 60 ! no semivolatiles, nothing to do
  M0a=PCP
  M0b=PCP
  do i=1,nsoa
    M0b=M0b+soamass(i)
...
11  continue ! Try with another M0
  if (iternum==0) then
    ...
    if (x2 <= 0.d0) M0err_curr=max(tiny(x2),(x2+sum_soamass)*M0err)
    x1=x2-M0err_curr
