...
!@ver  beta new_ prefix avoids name clash with the default version
      use model_com
      use domain_decomp_atm, only : grid
      use pario, only : write_attr, read_attr, get_natts,variable_exists
      use Dictionary_mod
      implicit none
      integer fid   !@var fid file id
      integer iaction !@var iaction flag for reading or writing to file
...
c strings with the character "|".
        ...
        enddo
      case (ioread) ! input
        if(variable_exists(grid,fid,'iparam')) then
     ...
     &           call set_param( trim(pname), pvali, plen, 'o' )
        ...
        if(variable_exists(grid,fid,'rparam')) then
     ...
     &           call set_param( trim(pname), pvalr, plen, 'o' )
        ...
        if(variable_exists(grid,fid,'cparam')) then
