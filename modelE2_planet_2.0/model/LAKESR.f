!**** LAKESR.f = overhaul of RIVERF eliminating emergency directions
...
!@dbparam RIVER_FAC (1/s) = river SPEED (m/s) / dH (m)
      REAL*8 :: river_fac=4.
...
!@auth Raul Garza-Robles
      ...
      USE LAKES, ONLY: KDIREC,IFLOW,JFLOW, DHORZ, XYZC
      IMPLICIT NONE
     ...
     *            DHORZ (I_0H:I_1H,J_0H:J_1H))
      Allocate ( XYZC (3,I_0H:I_1H,J_0H:J_1H))
...
!**** and enthalpy per unit area (?/m^2 over ocean fraction)
         ...
         EFLiqIn(I,J) = (EFLiqIn(I,J) - MFLiqIn(I,J)*GZATMO(I,J)) /
     /                  (FOCEAN(I,J)*AXYP(I,J))
#ifdef TRACERS_WATER
