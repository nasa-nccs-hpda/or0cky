...
Filesystem copy of the "ROCKE-3D → JAX: Status" slide deck (2026-09-21),
kept alongside `STATUS.md` in this directory.

**Live, viewable/presentable version**: https://claude.ai/artifact/LxdYuYeQXxHDsX18KWxBLA

**What's here**:
- `status_deck.pdf` — a real, standalone-viewable PDF of all 3 slides. Open
...
- `build_pdf.py` — generates `status_deck.pdf` from the content above using
  ...
  Google Fonts dependency) in the same navy/green/orange color scheme —
  it's a faithful content recreation, not a pixel-identical export of the
  live Artifact. Re-run with `python3 build_pdf.py status_deck.pdf` if the
  content changes; it has the slide text hardcoded, so edits go in that
  script (and ideally in `slides/*.html`/the live deck too, to keep them in
  sync).
...
automatically. Everything here is a snapshot as of 2026-09-21.
