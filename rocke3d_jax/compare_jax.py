...
def main():
    ...
    jax.block_until_ready((T_out, Q_out))
    drycnv_time = (time.time() - start) / ITER

    np.save(os.path.join(DATA_DIR, f"drycnv_jax_{device}_T_out.npy"), np.asarray(T_out))
    np.save(os.path.join(DATA_DIR, f"drycnv_jax_{device}_Q_out.npy"), np.asarray(Q_out))
    ...
    print(f"DRYCNV mean time ({device}): {drycnv_time:.6e} s/call")
